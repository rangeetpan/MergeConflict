...
#else
#include "components/dom_distiller/content/common/mojom/edge_distillability_service.mojom.h"
#endif
#if !defined(MICROSOFT_EDGE_BUILD) || defined(OS_ANDROID)
...
#include "components/edge_entity_extraction/content/common/entity_extraction.mojom.h"
#include "components/edge_entity_extraction/core/common/edge_entity_extraction_features.h"
#include "components/edge_learning_tools/content/common/learning_tools_javascript_service.mojom.h"
#include "components/edge_learning_tools/content/browser/learning_tools_helper.h"
#if defined(OS_MACOSX)
#include "components/edge_mac_touch_bar/content/common/mac_touch_bar_javascript_service.mojom.h"
...
#if defined(MICROSOFT_EDGE_BUILD) && !defined(OS_ANDROID)
#include "components/dom_distiller/content/browser/edge_distiller_javascript_service_impl.h"
#include "components/dom_distiller/content/browser/edge_reading_view_tab_helper.h"
#include "components/dom_distiller/content/common/mojom/edge_distiller_javascript_service.mojom.h"
#endif
...
>>>>>>> d6300f332f9b643e4f488f153e85fb369460f353
...
#include "chrome/browser/ui/webui/edge_downloads/edge_downloads_ui.h"
...
#include "components/edge_hj/browser/page_content_info.h"
#include "components/edge_hj/content/common/mojom/anchor_click.mojom.h"
#include "components/edge_hj/content/common/mojom/page_content_host.mojom.h"
...
void BindDistillabilityService(
  ...
  dom_distiller::ReadingViewTabHelper* reading_view_tab_helper =
      dom_distiller::ReadingViewTabHelper::FromWebContents(web_contents);
  if (reading_view_tab_helper)
    driver = reading_view_tab_helper->GetDistillabilityDriver();
#else
  driver = dom_distiller::DistillabilityDriver::FromWebContents(web_contents);
#endif
  if (!driver)
    return;
...
#if defined(MICROSOFT_EDGE_BUILD) && !defined(OS_ANDROID)
  dom_distiller::CreateDistillerJavaScriptService(std::move(receiver),
                                                  frame_host);
#else
...
void PopulateChromeFrameBinders(
  ...
  if (extraction::features::edge::IsEntityExtractionEnabled()) {
    map->Add<entity_extraction::mojom::ContentEntityExtractionDriver>(
        base::BindRepeating(&entity_extraction::EntityExtractionDriver::
                                BindContentEntityExtractionDriver));
  }

#if defined(EDGE_HISTORY_JOURNAL_TELEMETRY)
  if (edge_hj::features::IsEdgeHJTelemetryEnabled()) {
    if (edge_hj::features::IsEdgeHJPageContentInfoEnabled()) {
      map->Add<edge_hj::mojom::PageContentHost>(
          base::BindRepeating(&edge_hj::PageContentInfo::Create));
    }

    if (edge_hj::features::IsEdgeHJAnchorClickedEnabled()) {
      map->Add<edge_hj::mojom::AnchorElementClickMetricsHost>(
          base::BindRepeating(&edge_hj::AnchorClickMetrics::Create));
    }
  }
#endif  // defined(EDGE_HISTORY_JOURNAL_TELEMETRY)
  ...
  map->Add<edge::learning_tools::mojom::GrammarToolsCommandFromScript>(
    base::BindRepeating(&edge::learning_tools::LearningToolsHelper::
                            BindGrammarToolsCommandFromScript));

  map->Add<edge::learning_tools::mojom::ReadAloudCommandFromScript>(
      base::BindRepeating(&edge::learning_tools::LearningToolsHelper::
                              BindReadAloudCommandFromScript));
#if defined(OS_MACOSX)
  map->Add<edge::mac_touch_bar::mojom::MacTouchBarJavaScriptService>(
      base::BindRepeating(&edge::mac_touch_bar::MacTouchBarTabHelper::
                              BindMacTouchBarJavaScriptService));
#endif
}
