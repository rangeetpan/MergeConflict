...
>>>>>>> 3607308fe26ae5d207dc84e7cce394ee06af34e4
...
#include "edge_pdf/edge_pdf_features.h"
...
#include "pdf/pdf_trace_common.h"
...
// kJSIsFitToChanged, kJSFitToState represent the view changes and state.
constexpr char kJSIsFitToChanged[] = "isFitToChanged";
constexpr char kJSFitToState[] = "fitToState";
...
constexpr char kJSLoadErrorType[] = "errorType";
...
constexpr char kJSPasswordCancel[] = "getPasswordUserCancelled";
// Print (Page -> Plugin)
constexpr char kJSPrintType[] = "print";
// Save (Page -> Plugin)
constexpr char kJSSaveType[] = "save";
constexpr char kJSSaveStatusType[] = "recordSaveStatus";
constexpr char kJSToken[] = "token";
constexpr char kJSForce[] = "force";
constexpr char kJSStatus[] = "status";
...
constexpr char kJSSetHasEditsType[] = "setHasEdits";
...
// Document focus state (Page -> Plugin)
constexpr char kJSDocumentFocus[] = "documentFocus";
constexpr char kJSDocumentFocusState[] = "state";

// Print restriction in document (Plugin -> Page)
constexpr char kJSDisablePrinting[] = "disablePrinting";
constexpr char kJSContentRestriction[] = "isContentRestriction";

// Show the sign-in dialog (Page -> Plugin)
constexpr char kJSShowSignInDialog[] = "showSignInDialog";

// Notify plugin whether toolbar is pinned (Page -> Plugin)
constexpr char kJSSetIsToolbarPinnedType[] = "setIsToolbarPinned";
constexpr char kJSPinned[] = "pinned";

constexpr int kFindResultCooldownMs = 100;

// Do not save forms with over 2 GB. This cap should be kept in sync
// with and is also enforced in chrome/browser/resources/edge_pdf/pdf_viewer.js.
constexpr size_t kMaximumSavedFileSizeUpperLimit = 2000u * 1000u * 1000u;

// Do not save forms with over 100 MB. This cap should be kept in sync
// with and is also enforced in chrome/browser/resources/edge_pdf/pdf_viewer.js.
constexpr size_t kMaximumSavedFileSizeLowerLimit = 100u * 1000u * 1000u;
...
// Enum indicating the document protection handler.
enum DocumentProtectionHandler {
  None = 0,
  MicrosoftInformationProtection = 1,
  Other = 2
};
...
void UpdateSystemColors(
    PP_Instance instance,
    const PP_PdfUpdateSystemColorParams& system_color_params) {}
...
const PPP_Pdf ppp_private = {
    ...
    &UpdateSystemColors,
    &PdfPrintBegin,
    nullptr /*OnAccountDetailsReceived*/,
    nullptr /*SendMIPHttpResponse*/,
    nullptr /*OnPdfAuthError*/,
};
...
bool IsSaveDataSizeValid(size_t size, size_t save_size_limit_) {
  return ((size > 0) && (size <= save_size_limit_));
}
...
OutOfProcessInstance::OutOfProcessInstance(PP_Instance instance)
    : pp::Instance(instance),
      ...
      uma_(this) {
  TRACE_EVENT0("pdf_plugin", "First Render Begins");
  ...
  save_size_limit_ =
      base::FeatureList::IsEnabled(pdf::features::edge::kEdgePDFSaveLimit)
          ? kMaximumSavedFileSizeUpperLimit
          : kMaximumSavedFileSizeLowerLimit;
}

OutOfProcessInstance::~OutOfProcessInstance() {
  // Log load status if not done already.
  if (!is_loading_complete_ && is_open_recorded_) {
    if (((base::Time::Now() - last_byte_fetch_) <
         base::TimeDelta::FromSeconds(kTimeToRenderInSeconds)) ||
        is_password_requested_) {
      LogLoadState(LoadState::TabClosedBeforeLoadingCouldComplete);
    } else {
      LogLoadState(LoadState::Inconclusive);
    }
  }
...
bool OutOfProcessInstance::Init(uint32_t argc,
  ...
  LogLoadState(LoadState::PdfOpened);
  is_open_recorded_ = true;
  ...
  for (uint32_t i = 0; i < argc; ++i) {
    ...
    } else if (strcmp(argn[i], "first-page-separator") == 0) {
      success =
          base::StringToInt(argv[i], &first_page_separator_in_viewport_coords_);
    } else if (strcmp(argn[i], "javascript") == 0) {
      enable_javascript = (strcmp(argv[i], "allow") == 0);
    } else if (strcmp(argn[i], "stream_timestamp") == 0) {
      base::StringToInt64(argv[i], &stream_creation_timestamp_);
    }
    if (!success)
      return false;
  }
  ...
  if (!engine_) {
    ...
    engine_ = PDFEngine::Create(
        this, /* enable_javascript =*/!(
            base::FeatureList::IsEnabled(features::kDisableJavascriptInPDF)));
  }
...
void OutOfProcessInstance::HandleMessage(const pp::Var& message) {
  ...
  if (type == kJSViewportType) {
    ...
    if (!(dict.Get(pp::Var(kJSXOffset)).is_number() &&
          ...
          dict.Get(pp::Var(kJSIsFitToChanged)).is_bool() &&
          dict.Get(pp::Var(kJSFitToState)).is_number())) {
      NOTREACHED();
      return;
    }
    ...
    TRACE_EVENT2("pdf_plugin", "Viewport Message Handle", "plugin_height",
                 plugin_size_.height(), "y_offset", scroll_offset.y());
    ...
    if (dict.Get(pp::Var(kJSIsFitToChanged)).AsBool()) {
      int32_t fit_to_state = dict.Get(pp::Var(kJSFitToState)).AsInt();
      // bound check before casting fit_to_state(int) to enum
      if (fit_to_state < PP_FittingType::PP_FITTOPAGE ||
          fit_to_state > PP_FittingType::PP_NONE) {
        NOTREACHED();
        return;
      }
      // notify fit-to state changed to plugin's host.
      pp::PDF::NotifyFitToChanged(this,
                                  static_cast<PP_FittingType>(fit_to_state));
    }
  } else if (type == kJSPasswordCancel) {
    ...
    base::FeatureList::TriggerUsage(pdf::features::edge::kEdgePDFSave);
  ...
  } else if (type == kJSSaveStatusType) {
    const bool status = dict.Get(pp::Var(kJSStatus)).AsBool();
    UMA_HISTOGRAM_BOOLEAN("Microsoft.Pdf.SaveStatus", status);
    ...
    PostMessage(reply);
  } else if (type == kJSDocumentFocus) {
    if (dict.Get(pp::Var(kJSDocumentFocusState)).AsBool())
      OnFocusGained();
    else
      OnFocusLost();
  } else if (type == kJSShowSignInDialog) {
    pp::PDF::ShowSignInDialog(this);
  } else if (type == kJSSetIsToolbarPinnedType) {
    if (!(dict.Get(pp::Var(kJSPinned)).is_bool())) {
      NOTREACHED();
      return;
    }
    pp::PDF::SetPdfBarLastPinStatePref(GetPluginInstance(),
                                       dict.Get(pp::Var(kJSPinned)).AsBool());
  } else {
    NOTREACHED();
  }
}
...
void OutOfProcessInstance::DidChangeView(const pp::View& view) {
  ...
  if (!stop_scrolling_) {
    ...
    pp::FloatPoint scroll_offset_float(
        scroll_offset_.x(),
        scroll_offset_.y() - first_page_separator_in_viewport_coords_);
...
void OutOfProcessInstance::SendAccessibilityViewportInfo() {
  ...
  viewport_info.scroll.y = -first_page_separator_in_viewport_coords_;
...
void OutOfProcessInstance::RecordFirstRenderTime() {
  if (IsPrintPreview())
    return;
  if (stream_creation_timestamp_ < 0)
    return;
  base::TimeDelta first_render_time =
      base::Time::Now() - base::Time::FromJavaTime(stream_creation_timestamp_);
  TelemetryHelper::RecordFirstRenderTime(first_render_time);
  TRACE_EVENT_INSTANT0("pdf_plugin", "FirstRenderComplete",
...
void OutOfProcessInstance::DidOpen(int32_t result) {
  if (result == PP_OK) {
    if (!engine_->HandleDocumentLoad(embed_loader_)) {
      document_load_state_ = LOAD_STATE_LOADING;
      DocumentLoadFailed(engine_->GetLastError());
    }
  } else if (result != PP_ERROR_ABORTED) {  // Can happen in tests.
    DocumentLoadFailed(FileOpenError::UrlLoaderOpenError);
  }
}
...
void OutOfProcessInstance::ScrollToY(int y_in_screen_coords,
  ...
  if (compensate_for_toolbar) {
    new_y_viewport_coords -= first_page_separator_in_viewport_coords_;
  }
...
bool OutOfProcessInstance::ShouldSaveEdits() const {
  return edit_mode_ &&
         ((base::FeatureList::IsEnabled(
          ...
          (base::FeatureList::IsEnabled(pdf::features::edge::kEdgePDFSave)));
}

bool OutOfProcessInstance::UsePluginForSave() {
  return (ShouldSaveEdits() ||
          (base::FeatureList::IsEnabled(pdf::features::edge::kEdgePDFSave) &&
           IsLocalFile(GetURL())));
}

bool OutOfProcessInstance::IsLocalFile(const std::string& url) const {
  return GURL(url).SchemeIsFile();
}
...
void OutOfProcessInstance::SaveToBuffer(const std::string& token) {
  ...
  if (ShouldSaveEdits()) {
    ...
    if (IsSaveDataSizeValid(data.size(), save_size_limit_)) {
    ...
    if (IsSaveDataSizeValid(length, save_size_limit_)) {
...
void OutOfProcessInstance::DocumentLoadComplete(
  ...
  LogLoadState(LoadState::Success);
  ...
  if (!engine_->HasPermission(PDFEngine::PERMISSION_PRINT_LOW_QUALITY) &&
    ...
    // Send information about print restriction to PdfViewer
    pp::VarDictionary message;
    message.Set(pp::Var(kType), pp::Var(kJSDisablePrinting));
    message.Set(pp::Var(kJSContentRestriction), pp::Var(true));
    PostMessage(message);
  }

  pp::PDF::SetContentRestriction(this, content_restrictions);
  pp::PDF::SetPluginCanSave(this, UsePluginForSave());
...
void OutOfProcessInstance::DocumentLoadFailed(FileOpenError error) {
  DCHECK_EQ(LOAD_STATE_LOADING, document_load_state_);
  UserMetricsRecordAction("PDF.LoadFailure");
  is_loading_complete_ = true;
  HistogramEnumerationDeprecated(
      "Microsoft.Pdf.FileOpenError", static_cast<int32_t>(error),
      static_cast<int32_t>(chrome_pdf::FileOpenError::kMaxValue));
  if (TelemetryHelper::IsDocumentLoadErrorExpected(error))
    LogLoadState(LoadState::ExpectedFailure);
  else
    LogLoadState(LoadState::Failure);
  if (did_call_start_loading_) {
    pp::PDF::DidStopLoading(this);
    did_call_start_loading_ = false;
  }
  ...
  message.Set(pp::Var(kJSLoadErrorType), pp::Var(int32_t(error)));
  PostMessage(message);
...
void OutOfProcessInstance::DocumentLoadProgress(uint32_t available,
  ...
  // last_byte_fetch_ helps in tracking file open status of PDF.
  if (progress > last_progress_sent_) {
    last_byte_fetch_ = base::Time::Now();
  }
...
void OutOfProcessInstance::OnGeometryChanged(double old_zoom,
  ...
  int bottom_of_document =
      ...
      (first_page_separator_in_viewport_coords_ * device_scale_);
...
void OutOfProcessInstance::IsEditModeChanged(bool is_edit_mode) {
  if (is_edit_mode) {
    pp::VarDictionary reply;
    reply.Set(pp::Var(kType), pp::Var(kJSSetHasEditsType));
    PostMessage(reply);
  }
  if (is_edit_mode == edit_mode_) {
    return;
  }
...
float OutOfProcessInstance::GetToolbarHeightInScreenCoords() {
  return first_page_separator_in_viewport_coords_ * device_scale_;
}
...
pp::FloatPoint OutOfProcessInstance::BoundScrollOffsetToDocument(
  ...
  float min_y = -first_page_separator_in_viewport_coords_;
  float max_y = std::max(
...
void OutOfProcessInstance::OnFocusGained() {
  focus_timestamp_ = base::Time::Now();
  is_in_focus_ = true;
}

void OutOfProcessInstance::OnFocusLost() {
  if (is_in_focus_ && !is_print_preview_) {
    total_focus_time_ += (base::Time::Now() - focus_timestamp_);
  }
  is_in_focus_ = false;
}

// This is not optimal, and is just a work around until https://crbug.com/911161
// is fixed by someone. Ideally we want to log this without involving plugin
// process.
void OutOfProcessInstance::LogDwellTime() {
  if (!is_print_preview_) {
    if (is_in_focus_) {
      total_focus_time_ += (base::Time::Now() - focus_timestamp_);
    }
    TelemetryHelper::LogDwellTime(total_focus_time_);
  }
}

void OutOfProcessInstance::LogLoadState(LoadState load_state) {
  HistogramEnumerationDeprecated(
      "Microsoft.Pdf.LoadState", static_cast<int32_t>(load_state),
      static_cast<int32_t>(chrome_pdf::LoadState::kMaxValue));
}
