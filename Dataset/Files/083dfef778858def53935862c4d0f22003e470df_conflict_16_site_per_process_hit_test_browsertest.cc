...
IN_PROC_BROWSER_TEST_F(SitePerProcessHitTestBrowserTest, PopupMenuTest) {
#if defined(OS_MACOSX)
  if (features::edge::IsControlsFluentStyleEnabled()) {
    // There is no special behavior for the fluent select on Mac.
    return;
  }
#endif
  GURL main_url(
  ...
  if (!IsScreenTooSmallForPopup(screen_info)) {
    ...
    EXPECT_EQ(popup_rect.y() - rwhv_root->GetViewBounds().y(), 98);
  }
#endif
...
IN_PROC_BROWSER_TEST_F(SitePerProcessHitTestBrowserTest,
  ...
  if (!IsScreenTooSmallForPopup(screen_info)) {
    EXPECT_EQ(popup_rect.x() - rwhv_root->GetViewBounds().x(), 354);
    EXPECT_EQ(popup_rect.y() - rwhv_root->GetViewBounds().y(), 158);
  }
#endif
  ...
  if (!IsScreenTooSmallForPopup(screen_info)) {
    ...
    EXPECT_EQ(popup_rect.y() - rwhv_root->GetViewBounds().y(), 252);
  }
#endif
}
