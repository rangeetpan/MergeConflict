...
#include "chrome/browser/edge_domain_actions/domain_actions_parsing_helpers.h"
#include "chrome/browser/edge_session_context/edge_session_context.h"
...
#include "chrome/browser/safe_browsing/password_protection_client.h"
...
#include "components/parked_domains/edge_parked_domains_features.h"
#include "components/parked_domains/parked_domains_checker.h"
#include "components/parked_domains/parklist_loader.h"
...
#include "components/variations/edge_pref_names.h"
...
=======
>>>>>>> 3c8fc9c439c35d92bb9e6d62d26e3946a426c3f9
...
#include "components/edge_smartscreen/common/features.h"
#include "components/edge_smartscreen/common/smartscreen_prefs.h"

#if defined(OS_WIN)
#include "chrome/browser/edge_security_preference_win.h"
#include "chrome/install_static/install_util.h"
#endif  // OS_WIN
#endif  // BUILDFLAG(ENABLE_SMARTSCREEN)

#if BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
#include "chrome/browser/password_manager/edge_breach_detection/breach_detection_service.h"
#include "components/password_manager/core/common/edge_password_manager_features.h"
#endif  // BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
...
#if BUILDFLAG(ENABLE_WDAG) || BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
#include "base/edge_dlp/edge_dlp.h"
#endif

#if BUILDFLAG(ENABLE_WDAG)
#include "chrome/browser/win/wdag/manager.h"
...
void BrowserProcessImpl::Init() {
  ...
  // (typically this call is a no-op as MetricsServicesManager has already been
  // created).
  if (!IsWindowsServer() && IsWindows10OrGreater()) {
    GetMetricsServicesManager();
    DCHECK(metrics_services_manager_client_);
    metrics_services_manager_client_->OnWindowsSettingsCreated();
  }
#endif  // OS_WIN
  ...
  pref_change_registrar_.Add(
      metrics::prefs::kUrlReportingEnabled,
                             base::Bind(&ApplyUrlReportingPolicy));
#endif

#if BUILDFLAG(ENABLE_SMARTSCREEN)
  if (security::features::edge::IsSmartScreenEnabled()) {
    UMA_HISTOGRAM_BOOLEAN("Microsoft.SmartScreen.SmartScreenEnabled",
        g_browser_process->local_state()
            ->GetBoolean(prefs::kSmartScreenEnabled));

    if (security::features::edge::IsPuaRepFeatureEnabled()) {
      UMA_HISTOGRAM_BOOLEAN("Microsoft.SmartScreen.PuaProtectionEnabled",
        g_browser_process->local_state()
        ->GetBoolean(prefs::kSmartScreenPuaProtectionEnabled));
    }
  }
  ...
  if (!command_line->HasSwitch(switches::kBrowserTest) &&
    ...
    smartscreen_preference_ = std::make_unique<SecurityPreference>(
        local_state(), prefs::kSmartScreenEnabled, L"\\SmartScreenEnabled");

    const base::string16 pua_subregkey = L"\\SmartScreenPuaEnabled";
    if (security::features::edge::IsPuaRepFeatureEnabled()) {
      smartscreen_pua_preference_ = std::make_unique<SecurityPreference>(
          local_state(), prefs::kSmartScreenPuaProtectionEnabled,
          pua_subregkey);
    } else {
      // The Windows Security App UI for PUA is hidden unless it finds the
      // reg key for PUA present in the registry. For that reason, we should
      // delete the key in the registry if the feature is disabled.
      SecurityPreference::DeleteKey(pua_subregkey);
    }
  }
  ...
  SetDomainActionsList(
      edge::domain_actions_parsing_helpers::GetEmptyDomainActionsList());

#if BUILDFLAG(ENABLE_WDAG)
  if ((wdag::IsHostInstance() && wdag::host::IsEnabledAndSupported()) ||
      wdag::IsContainerInstance())
    wdag_manager_ = std::make_unique<wdag::Manager>();
  // In the WDAG container, Data Protection is turned off since WDAG overrides
  // Data Protection
  if (wdag::IsContainerInstance())
    dlp::TurnOffDlp();
#endif
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    // Currently, Dlp is not supported on WebView, so we turn it off.
    dlp::TurnOffDlp();
#endif

  // Get instance of Microsoft APIs wrapper singleton to
  // guarantee that microsoft_apis.* loads while the browser
  // process has access to it.
  std::string edge_services_key =
      MicrosoftApiKeyWrapper::GetInstance()->GetEdgeServicesApiKey();

#if BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
  CreatePasswordBreachDetectionService();
#endif  // BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
}
...
void BrowserProcessImpl::StartTearDown() {
  ...
  if (password_protection_client_.get())
    password_protection_client()->ShutDown();
#if BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
  if (password_breach_detection_service_.get())
    password_breach_detection_service()->ShutDown();
#endif  // BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
...
#if defined(OS_WIN) && BUILDFLAG(ENABLE_SMARTSCREEN)
  // Delete while local_state_ is still valid to avoid crash on shutdown.
  // If smartscreen_preference_ is non-null, local should be non-null also.
  DCHECK(!smartscreen_preference_ || local_state_);
  smartscreen_preference_.reset();
  DCHECK(!smartscreen_pua_preference_ || local_state_);
  smartscreen_pua_preference_.reset();
#endif
}
...
#if EXCLUDED_FROM_EDGE  // Remove unused functions to avoid build errors.
#if !defined(OS_ANDROID)
...
#if BUILDFLAG(ENABLE_WDAG)
wdag::Manager* BrowserProcessImpl::wdag_manager() {
  return wdag_manager_.get();
}
#endif
...
bool BrowserProcessImpl::IsRemoteDebuggingEnabled() const {
#if !defined(OS_ANDROID)
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (remote_debugging_server_) {
    return remote_debugging_server_->IsRemoteDebuggingEnabled();
  }
#endif
...
BrowserProcessImpl::GetDomainActionsList() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  // If we haven't already registered for domain action updates, let's do so
  // now. This will happen at the first navigation after the browser starts.
  if (!registered_for_domain_actions_updates_) {
    pref_change_registrar_.Add(
        variations::prefs::kExternalConfigDomainActions,
        base::BindRepeating(
            &edge::domain_actions_parsing_helpers::OnDomainActionsChanged));

    // Let's check our existing policies right away.
    edge::domain_actions_parsing_helpers::OnDomainActionsChanged();

    registered_for_domain_actions_updates_ = true;
  }

  return domain_actions_list_;
}

void BrowserProcessImpl::SetDomainActionsList(
    const scoped_refptr<content::edge::DomainActionsList>&
        domain_actions_list) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_NE(domain_actions_list, nullptr);
  domain_actions_list_ = domain_actions_list;
}
...
void BrowserProcessImpl::RegisterPrefs(PrefRegistrySimple* registry) {
  ...
  // Metrics reporting and url reporting policies would be replaced or disabled
  // in Windows 10+ Release Stable channel once the browser follows Machine's
  // Data Collection Level and hide consent toggles in edge://settings/privacy.
  registry->RegisterBooleanPref(metrics::prefs::kMetricsReportingEnabled,
                                GoogleUpdateSettings::GetCollectStatsConsent());
  registry->RegisterBooleanPref(metrics::prefs::kUrlReportingEnabled,
                                GoogleUpdateSettings::GetCollectUrlConsent());
  registry->RegisterBooleanPref(
      metrics::prefs::kKeystrokesAndOmniboxTelemetryReportingEnabled, true);
#if defined(OS_ANDROID)
  registry->RegisterBooleanPref(
      prefs::kCrashReportingEnabled, false);
#endif  // defined(OS_ANDROID)
  registry->RegisterBooleanPref(
      metrics::prefs::kTailoredExperiencesEnabled, false);
}
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
void BrowserProcessImpl::CreateSmartScreenServiceClient() {
  DCHECK(!smartscreen_service_client_);

  created_smartscreen_service_client_ = true;
  smartscreen_service_client_ =
      smartscreen::SmartScreenServiceClient::CreateSmartScreenServiceClient();
  smartscreen_service_client_->Initialize();
}

smartscreen::SmartScreenServiceClient*
BrowserProcessImpl::smartscreen_service_client(bool get_when_pref_off) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

  // If feature has been disabled, do not create smartscreen client.
  if (!security::features::edge::IsSmartScreenEnabled()) {
    return nullptr;
  }

  // Creating the SmartScreen client (and loading libsmartscreen.dll) must
  // be avoided if users have disabled the SmartScreen user preference.
  // We need to handle the following cases:
  // 1) Edge was launched with the feature off (easy, handled above).
  // 2) Feature is on, the preference is enabled:
  //    We create all objects; all code is active.
  // 3) Feature is on, the user turns the preference off after launch.
  //    a) Entry points into features such as DL rep, AppRep, etc should
  //       be disabled.
  //    b) On going operations, started before the pref was turned off, must
  //       continue to work. For example, if the security badge is showing
  //       a warning turning the pref off shouldn't make the warning disappear.
  //
  // To support the above we will return the service client even if the pref is
  // is off if get_when_pref_off is true AND the service client had previously
  // been created.
  const bool ss_pref_enabled =
      g_browser_process->local_state()->GetBoolean(prefs::kSmartScreenEnabled);
  if (!ss_pref_enabled &&
      (!get_when_pref_off || !smartscreen_service_client_)) {
    return nullptr;
  }

  if (!created_smartscreen_service_client_) {
    CreateSmartScreenServiceClient();
  }
  return smartscreen_service_client_.get();
}
#endif // BUILDFLAG(ENABLE_SMARTSCREEN)

void BrowserProcessImpl::CreatePasswordProtectionClient() {
  DCHECK(!password_protection_client_);

  created_password_protection_client_ = true;
  password_protection_client_ =
      safe_browsing::PasswordProtectionClient::CreatePasswordProtectionClient();
}

safe_browsing::PasswordProtectionClient*
BrowserProcessImpl::password_protection_client() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

  if (!created_password_protection_client_) {
    CreatePasswordProtectionClient();
  }
  return password_protection_client_.get();
}

#if BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
void BrowserProcessImpl::CreatePasswordBreachDetectionService() {
  DCHECK(!password_breach_detection_service_);
  if (!base::FeatureList::IsEnabled(
          password_manager::features::kPasswordBreachDetection)) {
    return;
  }

  password_breach_detection_service_ = password_manager::breach_detection::
      BreachDetectionService::CreateBreachDetectionService();
}
#endif  // BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)

password_manager::breach_detection::BreachDetectionService*
BrowserProcessImpl::password_breach_detection_service() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

#if BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
  return password_breach_detection_service_.get();
#else   // BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
  return nullptr;
#endif  // BUILDFLAG(ENABLE_PASSWORD_BREACH_DETECTION)
}
...
scoped_refptr<edge_trust::TrustManager> BrowserProcessImpl::trust_manager() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  if (!created_trust_manager_) {
    CreateTrustManager();
  }
  return trust_manager_;
}

// Helper to ensure any TrackingChecker objects get their lists updated.
void BrowserProcessImpl::UpdateTrackingChecker(
  ...
  // Update our in-proc list then send a mojo message to the network service, if
  // it's present.
  trust_manager_->UpdateTrackingChecker(install_dir);

  if (base::FeatureList::IsEnabled(network::features::kNetworkService)) {
    // Now signal to our network service that it should also update.
    network::mojom::NetworkService* network_service =
        content::GetNetworkService();
    network_service->UpdateTrackingProtectionLists(install_dir,
                                                   base::NullCallback());
  }
}

scoped_refptr<parked_domains::ParkedDomainsChecker>
  BrowserProcessImpl::parked_domains_checker() {
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!created_parked_domains_checker_) {
      CreateParkedDomainsChecker();
    }
    return parked_domains_checker_;
}

// Helper to ensure any ParkedDomainsChecker objects get their parklist updated.
void BrowserProcessImpl::UpdateParklist() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

  if (!parked_domains_checker()) {
    parked_domains::RecordParkedDomainsErrorHistogram(
        parked_domains::ParkedDomainsError::NULL_PARKED_DOMAINS_CHECKER);
    return;
  }

  parked_domains::RecordParkedDomainsErrorHistogram(
      parked_domains::ParkedDomainsError::NONE);
  parked_domains_checker_->UpdateParklist();
}
...
session_context::SessionContext* BrowserProcessImpl::session_context() {
  if (!session_context_) {
    session_context_ = std::make_unique<session_context::SessionContext>();
  }

  DCHECK(base::FeatureList::IsEnabled(features::edge::kEdgeSessionContext));
...
void BrowserProcessImpl::PreMainMessageLoopRun() {
  ...
  UMA_HISTOGRAM_ENUMERATION("Microsoft.Startup.PreMessage", 1, 2);
...
// Task 19137987: [Plugins] 3rd Party Exhaust Detected in
// chrome/browser/plugins/plugins_resource_service.cc:26
#if EXCLUDED_FROM_EDGE
...
void BrowserProcessImpl::SetTrackingCheckerForTesting(
    enhanced_tracking_protection::TrackingChecker* tracking_checker) {
  if (trust_manager_) {
    trust_manager_->SetTrackingCheckerForTesting(tracking_checker);
  }
}

void BrowserProcessImpl::SetTrustManagerForTesting(
    edge_trust::TrustManager* trust_manager) {
  trust_manager_ = trust_manager;
}

void BrowserProcessImpl::CreateTrustManager() {
  DCHECK(!created_trust_manager_);
  created_trust_manager_ = true;

  base::FilePath user_data_dir;
  base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir);
  trust_manager_ = edge_trust::TrustManager::Create(
      shared_url_loader_factory(), user_data_dir,
      g_browser_process->local_state());
}

void BrowserProcessImpl::SetParkedDomainsCheckerForTesting(
    parked_domains::ParkedDomainsChecker* parked_domains_checker) {
  parked_domains_checker_ = parked_domains_checker;
}

void BrowserProcessImpl::CreateParkedDomainsChecker() {
  DCHECK(!created_parked_domains_checker_);
  created_parked_domains_checker_ = true;

  if (!features::edge::IsParkedDomainDetectionEnabled()) {
    return;
  }

  base::FilePath user_data_dir;
  base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir);
  parked_domains_checker_ = parked_domains::ParkedDomainsChecker::Create(
