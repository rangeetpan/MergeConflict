...
#include "components/telemetry_client/telemetry_protos/edge_report_extension.pb.h"
#include "components/telemetry_client/test/proto_utils.h"
#include "components/ukm/edge_ukm_control_sampling_state_provider.h"
#include "components/ukm/edge_ukm_features.h"
 ...
 public:
  explicit ScopedUkmFeatureParams(const base::FieldTrialParams& params) {
    scoped_feature_list_.InitWithFeaturesAndParameters(
        {{kUkmFeature, params}}, {features::edge::kUkmAllowListEnabled});
  }
...
class UkmServiceTest : public testing::Test {
 public:
  UkmServiceTest()
    ...
    ClearPrefs();
    variations::VariationsSeedStore::RegisterPrefs(prefs_.registry());
  }
  ...
  Report GetPersistedReport() {
    ...
    telemetry_client::EdgeReportExtension report_extension;
    EXPECT_TRUE(report_extension.ParseFromString(uncompressed_log_data));
    std::string result;
    base::Base64Decode(report_extension.payload_report(), &result);

    Report report;
    EXPECT_TRUE(report.ParseFromString(result));
    return report;
  }
  ...
  void TestReportExtensionWithoutEntries(const Report& report, const telemetry_client::EdgeReportExtension& report_extension) {
    EXPECT_EQ(report.client_id(), report_extension.client_id());
    EXPECT_EQ(report.session_id(), report_extension.session_id());
    EXPECT_EQ(report.report_id(), report_extension.report_id());
    EXPECT_EQ(report.system_profile().app_version(),
              report_extension.app_version());

    // Test payload report
    Report payload_report;
    std::string decode_output;
    EXPECT_TRUE(report_extension.has_payload_report());
    EXPECT_TRUE(
        base::Base64Decode(report_extension.payload_report(), &decode_output));
    EXPECT_TRUE(payload_report.ParseFromString(decode_output));

    EXPECT_TRUE(payload_report.has_system_profile());
    EXPECT_GT(payload_report.entries().size(), static_cast<int>(0));
    EXPECT_GT(payload_report.sources().size(), static_cast<int>(0));
    EXPECT_EQ(payload_report.aggregates().size(), static_cast<int>(0));

    // Test aggregates report
    Report aggregates_report;
    EXPECT_TRUE(report_extension.has_payload_aggregates());
    EXPECT_TRUE(base::Base64Decode(report_extension.payload_aggregates(),
                                  &decode_output));
    EXPECT_TRUE(aggregates_report.ParseFromString(decode_output));

    EXPECT_TRUE(aggregates_report.has_system_profile());
    EXPECT_EQ(aggregates_report.entries().size(), static_cast<int>(0));
    EXPECT_EQ(aggregates_report.sources().size(), static_cast<int>(0));
    EXPECT_GT(aggregates_report.aggregates().size(), static_cast<int>(0));

    // Test system profile report
    Report system_profile_report;
    EXPECT_TRUE(report_extension.has_payload_system_profile());
    EXPECT_TRUE(base::Base64Decode(report_extension.payload_system_profile(),
                                  &decode_output));
    EXPECT_TRUE(system_profile_report.ParseFromString(decode_output));

    EXPECT_TRUE(system_profile_report.has_system_profile());
    EXPECT_EQ(system_profile_report.entries().size(), static_cast<int>(0));
    EXPECT_EQ(system_profile_report.sources().size(), static_cast<int>(0));
    EXPECT_EQ(system_profile_report.aggregates().size(), static_cast<int>(0));
  }
...
TEST_F(UkmServiceTest, PersistAndPurge) {
  ...
  service.Flush();
  // TODO (joshber): Bug 22117678: Edge reporting service implementation does not
  // have the test infrastructure to test interstitial states of the uploader and
  // metrics/ukm service
...
TEST_F(UkmServiceTest, LogsSizeTriggeredRotation) {
  UkmService service(&prefs_, &client_,
                     true /* restrict_to_whitelisted_entries */,
                     std::make_unique<MockDemographicMetricsProvider>());

  base::HistogramTester tester;
  const char* kHistogram = "Microsoft.UKM.BuildAndStoreLogTime";
  ...
  // ClearPendingTasks ensures that the LogRotation can only triggered on size
  // condition rather than timer condition.
  task_runner_->ClearPendingTasks();
  EXPECT_EQ(0, service.report_count());

  // Record enough navigations to trigger LogRotation.
  for (int i = 0; i <= 500; i++) {
    UkmSource::NavigationData navigation_data;
    std::string count = "/number" + std::to_string(i);
    navigation_data.urls = {GURL("https://microsoft.com/initial" + count),
                            GURL("https://microsoft.com/final" + count)};

    ukm::SourceId id = GetWhitelistedSourceId(i);
    recorder.RecordNavigation(id, navigation_data);
  }
  EXPECT_EQ(1, service.report_count());
...
TEST_F(UkmServiceTest, EDGE_DISABLED_PENDING(PurgeMidUpload, 22117678)) {
  UkmService service(&prefs_, &client_,
                     true /* restrict_to_whitelisted_entries */,
                     std::make_unique<MockDemographicMetricsProvider>());
  TestRecordingHelper recorder(&service);
...
TEST_F(UkmServiceTest, AllowListEntryTest) {
  ClearPrefs();
  UkmService service(&prefs_, &client_,
                     true /* restrict_to_whitelisted_entries */,
					 std::make_unique<MockDemographicMetricsProvider>());
  // Set up ukm allow list
  base::test::ScopedFeatureList scoped_feature;
  scoped_feature.InitAndEnableFeature(features::edge::kUkmAllowListEnabled);
  const auto entries =
      base::SplitString(Entry1And2Whitelist(), ",", base::TRIM_WHITESPACE,
                        base::SPLIT_WANT_NONEMPTY);
  std::vector<telemetry_client::MetricGovernanceData<uint64_t>> ukm_allow_list;
  for (const auto& entry_string : entries) {
    ukm_allow_list.push_back(
        {base::HashMetricName(entry_string),
         telemetry_client::ControlState::ENABLED_BY_DEFAULT,
         base::Group::GROUP_4});
  }
  service.control_sampling_state_provider_ =
      ukm::UKMControlSamplingStateProvider(&ukm_allow_list[0],
                                           ukm_allow_list.size());
...
TEST_F(UkmServiceTest, UnreferencedNonWhitelistedSources) {
  ...
  for (bool restrict_to_whitelisted_source_ids : {true, false}) {
    ClearPrefs();
    base::test::ScopedFeatureList scoped_feature;
    scoped_feature.InitWithFeaturesAndParameters(
        {{kUkmFeature,
          {{"MaxKeptSources", "3"},
           {"RestrictToWhitelistedSourceIds",
            restrict_to_whitelisted_source_ids ? "true" : "false"}}},
         {features::edge::kUkmAllowListEnabled, {/* no parameters */}}},
        {} /* disabled features */);
    UkmService service(&prefs_, &client_,
                       true /* restrict_to_whitelisted_entries */,
                       std::make_unique<MockDemographicMetricsProvider>());
    ...
    const auto entries =
        base::SplitString(Entry1And2Whitelist(), ",", base::TRIM_WHITESPACE,
                          base::SPLIT_WANT_NONEMPTY);
    std::vector<telemetry_client::MetricGovernanceData<uint64_t>>
        ukm_allow_list;
    for (const auto& entry_string : entries) {
      ukm_allow_list.push_back(
          {base::HashMetricName(entry_string),
...
TEST_F(UkmServiceTest, SanitizeChromeUrlParams) {
  struct {
    ...
    const char* expected_url;
      ...
      {"edge://version/?foo=bar", "edge://version/"},
      {"about:blank?foo=bar", "about:blank"},
      {"edge://histograms/Variations", "edge://histograms/Variations"},
...
TEST_F(UkmServiceTest, TestStoreReportInReportExtension) {
  UkmService service(&prefs_, &client_,
                     true /* restrict_to_whitelisted_entries */,
                     std::make_unique<MockDemographicMetricsProvider>());

  Report report = telemetry_client::GetSmallUkmLog();
  telemetry_client::EdgeReportExtension report_extension;
  service.StoreReportInReportExtension(&report, &report_extension);

  TestReportExtensionWithoutEntries(report, report_extension);

  EXPECT_FALSE(report_extension.has_payload_entries());
}

TEST_F(UkmServiceTest, TestStoreReportInReportExtensionWithEntries) {
  UkmService service(&prefs_, &client_,
                     true /* restrict_to_whitelisted_entries */,
                     std::make_unique<MockDemographicMetricsProvider>());

  base::test::ScopedFeatureList scoped_feature_list;
  scoped_feature_list.InitAndEnableFeature(features::edge::kEdgeUkmSendEntries);
  ...
  Report entries_report;
  EXPECT_TRUE(report_extension.has_payload_entries());
  std::string decode_output;
  EXPECT_TRUE(base::Base64Decode(report_extension.payload_entries(),
                                &decode_output));
  EXPECT_TRUE(entries_report.ParseFromString(decode_output));

  EXPECT_TRUE(entries_report.has_system_profile());
  EXPECT_GT(entries_report.entries().size(), static_cast<int>(0));
  EXPECT_EQ(entries_report.sources().size(), static_cast<int>(0));
  EXPECT_EQ(entries_report.aggregates().size(), static_cast<int>(0));
}
