...
#include "chrome/browser/edge_share/edge_enhanced_url_copy_paste_clipboard_helper.h"
...
#include "chrome/browser/renderer_context_menu/voice_typing_panel_menu_observer.h"
...
>>>>>>> 0f134b7aee23f9b842904541586c099e84d0c52f
...
#include "components/dom_distiller/core/edge_url_utils.h"
#endif
...
const std::map<int, int>& GetIdcToUmaMap(UmaEnumIdLookupType type) {
  ...
  static base::NoDestructor<std::map<int, int>> general_map(
      {// NB: UMA values for 0 and 1 are detected using
       ...
       {IDC_CONTENT_CONTEXT_PASTE_AS_PLAIN_TEXT, kMicrosoftOffset + 5},
       {IDC_CONTENT_CONTEXT_PASTE_AS_LINK, kMicrosoftOffset + 6},
       {IDC_CONTENT_CONTEXT_PASTE_AS_WEB_PREVIEW, kMicrosoftOffset + 7},
       {IDC_CONTENT_CONTEXT_VOICE_TYPING, kMicrosoftOffset + 8},
       {IDC_CONTENT_CONTEXT_PASTE_AS_SETTINGS, kMicrosoftOffset + 9},
       {kMicrosoftSentinel, kMicrosoftOffset + 10}});
...
ui::ImageModel HighlighterMenuModel::GetIcon(const gfx::VectorIcon& icon,
                                             SkColor color) {
  return ui::ImageModel::FromVectorIcon(
      icon, color, views::MenuConfig::instance().icon_size);
}

ui::ImageModel HighlighterMenuModel::GetIconWithBorder(
    const gfx::VectorIcon& fill_icon,
    const gfx::VectorIcon& border_icon,
    SkColor color) {
  gfx::ImageSkia icon = gfx::CreateVectorIcon(
      fill_icon, views::MenuConfig::instance().icon_size, color);
  gfx::ImageSkia border = gfx::CreateVectorIcon(
      border_icon, views::MenuConfig::instance().icon_size, GetDefaultColor());
  return ui::ImageModel::FromImageSkia(
      gfx::ImageSkiaOperations::CreateSuperimposedImage(icon, border));
}
...
void HighlighterMenuModel::BuildAdd() {
  const int num_color_sub_menu_items = 5;
  ...
  static constexpr HighlightSubMenuItemInfo color_sub_menu_items[] = {
      ...
      {IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_ADD_RED,
       IDS_CONTENT_CONTEXT_PDF_HIGHLIGHT_RED,
       gfx::edge::kHighlighterRed},
  };
...
void HighlighterMenuModel::BuildEdit(SkColor current_highlight_icon_color) {
  ...
  static constexpr HighlightSubMenuItemInfo color_sub_menu_items[] = {
      ...
      {IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_PINK,
       IDS_CONTENT_CONTEXT_PDF_HIGHLIGHT_PINK,
       gfx::edge::kHighlighterPink},
      {IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_RED,
       IDS_CONTENT_CONTEXT_PDF_HIGHLIGHT_RED,
       gfx::edge::kHighlighterRed},
  };
...
void RenderViewContextMenu::AddItemWithStringIdAndIcon(
  ...
  AddItemWithStringIdAndIcon(id, localization_id, icon, menu_model_);
}
...
void RenderViewContextMenu::AddItemWithStringIdAndIcon(
  ...
  menu_model.AddItemWithStringIdAndIcon(id, localization_id,
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  if (features::IsLinkPreviewInPageEnabled()) {
    // Add advanced html on the clipboard
    advanced_clipboard_.reset(new ui::AdvancedClipBoard(url, false));
    advanced_clipboard_->DoAdvancedProcessing(
        base::UTF16ToUTF8(params_.link_text),
...
void RenderViewContextMenu::InitMenu() {
  ...
  if (content_type_->SupportsGroup(
      ...
      !should_hide_for_collections && !is_embedded_browser) {
...
#if defined(MICROSOFT_EDGE_BUILD) && !defined(OS_LINUX)
...
void RenderViewContextMenu::AppendPluginItems() {
  if (params_.page_url == params_.src_url ||
    ...
    if (IsPdfInPlaceSaveEnabled()) {
      AddItemWithStringIdAndIcon(IDC_SAVE_PAGE, IDS_CONTENT_CONTEXT_SAVEPAGE,
                                 kSaveIcon);
    } else {
  ...
  if (features::edge::IsEdgeTranslateEnabled() &&
    ...
    if (TranslateService::IsTranslatableURL(params_.page_url)) {
      ...
      if (prefs->IsTranslateAllowedByPolicy() &&
        ...
        base::string16 undefined_language = base::UTF8ToUTF16("und");
        if (undefined_language.compare(language) == 0) {
...
void RenderViewContextMenu::AppendPdfItems() {
  ...
  if (base::FeatureList::IsEnabled(features::edge::kEdgePDFCMHighlightUX)) {
    ...
    SkColor default_highlight_icon_color =
        pdf_web_contents_helper->GetDefaultHighlightColor();
    ...
    if (pdf_web_contents_helper->IsMenuStateAvailable(
      ...
      if (pdf_web_contents_helper->GetColorInfo(
        ...
        SkColor current_highlight_icon_color = current_highlight_color;
...
void RenderViewContextMenu::AppendSearchProvider() {
  ...
  AutocompleteClassifierFactory::GetForProfile(GetProfile())
      ->Classify(params_.selection_text, false, false, true, /* is from context menu */
...
void RenderViewContextMenu::AppendEditableItems() {
  ...
  if (!params_.misspelled_word.empty() && !is_embedded_browser) {
    AppendSearchProvider();
    menu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
  }
...
// word. In this case both items are always shown.
  ...
  if (!IsDevToolsURL(params_.page_url) &&
      ...
      (!menu_model_.GetItemCount() ||
       (menu_model_.GetIndexOfCommandId(IDC_CONTENT_CONTEXT_EMOJI) != -1 ||
        menu_model_.GetIndexOfCommandId(IDC_CONTENT_CONTEXT_VOICE_TYPING) !=
            -1)) && !should_hide_for_collections) {
  ...
  bool enhanced_copy_paste_enabled =
      edge_share::features::edge::IsEdgeShareEnhancedCopyPasteEnabled();
  if (enhanced_copy_paste_enabled)
    AppendPasteAsItems();

  if (params_.misspelled_word.empty()) {
    // Enhanced Copy Paste has its own menu item for pasting and matching style.
    if (!should_hide_for_collections && !enhanced_copy_paste_enabled) {
      menu_model_.AddItemWithStringId(
...
void RenderViewContextMenu::AppendVoiceTypingPanelItem() {
  if (!voice_typing_panel_menu_observer_) {
    voice_typing_panel_menu_observer_ =
        std::make_unique<VoiceTypingPanelMenuObserver>(this);
  }
  observers_.AddObserver(voice_typing_panel_menu_observer_.get());
  voice_typing_panel_menu_observer_->InitMenu(params_);
  menu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
}
...
void RenderViewContextMenu::AppendPasteAsItems() {
  AddItemWithStringIdAndIcon(IDC_CONTENT_CONTEXT_PASTE_AS_PLAIN_TEXT,
                             IDS_CONTENT_CONTEXT_PASTE_AS_PLAIN_TEXT,
                             kPasteAsPlainTextIcon, paste_as_submenu_model_);
  AddItemWithStringIdAndIcon(IDC_CONTENT_CONTEXT_PASTE_AS_LINK,
                             IDS_CONTENT_CONTEXT_PASTE_AS_LINK,
                             kPasteAsLinkIcon, paste_as_submenu_model_);
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  if (::features::IsLinkPreviewOmniboxEnabled()) {
     AddItemWithStringIdAndIcon(IDC_CONTENT_CONTEXT_PASTE_AS_WEB_PREVIEW,
                                IDS_CONTENT_CONTEXT_PASTE_AS_WEB_PREVIEW,
                                kPasteAsWebPreviewIcon, paste_as_submenu_model_);
  }
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  paste_as_submenu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
  AddItemWithStringIdAndIcon(IDC_CONTENT_CONTEXT_PASTE_AS_SETTINGS,
                             IDS_CONTENT_CONTEXT_PASTE_AS_SETTINGS,
                             kSettingsGearIcon, paste_as_submenu_model_);

  menu_model_.AddSubMenuWithStringId(IDC_CONTENT_CONTEXT_PASTE_AS,
                                     IDS_CONTENT_CONTEXT_PASTE_AS,
                                     &paste_as_submenu_model_);
}

void RenderViewContextMenu::AppendAreaSelectionItems() {
  // Do not show web select option in devtool.
  if (IsDevToolsURL(params_.page_url))
    return;
...
bool RenderViewContextMenu::IsCommandIdEnabled(int id) const {
  ...
  switch (id) {
    ...
    case IDC_CONTENT_CONTEXT_PASTE_AS:
    case IDC_CONTENT_CONTEXT_PASTE_AS_SETTINGS:
      return IsPasteAsEnabled();

    case IDC_CONTENT_CONTEXT_PASTE_AS_PLAIN_TEXT:
      return IsPasteAsPlainTextEnabled();

    case IDC_CONTENT_CONTEXT_PASTE_AS_LINK:
      return IsPasteAsLinkEnabled();

    case IDC_CONTENT_CONTEXT_PASTE_AS_WEB_PREVIEW:
      return IsPasteAsLinkPreviewEnabled() && !IsCommandIdUnderManagement(id);
 ...
 switch (id) {
    ...
    case IDC_CONTENT_CONTEXT_PASTE_AS_WEB_PREVIEW:
      return prefs->IsManagedPreference(
          prefs::kBackgroundTemplateListUpdatesEnabled);
    default:
      return false;
  }
}
...
bool RenderViewContextMenu::IsCommandIdUnderFamilySafetyManagement(
  ...
  switch (id) {
    case IDC_CONTENT_CONTEXT_OPENLINKOFFTHERECORD: {
      ...
      return service && service->IsActivityReportingEnabled();
    }
...
void RenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  ...
  switch (id) {
    ...
    case IDC_CONTENT_CONTEXT_PASTE_AS_LINK:
      edge_share::EnhancedURLCopyPasteClipboardHelper::PasteTitledHyperlink(
          source_web_contents_);
      break;

    case IDC_CONTENT_CONTEXT_PASTE_AS_WEB_PREVIEW:
      edge_share::EnhancedURLCopyPasteClipboardHelper::PasteLinkPreview(
          source_web_contents_);
      break;
    ...
    case IDC_CONTENT_CONTEXT_PASTE_AS_SETTINGS: {
      Browser* browser = chrome::FindBrowserWithWebContents(source_web_contents_);
      if(browser) {
        chrome::ShowEdgeShareCopyPaste(browser);
      }
      break;
    }
...
bool RenderViewContextMenu::IsSavePageEnabled() const {
  ...
  // In place save functionality only applies to local PDF files since then only
  // the changes can be saved back to the file. Hence, save option is enabled
  // and disabled according to edits on the PDF.
  if (IsPdfInPlaceSaveEnabled() &&
      (embedder_web_contents_->GetVisibleURL().SchemeIsFile())) {
    pdf::PDFWebContentsHelper* pdf_web_contents_helper =
        pdf::PDFWebContentsHelper::FromWebContents(source_web_contents_);
    if (!pdf_web_contents_helper ||
        !pdf_web_contents_helper->IsCommandEnabled(IDC_SAVE_PAGE)) {
      return false;
    }
  }
...
bool RenderViewContextMenu::IsPdfInPlaceSaveEnabled() const {
  return (
      embedder_web_contents_->GetContentsMimeType() == "application/pdf" &&
      base::FeatureList::IsEnabled(pdf::features::edge::kEdgePDFInPlaceSave));
}
...
bool RenderViewContextMenu::IsPasteAsEnabled() const {
  if (!edge_share::features::edge::IsEdgeShareEnhancedCopyPasteEnabled())
    return false;
...
bool RenderViewContextMenu::IsPasteAsLinkEnabled() const {
  if (!IsPasteAsEnabled())
    return false;

  if (!(params_.edit_flags & ContextMenuDataEditFlags::kCanEditRichly))
    return false;

  return edge_share::EnhancedURLCopyPasteClipboardHelper::
      CanPasteTitledHyperlink();
}

bool RenderViewContextMenu::IsPasteAsPlainTextEnabled() const {
  if (!IsPasteAsEnabled())
    return false;
...
bool RenderViewContextMenu::IsPasteAsLinkPreviewEnabled() const {
  if (!IsPasteAsEnabled())
    return false;
  ...
  return edge_share::EnhancedURLCopyPasteClipboardHelper::CanPasteLinkPreview();
}
...
void RenderViewContextMenu::ExecPdfItem(int command_id) {
  ...
  switch (command_id) {
    ...
    case IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_ADD_PINK:
      ...
      break;
    case IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_ADD_RED:
      pdf_web_contents_helper->CreateHighlight(gfx::edge::kHighlighterRed);
      break;
    ...
    case IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_PINK:
      pdf_web_contents_helper->ChangeHighlightColor(
          gfx::edge::kHighlighterPink);
      break;
    case IDC_CONTENT_CONTEXT_PDF_HIGHLIGHT_RED:
      pdf_web_contents_helper->ChangeHighlightColor(
          gfx::edge::kHighlighterRed);
      break;
