...
ContentSettingBubbleModel::ListItem CreateUrlListItem(int32_t id,
  ...
  return ContentSettingBubbleModel::ListItem(
      nullptr, title, base::string16(), true /* has_link */,
      false /* has_blocked_badge */, id, true /* use_bullet_point */);
}
...
#if defined(OS_WIN)
  // The bubble doesn't support running mixed script for IE tab.
  if(web_contents->IsHostingInternetExplorer()) {
    set_custom_link(base::string16());
    set_custom_link_enabled(false);
  }
#endif
  // TODO(Task 20646669): Add back Learn more button with updated forward link
...
void ContentSettingSubresourceFilterBubbleModel::OnManageCheckboxChecked(
  ...
  set_done_button_text(l10n_util::GetStringUTF16(
      is_checked ? IDS_BLOCKED_ADS_PROMPT_REFRESH : IDS_OK));
...
void ContentSettingNotificationsBubbleModel::CommitChanges() {
  base::RecordAction(
      base::UserMetricsAction("Notifications.Quiet.PromptDismissed"));
}
