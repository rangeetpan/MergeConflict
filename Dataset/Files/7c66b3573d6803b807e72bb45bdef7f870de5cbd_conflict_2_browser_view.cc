...
#include "chrome/browser/edge_kiosk/buildflags.h"
#if BUILDFLAG(ENABLE_EDGE_KIOSK)
#include "chrome/browser/edge_kiosk/kiosk_manager.h"
#endif  // BUILDFLAG(ENABLE_EDGE_KIOSK)
...
#include "chrome/browser/ui/views/accessibility/hc_with_theme_bubble_view.h"
=======
>>>>>>> 988e833e27e1c05099e8617359c01a0cfd3ba673
#include "chrome/browser/ui/views/accessibility/invert_bubble_view.h"
#include "chrome/browser/ui/views/autofill/autofill_bubble_handler_impl.h"
#include "chrome/browser/ui/views/bookmarks/bookmark_bar_view.h"
#include "chrome/browser/ui/views/bookmarks/bookmark_bubble_view.h"
#include "chrome/browser/ui/views/caret_browsing_dialog_delegate.h"
...
#include "chrome/browser/ui/views/frame/sidebar_contents_layout_manager.h"
#include "chrome/browser/ui/views/frame/sidebar_contents_resizing_strategy.h"
#include "chrome/browser/ui/views/frame/sidebar_contents_split_view.h"
#include "chrome/browser/ui/views/frame/sidebar_floating_view.h"
...
#include "chrome/browser/ui/views/tabs/edge_tab_strip.h"
#include "chrome/browser/ui/views/tabs/edge_tab_strip_controller.h"
...
// For edge, this is done to avoid any merge conflicts. (23197213)
...
#include "components/edge_collections/core/common/collections_prefs.h"
#endif

#include "chrome/browser/edge_reactive_search/reactive_search_window.h"
#include "chrome/browser/extensions/chrome_extension_web_contents_observer.h"
#include "chrome/browser/ui/views/edge_anchored_view_widget_host.h"
#include "chrome/browser/ui/views/frame/edge_vertical_tab_container_anchor_view.h"
#include "chrome/browser/ui/views/frame/edge_vertical_tab_container_view.h"
#include "chrome/grit/microsoft_edge_strings.h"
#include "components/edge_vertical_tabs/common/vertical_tabs_pref_names.h"

#if defined(MICROSOFT_EDGE_BUILD)
#include "chrome/browser/ui/edge_fre/edge_fre_helper.h"
#include "chrome/browser/ui/views/edge_read_aloud/read_aloud_bar_view.h"
#endif
...
#include "chrome/browser/ui/views/frame/dual_engine_hide_window_deferral_impl_win.h"
#include "chrome/browser/ui/views/frame/immersive_mode_controller_win.h"
#include "chrome/browser/ui/views/frame/windowed_find_bar_focus_proxy_view_win.h"
...
#include "chrome/browser/ui/views/edge_reading_view/reading_view_icon.h"
#endif

#if BUILDFLAG(ENABLE_WDAG)
#include "chrome/browser/ui/views/wdag_win/flyout_dialog.h"
#endif
...
#include "chrome/browser/ui/views/profiles/edge_guided_switch_background_view.h"
#include "chrome/browser/ui/views/profiles/edge_profile_chooser_view.h"
#include "components/signin/core/browser/edge_identity_features.h"

#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
#include "chrome/browser/edge_embedded_browser/embedded_browser_accelerators.h"
#include "chrome/browser/edge_embedded_browser/embedded_browser_impl.h"
#endif

const int COLLECTIONS_SIDEBAR_MINIMUM_WIDTH = 364;
const int SEARCH_SIDEBAR_MINIMUM_WIDTH = 440;
...
class BrowserViewLayoutDelegateImpl : public BrowserViewLayoutDelegate {
 public:
  ...
  bool IsVerticalTabStripVisible() const override {
    return browser_view_->IsVerticalTabStripVisible();
  }
...
const char BrowserView::kEdgeVerticalTabsAvailableHistogram[] =
    "Microsoft.Tabs.VerticalTabStripAvailable2";
const char BrowserView::kEdgeVerticalTabsFirstShownHistogram[] =
    "Microsoft.Tabs.VerticalTabStripFirstShown2";
const char BrowserView::kEdgeVerticalTabsShownHistogram[] =
    "Microsoft.Tabs.VerticalTabStripShown";
...
BrowserView::BrowserView(std::unique_ptr<Browser> browser)
  ...
  // Hook up to listen to the prefs changing here because we need to be able
  // to destroy the pref listeners on the UI thread
  pref_change_registrar_ = std::make_unique<PrefChangeRegistrar>();
  use_system_theme_.Init(
      prefs::kUsesSystemTheme, browser_->profile()->GetPrefs(),
      base::BindRepeating(&BrowserView::UserColorModePreferenceChanged,
                          base::Unretained(this)));
  use_dark_theme_.Init(
      prefs::kDarkTheme, browser_->profile()->GetPrefs(),
      base::BindRepeating(&BrowserView::UserColorModePreferenceChanged,
                          base::Unretained(this)));
  ...
  auto tabstrip_controller = std::make_unique<EdgeTabStripController>(
      browser_->tab_strip_model(), this);
  EdgeTabStripController* tabstrip_controller_ptr =
      tabstrip_controller.get();
  tabstrip_ = new EdgeTabStrip(std::move(tabstrip_controller));
...
#if defined(MICROSOFT_EDGE_BUILD)
  read_aloud_bar_view_ = top_container_->AddChildView(
      std::make_unique<edge::learning_tools::ReadAloudBarView>(browser_.get(),
                                                               this));
  read_aloud_bar_view_->AddObserver(this);
#endif

  if (IsVerticalTabStripSupported()) {
    // Configure the TabStrip.
    tabstrip_->SetDelineateFirstVisibleTab(!base::FeatureList::IsEnabled(
        features::edge::kEdgeVerticalTabsShortUI));

    // Create vertical tab container view. Parent of the TabStrip and other UI.
    edge_vertical_tab_container_ =
        std::make_unique<EdgeVerticalTabContainerView>(browser_.get());
    edge_vertical_tab_container_->set_owned_by_client();
    edge_vertical_tab_container_->AddContainerObserver(this);

    // Create vertical tab container anchor view. Will be tracked by the Widget
    // hosting the container; BrowserLayout positions it and the Widget follows.
    edge_vertical_tab_container_anchor_ =
        AddChildView(std::make_unique<EdgeVerticalTabContainerAnchorView>());
    edge_vertical_tab_container_anchor_->AttachContainer(
        edge_vertical_tab_container_.get());

    // Set the initial TabStrip orientation correctly based on persisted state.
    SetTabStripLayoutOrientation(
        GetProfile()->GetPrefs()->GetBoolean(edge_vertical_tabs::prefs::kOpened)
            ? views::LayoutOrientation::kVertical
            : views::LayoutOrientation::kHorizontal);

    base::UmaHistogramBoolean(kEdgeVerticalTabsAvailableHistogram, true);
  }
  ...
  bool reactive_search_enabled = features::edge::IsReactiveSearchEnabled();
  searchable_contents_split_container_ = nullptr;

  if (reactive_search_enabled) {
    reactive_search_web_view_ = new views::WebView(browser_->profile());
    reactive_search_web_view_->SetBorder(
        views::CreateSolidSidedBorder(0, 1, 0, 0, SK_ColorLTGRAY));
    reactive_search_web_view_->SetVisible(false);

    searchable_contents_split_container_ =
        new SidebarContentsSplitView(contents_container_,
                                     reactive_search_web_view_,
                                     SEARCH_SIDEBAR_MINIMUM_WIDTH);
  }

  if (IsCollectionsEnabled() || features::edge::IsHubEnabled()) {
    pref_change_registrar_->Init(browser_->profile()->GetPrefs());
    pref_change_registrar_->Add(
        prefs::kCollectionsPinterestRecommendationsEnabled,
        base::BindRepeating(
          &BrowserView::OnPinterestRecommendationsPrefChanged,
          base::Unretained(this)));
    sidebar_web_view_ = new views::WebView(browser_->profile());
    sidebar_web_view_->SetVisible(false);
    sidebar_web_view_->set_allow_accelerators(true);

    sidebar_contents_split_container_ =
        new SidebarContentsSplitView(reactive_search_enabled
                                        ? searchable_contents_split_container_
                                        : contents_container_,
                                     sidebar_web_view_,
                                     COLLECTIONS_SIDEBAR_MINIMUM_WIDTH);

    AddChildView(sidebar_contents_split_container_);
    set_contents_view(sidebar_contents_split_container_);
  } else {
    views::View* contents_view = reactive_search_enabled
        ? searchable_contents_split_container_
        : contents_container_;
  ...
  visible_pane_ = HubContentID::Empty;
  webcontents_loaded_content_ = HubContentID::Empty;

#if defined(OS_WIN)
  if (base::FeatureList::IsEnabled(features::edge::kGuidedSwitchAllowed)) {
    // This constructor creates view and adds itself as child.
    edge_guided_switch_background_view_ =
        new EdgeGuidedSwitchBackgroundView(browser_.get(), contents_web_view_);
  }
#endif
...
#if defined(OS_WIN)
  if (!features::edge::UseAuraForFindBar(GetProfile())) {
    windowed_find_bar_focus_proxy_view_ =
        AddChildView(std::make_unique<WindowedFindBarFocusProxyView>(this));
  }
#endif
}
...
BrowserView::~BrowserView() {
  ...
  // Cleanly tear down the EdgeVerticalTabContainerView and related objects.
  if (edge_vertical_tab_container_) {
    if (browser_view_layout)
      browser_view_layout->set_edge_vertical_tab_container_anchor(nullptr);
    edge_vertical_tab_container_anchor_->DetachContainer();
    edge_vertical_tab_container_->RemoveContainerObserver(this);
    edge_vertical_tab_container_.reset();
  }

#if defined(MICROSOFT_EDGE_BUILD)
  read_aloud_bar_view_->RemoveObserver(this);
#endif

  // Bookmark bar is dependent on tool bar view and location bar view to do
  // layout. Hence destroying it before.
  if (base::FeatureList::IsEnabled(features::edge::kEdgeOnRampDiscardData))
...
#if defined(MICROSOFT_EDGE_BUILD)
  read_aloud_bar_view_ = nullptr;
#endif
}

// EdgeVerticalTabContainerObserver:
void BrowserView::OnVerticalTabContainerStateChanged(
    const EdgeVerticalTabContainerStateChangedEvent& event) {
  if (event.IsMinimizedTransition()) {
    browser_->profile()->GetPrefs()->SetBoolean(
        edge_vertical_tabs::prefs::kOpened, !event.IsMinimized());

    // Re-parent |tabstrip_| appropriately.
    if (event.IsMinimized()) {
      edge_vertical_tab_container_->DetachTabStrip(tabstrip_);
      tab_strip_region_view_->AddChildView(tabstrip_);
    } else {
      tab_strip_region_view_->RemoveChildView(tabstrip_);
      edge_vertical_tab_container_->AttachTabStrip(tabstrip_);
    }
  }

  if (event.IsCollapsedPinnedTransition()) {
    browser_->profile()->GetPrefs()->SetBoolean(
        edge_vertical_tabs::prefs::kCollapsed, !event.IsPinned());
  }

  if (event.IsMinimizedTransition() || event.IsPinnedTransition()) {
    Layout();
    if (frame_) {
      // Update the non-client frame view. Only do this after our own re-layout
      // because its position is based on the vertical tab container's position.
      frame_->non_client_view()->UpdateCurrentFrame();
    }
  }
}

bool BrowserView::IsRectInBrowserControl(const gfx::Rect& rect) {
// Check for ShyUI un-fullscreen button.
#if defined(OS_WIN)
  if (immersive_mode_controller_->IsEnabled()) {
    const auto* const caption_button = static_cast<ImmersiveModeControllerWin*>(
                                           immersive_mode_controller_.get())
                                           ->exit_caption_button();

    if (caption_button && !caption_button->GetLocalBounds().Intersects(rect))
      return true;
  }
#endif
  return false;
...
#if defined(MICROSOFT_EDGE_BUILD)
void BrowserView::SettingsPaneOpened() {
  is_read_aloud_voice_options_visible_ = true;
}

void BrowserView::SettingsPaneClosed() {
  is_read_aloud_voice_options_visible_ = false;
}

void BrowserView::ReadingStateChanged(
    edge::learning_tools::ReadingState reading_state) {
  read_aloud_state_ = reading_state;
  browser_->command_controller()->UpdateCommandsForReadAloud();
}

bool BrowserView::IsReadAloudSettingsPaneOpen() {
  return is_read_aloud_voice_options_visible_;
}

edge::learning_tools::ReadingState BrowserView::GetReadAloudState() {
  return read_aloud_state_;
}
#endif
...
BrowserView* BrowserView::GetBrowserViewForBrowser(const Browser* browser) {
  ...
  //
  // Make sure that the browser is still live; if we're closing, it's possible
  // the browser has been torn down before the process exits.
  if (!base::Contains((*BrowserList::GetInstance()), browser))
    return nullptr;
...
int BrowserView::GetVerticalTabStripWidth() const {
  // We don't want to return edge_vertical_tab_container_->width() or its
  // PreferredSize because it might be overlaying other views via its host
  // widget right now. Its MinimumSize is what determines how much space in
  // the BrowserView is actually being dedicated to it, so return that.
  return IsVerticalTabStripVisible() &&
                 !base::FeatureList::IsEnabled(
                     features::edge::kEdgeVerticalTabsShortUI)
             ? edge_vertical_tab_container_->GetMinimumSize().width()
             : 0;
}

bool BrowserView::IsTabStripVisible() const {
  // Check to see we're in Immersive Mode and unrevealed.
  if (immersive_mode_controller_->IsEnabled() &&
      !immersive_mode_controller_->IsRevealed())
    return false;
  ...
  // If in vertical layout, tab strip (region view) isn't visible.
  if (GetTabStripLayoutOrientation() == views::LayoutOrientation::kVertical)
    return false;
...
bool BrowserView::IsVerticalTabStripSupported() const {
  // Check if this browser supports any TabStrip at all.
  if (!browser_->SupportsWindowFeature(Browser::FEATURE_TABSTRIP))
    return false;

  // Return whether specific feature flag is enabled or not.
  return features::edge::IsVerticalTabsEnabled();
}

bool BrowserView::IsVerticalTabStripVisible() const {
  if (!IsVerticalTabStripSupported())
    return false;

  // If in horizontal layout, vertical tab strip (container) isn't visible.
  if (GetTabStripLayoutOrientation() == views::LayoutOrientation::kHorizontal)
    return false;

  return (tabstrip_ && edge_vertical_tab_container_);
}

views::LayoutOrientation BrowserView::GetTabStripLayoutOrientation() const {
  if (!edge_vertical_tab_container_)
    return views::LayoutOrientation::kHorizontal;
  return edge_vertical_tab_container_->IsMinimized()
             ? views::LayoutOrientation::kHorizontal
             : views::LayoutOrientation::kVertical;
}

void BrowserView::SetTabStripLayoutOrientation(
    views::LayoutOrientation orientation) {
  if (!edge_vertical_tab_container_)
    return;  // Can't change orientation if no |edge_vertical_tab_container_|.

  // Set container state appropriately if orientation has changed. We'll trigger
  // re-parent and re-layout if necessary in OnVerticalTabContainerStateChanged.
  if (orientation != GetTabStripLayoutOrientation()) {
    if (orientation == views::LayoutOrientation::kVertical) {
      const EdgeVerticalTabContainerView::State preferred_state =
          browser_->profile()->GetPrefs()->GetBoolean(
              edge_vertical_tabs::prefs::kCollapsed)
              ? EdgeVerticalTabContainerView::State::kCollapsed
              : EdgeVerticalTabContainerView::State::kPinned;
      edge_vertical_tab_container_->SetState(preferred_state);
    } else {
      edge_vertical_tab_container_->SetState(
          EdgeVerticalTabContainerView::State::kMinimized);
    }
  }

  // Logging.
  base::UmaHistogramBoolean(kEdgeVerticalTabsShownHistogram,
                            orientation == views::LayoutOrientation::kVertical);

  if (orientation == views::LayoutOrientation::kVertical) {
    if (!browser_->profile()->GetPrefs()->GetBoolean(
            edge_vertical_tabs::prefs::kFirstOpened)) {
      base::UmaHistogramBoolean(kEdgeVerticalTabsFirstShownHistogram, true);
      browser_->profile()->GetPrefs()->SetBoolean(
          edge_vertical_tabs::prefs::kFirstOpened, true);
    }
  }
}

bool BrowserView::IsTabStripCollapsed() const {
  return edge_vertical_tab_container_ &&
         edge_vertical_tab_container_->IsCollapsed();
}

bool BrowserView::IsTabStripPinned() const {
  return edge_vertical_tab_container_ &&
         edge_vertical_tab_container_->IsPinned();
}
...
#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
bool BrowserView::IsBrowserEmbedded() const {
  return browser_->is_embedded();
}

bool BrowserView::IsBrowserEmbeddedVisualHosted() const {
  return browser_->is_embedded_visual_hosted();
}
#endif
...
#if defined(MICROSOFT_EDGE_BUILD)
  MaybeShowHCWithThemeBubbleView(this);
#endif
...
#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
  const bool status_bar_disabled = embedded_browser::EmbeddedBrowserImpl::
      StatusBarDisabledForEmbeddedBrowser(GetActiveWebContents());
  if (status_bar_disabled && status_bubble_) {
    contents_web_view_->SetStatusBubble(nullptr);
    status_bubble_ = nullptr;
  } else if (!status_bar_disabled && !status_bubble_) {
    InitStatusBubble();
  }
#endif  // ENABLE_EMBEDDED_BROWSER_WEBVIEW
  return status_bubble_.get();
}

#if defined(MICROSOFT_EDGE_BUILD)
edge::fre::EdgeFreUIHandler* BrowserView::GetEdgeFreUIHandler() {
  return edge_fre_view_;
}
edge::learning_tools::ReadAloudUIHandler* BrowserView::GetReadAloudUIHandler() {
  return read_aloud_bar_view_;
}
edge::learning_tools::ReadAloudUINotifier*
BrowserView::GetReadAloudUINotifier() {
  return read_aloud_bar_view_;
}
#endif
...
void BrowserView::UpdateReactiveSearch() {
  UpdateReactiveSearchForContents(GetActiveWebContents(), true);
  Layout();
}

#if defined(MICROSOFT_EDGE_BUILD)
void BrowserView::HideFloatingSidebar() {
  // TODO: Find a better way to close the floating view
  if (sidebar_floating_view_) {
    sidebar_floating_view_->Close();
    sidebar_floating_view_ = nullptr;
    this->Focus();
  }
}

void BrowserView::HideDockedSidebar() {
  // Restore focus to the last focused view when hiding the sidebar.
  if (sidebar_focus_tracker_.get()) {
    sidebar_focus_tracker_->FocusLastFocusedExternalView();
    sidebar_focus_tracker_.reset();
  }

  if (sidebar_web_view_) {
    sidebar_web_view_->SetVisible(false);
    sidebar_web_view_->SetWebContents(nullptr);
  }

  // Clean up web contents to end the extension process.
  Browser* browser = browser_.get();
  CollectionsManager::CleanupWebContents(browser);

  if (sidebar_contents_split_container_) {
    sidebar_contents_split_container_->HideSidebar();
  }
}

void BrowserView::ShowCollections() {
  visible_pane_ = HubContentID::Collections;
  Browser* browser = browser_.get();
  if (CollectionsManager::IsDocked(browser)) {
    HideFloatingSidebar();

    if (!sidebar_focus_tracker_.get()) {
      // Install sidebar focus tracker when the sidebar is shown.
      sidebar_focus_tracker_.reset(new views::ExternalFocusTracker(
          sidebar_web_view_, GetFocusManager()));
    }

    if (sidebar_web_view_) {
      sidebar_web_view_->SetWebContents(
          CollectionsManager::GetWebContents(browser));
      sidebar_web_view_->SetVisible(true);
    }

    sidebar_contents_split_container_->ShowSidebar();
  } else {
    HideDockedSidebar();

    if (sidebar_floating_view_) {
      sidebar_floating_view_ =
          SidebarFloatingView::ShowSidebar(bookmark_bar_view_.get(), browser);
    }
  }

  toolbar_->UpdatePaneButtonStates();
}

void BrowserView::ShowHubPane(HubContentID content) {
  visible_pane_ = content;
  Browser* browser = browser_.get();
  HideFloatingSidebar();
  ...
  if (!sidebar_web_contents_ || content != webcontents_loaded_content_){
    sidebar_web_contents_ = WebContents::Create(WebContents::CreateParams(browser->profile()));
    GURL url = HubManager::GetURLForContent(content, /*isPinned=*/ true);
    sidebar_web_contents_->GetController().LoadURL(
        url, content::Referrer(), ui::PAGE_TRANSITION_AUTO_TOPLEVEL, std::string());
    extensions::ChromeExtensionWebContentsObserver::CreateForWebContents(
        sidebar_web_contents_.get());
    HubManager::SetDelegate(browser, sidebar_web_contents_.get());
    webcontents_loaded_content_ = content;
  }
  sidebar_web_view_->SetWebContents(sidebar_web_contents_.get());
  sidebar_web_view_->SetVisible(true);
...
void BrowserView::ShowHubPopup(HubContentID content) {
  toolbar_->ShowHubPopup(content);
}

void BrowserView::HideCollections() {
  visible_pane_ = HubContentID::Empty;
  HideFloatingSidebar();
  HideDockedSidebar();
  toolbar_->UpdatePaneButtonStates();
...
void BrowserView::HideHubPane(HubContentID content) {
  visible_pane_ = HubContentID::Empty;
  HideFloatingSidebar();
  HideDockedSidebar();
  toolbar_->UpdatePaneButtonStates();
...
HubContentID BrowserView::GetVisibleHubContent() {
  return visible_pane_;
}

bool BrowserView::IsHubPaneVisible(HubContentID content) const {
  return sidebar_floating_view_ ||
         (sidebar_web_view_ && sidebar_web_view_->GetVisible() && visible_pane_ == content);
}
#endif

void BrowserView::ShowReactiveSearch() {
  reactive_search_web_view_->SetWebContents(
      ReactiveSearchWindow::GetMainWebContents(
          GetActiveWebContents()));
  reactive_search_web_view_->SetVisible(true);
  searchable_contents_split_container_->ShowSidebar();
}

void BrowserView::HideReactiveSearch() {
  reactive_search_web_view_->SetVisible(false);
  searchable_contents_split_container_->HideSidebar();
}

void BrowserView::UpdateReactiveSearchForContents(WebContents* web_contents,
                                                  bool update_web_contents) {
  if (!features::edge::IsReactiveSearchEnabled()) {
    return;
  }

  WebContents* reactive_search_web_contents =
      ReactiveSearchWindow::GetMainWebContents(web_contents);

  // Replaces the reactive search WebContents.
  if (reactive_search_web_view_->web_contents() !=
        reactive_search_web_contents &&
      update_web_contents) {
    reactive_search_web_view_->SetWebContents(reactive_search_web_contents);
  }

  if (reactive_search_web_contents) {
    reactive_search_web_view_->SetVisible(true);
    searchable_contents_split_container_->ShowSidebar();
  } else {
    reactive_search_web_view_->SetVisible(false);
...
#if defined(MICROSOFT_EDGE_BUILD) && !defined(OS_ANDROID)
void BrowserView::UpdateReadingViewIcon() {
  UpdatePageActionIcon(PageActionIconType::kReadingView);
}

void BrowserView::CloseTranslateBubble() {
...
#if defined(OS_WIN)
void BrowserView::InternetExplorerHideWindowCallback(bool is_timeout,
                                                     int correlation) {
  if (ie_hide_window_deferral_ && ie_hide_window_deferral_->IsActive()) {
    if (hide_ie_timeout_correlation_ != correlation) {
      VLOG(1) << "InternetExplorerHideWindowCallback not correlated";
      return;
    }
    if (is_timeout) {
      VLOG(1) << "InternetExplorerHideWindowCallback timeout reached";
    }
    ie_hide_window_deferral_->Complete(GetActiveWebContents());
    ie_hide_window_deferral_ = nullptr;
  }
}
#endif

void BrowserView::UpdateToolbarBookmarksButtonVisibility() {
  toolbar_->UpdateToolbarBookmarksButtonVisibility();
}

void BrowserView::UpdateToolbarFeedbackButtonVisibility() {
  toolbar_->UpdateToolbarFeedbackButtonVisibility();
}

void BrowserView::UpdateToolbarCollectionsButtonVisibility() {
  toolbar_->UpdateToolbarCollectionsButtonVisibility();
}

void BrowserView::UpdateToolbarShareButtonVisibility() {
  toolbar_->UpdateToolbarShareButtonVisibility();
}

void BrowserView::UpdateToolbarHomeButtonVisibility() {
  toolbar_->UpdateHomeButtonVisibility();
}
...
void BrowserView::OnActiveTabChanged(content::WebContents* old_contents,
...
#else
  // Edge: The TabStrip might be owned by an active Widget other than our own.
  // In that case, we still want to restore focus to our BrowserView widget.
  // So, check both Widgets to see if either of them are currently active.
  bool will_restore_focus =
      !browser_->tab_strip_model()->closing_all() &&
      (GetWidget()->IsActive() || tabstrip_->GetWidget()->IsActive()) &&
      GetWidget()->IsVisible();
#endif
  ...
  if (change_tab_contents) {
...
#if defined(OS_WIN)
    // Detect a switch from an AndIE tab to an Anaheim tab.
    if (old_contents && change_tab_contents &&
        !(frame()->tab_drag_kind() != TabDragKind::kNone) &&
        old_contents->GetInternetExplorerNativeView()) {
      std::unique_ptr<content::DualEngineDeferredWindowHider> ie_hider =
          old_contents->CreateInternetExplorerDeferredWindowHider();

      if (ie_hider) {
        hide_ie_timeout_correlation_++;
        if (ie_hide_window_deferral_) {
          // If a window is pending being hidden, just hide it now.
          ie_hide_window_deferral_->Complete(new_contents);
        }
        ie_hide_window_deferral_ =
            std::make_unique<dual_engine::DualEngineHideWindowDeferralImpl>(
                old_contents, new_contents, std::move(ie_hider));

        // Set the maximum time for the deferral to hide the window.
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
            FROM_HERE,
            base::BindOnce(&BrowserView::InternetExplorerHideWindowCallback,
                           dual_engine_weak_ptr_factory_.GetWeakPtr(), true,
                           hide_ie_timeout_correlation_),
            base::TimeDelta::FromMilliseconds(500));
      }
    } else if (ie_hide_window_deferral_) {
      // If a window is pending being hidden, just hide it now.
      ie_hide_window_deferral_->Complete(new_contents);
      ie_hide_window_deferral_ = nullptr;
    }
    ...
    SetBrowserWebContents(old_contents, nullptr);
  ...
  UpdateReactiveSearchForContents(new_contents, true /* update_web_contents */);
  ...
  if (change_tab_contents) {
    ...
    SetBrowserWebContents(old_contents, new_contents);
  ...
  if (edge_guided_switch_background_view_) {
    edge_guided_switch_background_view_->HandleActiveTabChanges(new_contents,
                                                                old_contents);
  }

#if defined(MICROSOFT_EDGE_BUILD)
  if (edge_fre_view_ != nullptr) {
    edge_fre_view_->OnActiveTabChanges(new_contents,old_contents);
  }
  if (read_aloud_bar_view_)
    read_aloud_bar_view_->OnActiveTabChanged();
  ...
  if (features::edge::IsCollectionsFeatureEnabled(
    ...
    if (new_contents->IsHostingInternetExplorer()) {
      HideCollections();
    } else {
      toolbar_->UpdatePaneButtonStates();
    }
...
void BrowserView::OnTabDetached(content::WebContents* contents,
  ...
  if (was_active) {
    ...
    if (loading_bar_)
      loading_bar_->SetWebContents(nullptr);
    SetBrowserWebContents(contents, nullptr);
...
void BrowserView::UpdateExclusiveAccessExitBubbleContent(
  ...
  // bubble.
  // Edge exception- Windows Immersive mode has the bubble and should only have
  // to do a check for kiosk mode instead.
  ...
  if (bubble_type == EXCLUSIVE_ACCESS_BUBBLE_TYPE_NONE ||
#if BUILDFLAG(ENABLE_EDGE_KIOSK)
      (edge::kiosk::Manager::IsFullScreenMode(
...
bool BrowserView::IsImmersiveModeEnabledForFullscreen() const {
  return immersive_mode_controller_->IsEnabled();
}

void BrowserView::MaybeStartImmersiveReveal() {
...
void BrowserView::LocationBarPopupVisibilityChanged(bool visible) {
#if defined(OS_WIN)
  // If the find bar is a popup, temporarily hide it while the Omnibox is
  // showing to prevent overlap.
  if (!features::edge::UseAuraForFindBar(GetProfile()) &&
      browser()->HasFindBarController()) {
    browser()->GetFindBarController()->find_bar()->DisableRendering(visible);
  }
#endif
}
...
#if defined(OS_WIN) || defined(OS_CHROMEOS)
  if (!chrome::IsLogicallyActive(browser()))
    return;
#endif
...
void BrowserView::RotatePaneFocus(bool forwards) {
  // This method has been updated to allow F6 focus for notifications.
  // BrowserView passes a list of notifications, |notifications_to_focus_| to
  // the focus manager, which will be focused on after the BrowserView views.

  GetFocusManager()->RotatePaneFocus(
      forwards ? views::FocusManager::kForward : views::FocusManager::kBackward,
      views::FocusManager::FocusCycleWrapping::kEnabled, &notifications_to_focus_);
}
...
void BrowserView::ConfirmBrowserCloseWithPendingDownloads(
    int download_count,
    bool ie_has_downloads,
    Browser::DownloadCloseType dialog_type,
    bool app_modal,
    const base::Callback<void(bool, bool, bool)>& callback) {
  ...
  DownloadInProgressDialogView::Show(GetNativeWindow(), download_count,
                                     ie_has_downloads, dialog_type, app_modal,
                                     callback);
}
...
void BrowserView::ShowCaretBrowsingDialog() {
  CaretBrowsingDialogDelegate::Show(this);
}

void BrowserView::ToggleCaretBrowsingMode(PrefService* prefService) {
  prefService->SetBoolean(
      prefs::kCaretBrowsingEnabled,
      !prefService->GetBoolean(prefs::kCaretBrowsingEnabled));

    ukm::builders::Microsoft_CaretBrowsingToggle(
        ukm::GetSourceIdForWebContentsDocument(GetActiveWebContents()))
        .SetEnabled(prefService->GetBoolean(prefs::kCaretBrowsingEnabled))
        .SetSkippedDialog(!prefService->GetBoolean(prefs::kShowCaretBrowsingDialog))
        .Record(ukm::UkmRecorder::Get());

    // Since UKM data is not yet being transmitted to server, also record a
    // usage count.
    if (prefService->GetBoolean(prefs::kCaretBrowsingEnabled)) {
      base::RecordAction(
          base::UserMetricsAction("Microsoft.CaretBrowsing_Enabled"));
    } else {
      base::RecordAction(
          base::UserMetricsAction("Microsoft.CaretBrowsing_Disabled"));
    }
}
...
#if defined(OS_WIN)
    contents.contents->UpdateInternetExplorerHostedWindow();
#endif
...
void BrowserView::OnBeforeHideFindBar() {
#if defined(OS_WIN)
  if (windowed_find_bar_focus_proxy_view_) {
    bool proxy_has_focus = GetFocusManager()->GetStoredFocusView() ==
        windowed_find_bar_focus_proxy_view_;
    VLOG(1) << "OnBeforeHideFindBar - proxy had focus: " << proxy_has_focus;
    if (proxy_has_focus) {
      GetFocusManager()->SetStoredFocusView(nullptr);
      views::View* view_to_focus =
          windowed_find_bar_focus_proxy_view_->GetPreviousFocusedView();
      if (view_to_focus) {
...
void BrowserView::FindBarFocusChanged() {
#if defined(OS_WIN)
  if (!windowed_find_bar_focus_proxy_view_ ||
      !browser()->HasFindBarController()) {
    return;
  }
  if (browser()->GetFindBarController()->find_bar()->IsFindBarVisible()) {
    GetFocusManager()->SetStoredFocusView(windowed_find_bar_focus_proxy_view_);
    windowed_find_bar_focus_proxy_view_->PreventNextFindBarActivation(false);
  }
#endif
}

#if defined(OS_WIN)
void BrowserView::OnFindBarWindowActivationChanged(bool active) {
  if (windowed_find_bar_focus_proxy_view_ && active) {
    VLOG(1) << "Windowed find bar activated";
    GetFocusManager()->SetStoredFocusView(windowed_find_bar_focus_proxy_view_);
    windowed_find_bar_focus_proxy_view_->PreventNextFindBarActivation(true);
  }
}

bool BrowserView::IsFocusInFindBarWindow() {
  bool focused = false;
  if (windowed_find_bar_focus_proxy_view_) {
    // The find bar can be the active window during a tab switch (Ctrl+tab).
    FindBar* find_bar = browser()->GetFindBarController()->find_bar();
    focused = find_bar->IsFindBarActiveWindow() ||
        windowed_find_bar_focus_proxy_view_->FindBarHasFocusBeforeTabSwitch();
    VLOG(1) << "IsFocusInFindBarWindow - persist focus: " << focused;
    find_bar->SetNoActivate();

    // Ensure the frame is active before handling the tab switch. When handling
    // a tab switch by Ctrl+Tab or ActivateTabAt (used in tests) as opposed to
    // clicking on a tab, the find bar window won't deactivate and won't trigger
    // the logic that watches activation events.
    if (find_bar->IsFindBarActiveWindow()) {
      RestoreFocus();
    }
  }
  return focused;
...
void BrowserView::PreventNextFindBarActivation() {
  if (windowed_find_bar_focus_proxy_view_) {
    windowed_find_bar_focus_proxy_view_->PreventNextFindBarActivation(true);
  }
}
...
base::string16 BrowserView::GetAccessibleTabLabel(bool include_app_name,
  ...
  switch (alert.value()) {
    ...
    case TabAlertState::SLEEPING:
      return l10n_util::GetStringFUTF16(IDS_TAB_AX_LABEL_SLEEPING_FORMAT,
                                        title);
  }
...
views::View* BrowserView::GetContentsContainer() {
  if (IsCollectionsEnabled() || features::edge::IsHubEnabled()) {
    return sidebar_contents_split_container_;
  }

  if (features::edge::IsReactiveSearchEnabled()) {
    return searchable_contents_split_container_;
  }

  return contents_container_;
}

views::View* BrowserView::GetContentsContainerForTest() {
  return GetContentsContainer();
}
...
#if defined(MICROSOFT_EDGE_BUILD)
void BrowserView::RemoveEdgeFreFromView() {
  if (edge_fre_view_ != nullptr) {
    edge_fre_view_->parent()->RemoveChildView(edge_fre_view_);
    BrowserViewLayout* browser_view_layout = GetBrowserViewLayout();
    if (browser_view_layout)
      browser_view_layout->set_fre_view(nullptr);
    delete edge_fre_view_;
    edge_fre_view_ = nullptr;
  }
}

void BrowserView::UpdateTranslateIcon(translate::TranslateStep step) {
  TranslateIconView* translate_icon = static_cast<TranslateIconView*>(
      toolbar_button_provider_->GetPageActionIconView(
          PageActionIconType::kTranslate));
  translate_icon->UpdateIcon(step);
}
#endif
...
void BrowserView::EnsureFocusOrder() {
  ...
  View* real_contents_container = GetContentsContainer();
  if (download_shelf_ && real_contents_container)
    InsertIntoFocusOrderAfter(real_contents_container, download_shelf_.get());
...
#if defined(MICROSOFT_EDGE_BUILD)
    if (read_aloud_bar_view_->ShouldBeVisible()) {
      bounds->set_height(bounds->height() +
                         read_aloud_bar_view_->GetPreferredSize().height());
    }
#endif
...
void BrowserView::AddNotificationToFocus(views::View* view) {
  notifications_to_focus_.push_back(view);
}

bool BrowserView::RemoveNotificationFromFocus(views::View* view) {
  auto notification_to_remove = std::find(notifications_to_focus_.begin(),
                                          notifications_to_focus_.end(), view);
  if (notification_to_remove != notifications_to_focus_.end()) {
    notifications_to_focus_.erase(notification_to_remove);
    return true;
  }
  return false;
}

const std::vector<views::View*>* BrowserView::GetNotificationsToFocus() const {
  return &notifications_to_focus_;
}
...
#if defined(MICROSOFT_EDGE_BUILD)
    // Show Collections panel when there is a command line switch.
    // This is for testing Collections and not used for user scenarios.
    bool show_collections =
        IsCollectionsEnabled() &&
        CollectionsManager::ShouldShowAfterOpenNewWindow() && active &&
        !CollectionsManager::GetWebContents(browser_.get());

    if (show_collections) {
      CollectionsManager::Show(browser_.get(), CollectionsShowAction::Show());
    }
#endif
  }
...
void BrowserView::OnWidgetMove() {
  ...
  // If the find bar is a popup, reposition it when the window moves.
  if (!features::edge::UseAuraForFindBar(GetProfile()) &&
      browser()->HasFindBarController()) {
...
void BrowserView::GetAccessiblePanes(std::vector<views::View*>* panes) {
  ...
  // skipped.
  // This ordering was decided from an accessibility review. Refer to
  // https://microsoft.visualstudio.com/Edge/_workitems/edit/25560468 for
  // context.
#if BUILDFLAG(ENABLE_WEBUI_TAB_STRIP)
  if (webui_tab_strip_)
...
#if defined(MICROSOFT_EDGE_BUILD)
  if (read_aloud_bar_view_)
    panes->push_back(read_aloud_bar_view_);
#endif
  ...
  if (reactive_search_web_view_ && reactive_search_web_view_->GetVisible())
    panes->push_back(reactive_search_web_view_);
  if (sidebar_web_view_ && sidebar_web_view_->GetVisible())
    panes->push_back(sidebar_web_view_);
  if (download_shelf_.get())
...
void BrowserView::AddedToWidget() {
  ...
  auto browser_view_layout = std::make_unique<BrowserViewLayout>(
      ...
      contents_container_, GetContentsContainer(),
      immersive_mode_controller_.get(), web_footer_experiment,
      contents_separator_);
  browser_view_layout->set_edge_vertical_tab_container_anchor(
      edge_vertical_tab_container_anchor_);
#if defined(MICROSOFT_EDGE_BUILD)
  browser_view_layout->set_fre_view(edge_fre_view_);
  browser_view_layout->set_read_aloud_bar(read_aloud_bar_view_);
#endif
  if (edge_guided_switch_background_view_) {
    browser_view_layout->set_edge_guided_switch_background_view(
        edge_guided_switch_background_view_);
  }
  ...
  if (edge_vertical_tab_container_) {
    auto vertical_tab_host = std::make_unique<EdgeAnchoredViewWidgetHost>();
    vertical_tab_host->Init(
        edge_vertical_tab_container_anchor_, edge_vertical_tab_container_.get(),
        EdgeAnchoredViewWidgetHost::Lifetime::kAnchorWidget);

    if (base::FeatureList::IsEnabled(
            features::edge::kEdgeVerticalTabsShortUI)) {
      vertical_tab_host->SetMirrorAnchorVisibility(true);
    } else {
      vertical_tab_host->Show();
    }

    vertical_tab_host.release();  // Lifetime controlled by anchor widget now.
  }
...
#if defined(MICROSOFT_EDGE_BUILD)
  // The FRE view needs to be displayed on the top of ntp so painting the FRE
  // view in the last.
  if (edge_fre_view_ != nullptr) {
    edge_fre_view_->Paint(paint_info);
  }
#endif
}
...
#if defined(MICROSOFT_EDGE_BUILD)
      views::View* target_view = read_aloud_bar_view_;
      const int target_index = new_parent->GetIndexOf(target_view);
      DCHECK_GE(target_index, 0);
      new_parent->AddChildViewAt(bookmark_bar_view_.get(), target_index);
#else
...
void BrowserView::SetBrowserWebContents(content::WebContents* old_contents,
                                        content::WebContents* new_contents) {
  contents_web_view_->SetWebContents(new_contents);

#if defined(OS_WIN)
  WebContentsDualEngineBrowserObserver::Observe(new_contents);
#endif
}
...
void BrowserView::ProcessFullscreen(bool fullscreen,
  ...
  if (fullscreen) {
    ...
    // Move focus to web contents.
    content::WebContents* web_contents = GetActiveWebContents();
    if (web_contents) {
      web_contents->Focus();
    }

    // Notify the user we've exited full screen.
    edge::a11y::RaiseExitFullscreenEvent(GetNativeViewAccessible());
  }
  ...
  // locked mode. IsBrowserLockedFullscreen only implemented in ChromeOS
  if (platform_util::IsBrowserLockedFullscreen(browser_.get())) {
    ...
    // On Windows, immersive_mode_controller_ is a stub, the SetEnabled()
    // function was changed to not call NOTREACHED() so we wouldn't have to put
    // this code behind OS specific headers.
...
#if BUILDFLAG(ENABLE_EDGE_KIOSK)
  // Allow bubble for Edge Kiosk Public Browsing mode
  if (fullscreen &&
      (!chrome::IsRunningInAppMode() ||
        edge::kiosk::Manager::IsPublicBrowsingMode())) {
#else
  ...
  // Hide the vertical tabstrip container in fullscreen mode.
  if (edge_vertical_tab_container_) {
    edge_vertical_tab_container_->SetVisible(!fullscreen);
  }
}
...
bool BrowserView::ShouldUseImmersiveFullscreenForUrl(const GURL& url) const {
  ...
  return url.is_empty();
#else
  // Kiosk fullscreen modes should never have ImmersiveMode.
#if BUILDFLAG(ENABLE_EDGE_KIOSK)
  if (edge::kiosk::Manager::IsFullScreenMode(
        base::CommandLine::ForCurrentProcess()))
#else
  ...
  // Upstream thinks ImmersiveMode is ChromeOS only, but Edge has implemented on
  // other platforms.
  if (base::FeatureList::IsEnabled(features::edge::kEnableShyUI)) {
    return url.is_empty();
  }
...
#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
    // As settings can change, we will allow dev tools during intialization.
    // The current settings will be checked in FindCommandIdForAccelerator.
    const bool dev_tools_disabled = false;
    if (browser_->has_client_id_not_from_identity_proxy() &&
        !embedded_browser::IsCommandAllowedForEmbeddedBrowser(
            entry.command_id, browser_->is_embedded(), dev_tools_disabled))
      continue;
#endif  // ENABLE_EMBEDDED_BROWSER_WEBVIEW
...
void BrowserView::UpdateAcceleratorMetrics(const ui::Accelerator& accelerator,
  ...
  // Accelerator telemetry for Edge.
  switch (command_id) {
    case IDC_FOCUS_SEARCH:
      if (key_code == ui::VKEY_E) {
        base::RecordAction(UserMetricsAction("Accel_FocusSearch_E"));
    ...
    case IDC_DUPLICATE_TAB:
      if (key_code == ui::VKEY_K && accelerator.IsShiftDown()) {
        base::RecordAction(
            UserMetricsAction("Microsoft.Accel_DuplicateTab_Shift_K"));
      } else if (key_code == ui::VKEY_K) {
        base::RecordAction(UserMetricsAction("Microsoft.Accel_DuplicateTab_K"));
      }
      break;
    default:
      // Do nothing.
...
#endif
  EdgeProfileChooserView::ShowBubble(bubble_view_mode,
                                     access_point, avatar_button, browser(),
                                     focus_first_profile_button);
}

#if BUILDFLAG(ENABLE_WDAG)
void BrowserView::ShowWdagFlyoutDialog() {
  views::Button* wdag_flyout_button = toolbar_->wdag_flyout_button();
  wdag::FlyoutDialog::ShowFlyoutDialog(wdag_flyout_button);
}
#endif
...
#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
  if (browser_->has_client_id_not_from_identity_proxy() &&
      embedded_browser::IsDynamicAcceleratorForEmbeddedBrowser(*command_id)) {
    const bool dev_tools_disabled = embedded_browser::EmbeddedBrowserImpl::
        DevToolsDisabledForEmbeddedBrowser(GetActiveWebContents());
    return embedded_browser::IsCommandAllowedForEmbeddedBrowser(
        *command_id,
        browser_->is_embedded(),
        dev_tools_disabled);
  }
#endif
...
// ReparentTopContainerForEndOfImmersive was moved from Ended() to Exited() as
// creating and destroying the layer on each ShyUI dropdown presented problems.
// The layer now persists for the duration of ShyUI.
...
void BrowserView::UserColorModePreferenceChanged() {
  ui::NativeTheme::UserDarkModePreference user_dark_pref;
  if (browser_->profile()->GetPrefs()->GetBoolean(prefs::kUsesSystemTheme)) {
    user_dark_pref = ui::NativeTheme::UserDarkModePreference::FollowSystem;
  } else {
    // If we're not following system, it means we're either forcing light or dark
    user_dark_pref = browser_->profile()->GetPrefs()->GetBoolean(prefs::kDarkTheme) ?
      ui::NativeTheme::UserDarkModePreference::Dark :
      ui::NativeTheme::UserDarkModePreference::Light;
  }

  ui::NativeTheme::GetInstanceForNativeUi()->UpdateDarkModeStateByUserPref(user_dark_pref);
}

#if defined(MICROSOFT_EDGE_BUILD)
bool BrowserView::IsCollectionsEnabled() const {
  return features::edge::IsCollectionsFeatureEnabled(
             features::edge::kEdgeCollections) &&
         browser_->is_type_normal() &&
         browser_->profile()->GetPrefs()->GetBoolean(
             prefs::kEnableEdgeCollections) &&
         !browser_->profile()->IsOffTheRecord() && !IsGuestSession();
}

void BrowserView::OnPinterestRecommendationsPrefChanged() {
  CollectionsManager::PinterestRecommendationsPrefChanged(browser_.get());
}
#endif
...
#if defined(OS_WIN)
void BrowserView::OnEngineSwapped() {
  if (GetActiveWebContents()->IsHostingInternetExplorer()) {
    // Close the omnibox popup, if any.
    LocationBarView* location_bar_view = GetLocationBarView();
    if (location_bar_view) {
      location_bar_view->GetOmniboxView()->CloseOmniboxPopup();
    ...
    // Bring devtools window back if it's swapped from AndIE tab to Edge tab.
    UpdateDevToolsForContents(GetActiveWebContents(), true);
  }
}
#endif  // defined(OS_WIN)

void BrowserView::set_fre_container(edge::fre::EdgeFreView* edge_fre_view) {
  edge_fre_view_ = edge_fre_view;
  BrowserViewLayout* browser_view_layout = GetBrowserViewLayout();
  if (browser_view_layout) {
    browser_view_layout->set_fre_view(edge_fre_view);
  }
  AddChildView(edge_fre_view);
}
