...
class MetricsReportingStateTest : public InProcessBrowserTest,
 ...
 public:
  ...
  void SetUp() override {
    SetForceBrowserConsentLevel(true);
...
class EdgeMetricsReportingStateTest : public InProcessBrowserTest {
 public:
  EdgeMetricsReportingStateTest() = default;
  ~EdgeMetricsReportingStateTest() override = default;
...
IN_PROC_BROWSER_TEST_F(EdgeMetricsReportingStateTest,
                       DiagnosticDataDeleteInitiated_IsClientIdReset) {
  std::string initial_client_id = "09b470a7-2360-4670-b24a-c38eb8503cd4";

  pref_service_->SetString(metrics::prefs::kMetricsClientID, initial_client_id);

  // Trigger deleting of Diagnostic Data processing.
  OnDeleteDiagnosticDataRequested();

  // This should reset the client_id, so let's confirm that.
  EXPECT_NE(initial_client_id,
            pref_service_->GetString(metrics::prefs::kMetricsClientID));
}
