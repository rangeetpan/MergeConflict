...
<<<<<<< HEAD
#include "chrome/browser/edge_trust/trust_manager/mock_manager.h"
...
class ContentSettingImageModelTest : public BrowserWithTestWindowTest {
 public:
  ...
  void RegisterTrustManager() {
    scoped_refptr<edge_trust::MockTrustManager> trust_manager_;
    ...
    base::FilePath dummy_path;
    TestingPrefServiceSimple local_state;

    trust_manager_ = base::MakeRefCounted<edge_trust::MockTrustManager>(
        test_shared_loader_factory_, dummy_path, &local_state);
    trust_manager_->CreateNewMockTrackingChecker();
    TestingBrowserProcess::GetGlobal()->SetTrustManagerForTesting(
        trust_manager_.get());
  }

  void UnregisterTrustManager() {
    TestingBrowserProcess::GetGlobal()->SetTrustManagerForTesting(nullptr);
  }
...
TEST_F(ContentSettingImageModelTest, TrackingPreventionHidesCookies) {
  ...
  content_settings->OnContentBlocked(ContentSettingsType::TRACKERS);
  content_setting_image_model->Update(web_contents());
...
TEST_F(ContentSettingImageModelTest, TrackingPreventionChangesCookies) {
  ...
  // Block cookie. UI should show
  GURL origin("http://contoso.com");
  std::unique_ptr<net::CanonicalCookie> cookie(net::CanonicalCookie::Create(
      origin, "A=B", base::Time::Now(), base::nullopt /* server_time */));
  ASSERT_TRUE(cookie);
  web_contents()->OnCookieChange(origin, origin, *cookie, true);
  content_setting_image_model->Update(web_contents());
  EXPECT_TRUE(content_setting_image_model->is_visible());
  EXPECT_TRUE(HasIcon(*content_setting_image_model));
  EXPECT_FALSE(content_setting_image_model->get_tooltip().empty());

  // Block storage tracker. UI should hide.
  content_settings->OnTrackerBlocked(
      web_contents()->GetVisibleURL(), origin,
      tracking_prevention::TrackerType::STORAGE);
  content_setting_image_model->Update(web_contents());

  EXPECT_FALSE(content_setting_image_model->is_visible());

  UnregisterTrustManager();
}
