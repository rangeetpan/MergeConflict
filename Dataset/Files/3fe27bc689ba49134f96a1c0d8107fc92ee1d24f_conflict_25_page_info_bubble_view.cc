...
#include "chrome/browser/ui/views/hover_vector_icon_button.h"
...
#include "components/edge_demo_mode/common/demo_mode_features.h"
#include "components/edge_demo_mode/core/demo_settings.h"
#include "components/edge_demo_mode/core/replacement.h"
...
#include "chrome/browser/safe_browsing/edge_password_protection_service_impl.h"
#endif
...
#include "components/edge_smartscreen/common/features.h"
#include "components/edge_smartscreen/common/smartscreen_reporter.h"
#endif
...
bool ShouldTreatAsSecureForDemoMode(const GURL& url) {
  if (!features::edge::IsEdgeDemoModeEnabled()) {
    return false;
  }

  demo::Replacement* dataReplacement =
      demo::DemoSettings::GetReplacementData(url);
  return dataReplacement != nullptr;
}
}  // namespace
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
  void AddSmartScreenButtons(bool showing_interstitial, bool is_suspicious,
                             bool should_allow_proceed);

  void AddSmartScreenUnsafeUrls(content::WebContents* web_contents);
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
  // A container for the label buttons used to report site safe or continue to
  // the unsafe site.
  views::View* smartscreen_button_container_ = nullptr;

  // A container for the URL to be shown on unsafe sites.
  views::StyledLabel* smartscreen_unsafe_url_label_ = nullptr;
#endif
...
BubbleHeaderView::BubbleHeaderView(
  ...
  if (security::features::edge::IsSmartScreenEnabled()) {
    auto smartscreen_unsafe_url_label = std::make_unique<views::StyledLabel>(
        base::string16(), styled_label_listener);
    smartscreen_unsafe_url_label->SetID(
        PageInfoBubbleView::VIEW_ID_PAGE_INFO_LABEL_UNSAFE_URLS);
    smartscreen_unsafe_url_label_ =
        layout->AddView(std::move(smartscreen_unsafe_url_label), 1.0, 1.0,
  ...
  security_details_label->SetLineHeight(8);
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
  if (security::features::edge::IsSmartScreenEnabled()) {
    smartscreen_button_container_ =
        layout->AddView(std::make_unique<views::View>(), 1.0, 1.0,
                        views::GridLayout::FILL, views::GridLayout::LEADING);
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
void BubbleHeaderView::AddSmartScreenUnsafeUrls(
  content::WebContents* web_contents) {
  if (!security::features::edge::IsSmartScreenEnabled())
    return;

  scoped_refptr<smartscreen::SmartScreenUIManager> ss_ui_manager =
      g_browser_process->smartscreen_service_client(true /*get_when_pref_off*/)
          ->ui_manager();

  GURL subframe_url = ss_ui_manager->GetUnsafeFrameUrlToBeReported(web_contents);

  content::InterstitialPage* interstitial_page =
    web_contents->GetInterstitialPage();

  GURL main_frame_url;
  // This should be consistent with the feedback URL we use in
  // AddSmartScreenButtons below.
  if (interstitial_page == nullptr) {
    main_frame_url = web_contents->GetVisibleURL();
  } else {
    main_frame_url = interstitial_page->GetUrl();
  }

  // For Demo Mode, show the user-defined demo url.
  if (features::edge::IsEdgeDemoModeEnabled()) {
    if (const demo::Replacement* dataReplacement =
            demo::DemoSettings::GetReplacementData(main_frame_url)) {
      main_frame_url = dataReplacement->GetDisplayUrl();
    }
  }

  auto truncate_string = [](const std::string& str) {
    if (str.size() > 256) {
      return str.substr(0, 253) + "...";
    } else {
      return str;
    }
  };
  ...
  if (interstitial_page != nullptr ||
      ss_ui_manager->IsShowingTopLevelBlockingPage(web_contents) ||
      subframe_url.is_empty()) {
    smartscreen_unsafe_url_label_->SetText(
      l10n_util::GetStringFUTF16(
        IDS_PAGE_INFO_SMARTSCREEN_UNSAFE_URL_DESCRIPTION,
        base::UTF8ToUTF16(
          truncate_string(main_frame_url.GetWithEmptyPath().spec()))));
  } else {
    smartscreen_unsafe_url_label_->SetText(
      l10n_util::GetStringFUTF16(
        IDS_PAGE_INFO_SMARTSCREEN_UNSAFE_URL_IFRAME_DESCRIPTION,
        base::UTF8ToUTF16(truncate_string(main_frame_url.spec())),
        base::UTF8ToUTF16(truncate_string(subframe_url.spec()))));
  }

  // Add padding at the bottom.
  smartscreen_unsafe_url_label_->SetBorder(
    views::CreateEmptyBorder(0, views::GridLayout::kFixedSize, 8, 0));

  InvalidateLayout();
}

void BubbleHeaderView::AddSmartScreenButtons(
  bool showing_interstitial,
  bool is_suspicious,
  bool should_allow_proceed) {
  if (!security::features::edge::IsSmartScreenEnabled())
    return;

  if (!smartscreen_button_container_->children().empty()) {
    // Ensure all old content is removed from the container before re-adding it.
    smartscreen_button_container_->RemoveAllChildViews(true /*delete*/);
  }

  auto report_button =
      views::MdTextButton::CreateSecondaryUiBlueButton(
        button_listener_, base::string16());
  if (is_suspicious) {
    report_button->SetText(
      l10n_util::GetStringUTF16(IDS_PAGE_INFO_REPORT_SAFE_OR_UNSAFE_BUTTON));
    report_button->SetID(
        PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_REPORT_SUSPICIOUS);
  } else {
    report_button->SetText(
      l10n_util::GetStringUTF16(IDS_PAGE_INFO_REPORT_SAFE_BUTTON));
    report_button->SetID(
        PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_REPORT_SAFE);
  }

  // If the site is suspicious, there's no secondary button.
  // Otherwise, this is the "continue to unsafe site" button.
  // We will only show this button if should_allow_proceed is true.
  auto secondary_button =
      views::MdTextButton::CreateSecondaryUiButton(
        button_listener_, l10n_util::GetStringUTF16(
          IDS_PAGE_INFO_CONTINUE_TO_UNSAFE_SITE_BUTTON));
  secondary_button->SetID(
    PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_CONTINUE_TO_UNSAFE_SITE);

  int kSpacingBetweenButtons = 8;

  // If these two buttons cannot fit into a single line, stack them vertically.
  // If there's only one button then the layout doesn't matter.
  bool can_fit_in_one_line =
      (smartscreen_button_container_->width() - kSpacingBetweenButtons) >=
      (report_button->CalculatePreferredSize().width() +
       secondary_button->CalculatePreferredSize().width());
  auto layout = std::make_unique<views::BoxLayout>(
      can_fit_in_one_line ? views::BoxLayout::Orientation::kHorizontal
                          : views::BoxLayout::Orientation::kVertical,
      gfx::Insets(), kSpacingBetweenButtons);
  ...
  // become right-aligned.
  layout->set_main_axis_alignment(views::BoxLayout::MainAxisAlignment::kStart);
  smartscreen_button_container_->SetLayoutManager(std::move(layout));

  const bool disable_anyway = !g_browser_process
      ->smartscreen_service_client(true /*get_when_pref_off*/)
          ->GetDisableContinueAnywayPolicyValue();
#if defined(OS_WIN) || defined(OS_LINUX)
  smartscreen_button_container_->AddChildView(
      std::move(report_button));
  if ((showing_interstitial && disable_anyway) && !is_suspicious &&
      should_allow_proceed) {
    smartscreen_button_container_->AddChildView(std::move(secondary_button));
  }
  ...
  smartscreen_button_container_->AddChildView(std::move(report_button));
#else
  NOTREACHED();
#endif

  // Add padding at the top.
  smartscreen_button_container_->SetBorder(
      views::CreateEmptyBorder(8, views::GridLayout::kFixedSize, 0, 0));
...
InternalPageInfoBubbleView::InternalPageInfoBubbleView(
  ...
  if (extensions::util::HasExtensionScheme(url)) {
...
views::BubbleDialogDelegateView* PageInfoBubbleView::CreatePageInfoBubble(
  ...
  if (!ShouldTreatAsSecureForDemoMode(url)) {
    if (url.SchemeIs(content::kChromeUIScheme) ||
        url.SchemeIs(content::kChromeDevToolsScheme) ||
        extensions::util::HasExtensionScheme(url) ||
        url.SchemeIs(content::kViewSourceScheme) ||
#if EXCLUDED_FROM_EDGE
        url.SchemeIs(url::kFileScheme) ||
        url.SchemeIs(dom_distiller::kDomDistillerScheme)) {
#else
        // For edge, we want to show the full page info bubble currently instead
        // of one liner flyout for now.
        url.SchemeIs(url::kFileScheme)) {
#endif
      return new InternalPageInfoBubbleView(anchor_view, anchor_rect,
                                            parent_view, web_contents, url);
    }
  }
...
PageInfoBubbleView::PageInfoBubbleView(
  ...
  if (features::edge::IsEnhancedTrackingPreventionEnabled()) {
    layout->StartRow(views::GridLayout::kFixedSize, kColumnId);
    tracking_prevention_container_view_ = layout->AddView(
        TrackingPreventionView::CreateTrackingPreventionContainerView());
  }
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
void PageInfoBubbleView::OnReportSite(content::WebContents* web_contents) {
  if (!security::features::edge::IsSmartScreenEnabled())
    return;

#if defined(OS_WIN)
  const std::string prod_guid = smartscreen::kSmartScreenFeedbackProdGuidWin;
#elif defined(OS_MACOSX)
  const std::string prod_guid = smartscreen::kSmartScreenFeedbackProdGuidMac;
#elif defined(OS_LINUX)
  const std::string prod_guid = smartscreen::kSmartScreenFeedbackProdGuidLinux;
#else
  const std::string prod_guid = "";
  NOTREACHED();
#endif
  ...
  GURL feedback_url = ss_ui_manager->GetFeedbackUrl(web_contents);

  GURL request_url(feedback_url.GetWithoutQuery());
  std::string post_data = feedback_url.query();
...
void PageInfoBubbleView::ButtonPressed(views::Button* button,
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
  scoped_refptr<smartscreen::SmartScreenUIManager> ss_ui_manager;
  smartscreen::SmartScreenServiceClient* ss_service_client;
#endif // #if BUILDFLAG(ENABLE_SMARTSCREEN)
  ...
  switch (button->GetID()) {
    case PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_CLOSE:
      ...
      break;
#if BUILDFLAG(ENABLE_SMARTSCREEN)
    case PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_REPORT_SAFE:
    case PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_REPORT_SUSPICIOUS:
      GetWidget()->Close();
      OnReportSite(web_contents());
      break;
    case PageInfoBubbleView::VIEW_ID_PAGE_INFO_BUTTON_CONTINUE_TO_UNSAFE_SITE:
      GetWidget()->Close();
      ss_service_client = g_browser_process
          ->smartscreen_service_client(true /*get_when_pref_off*/);
      ss_ui_manager = ss_service_client->ui_manager();

      if (base::FeatureList::IsEnabled(
              security::features::edge::kSmartScreenTopLevelFrameBlocking) &&
          ss_ui_manager->IsShowingTopLevelBlockingPage(web_contents())) {
        smartscreen::SmartScreenServiceClient::
            NotifyTopLevelFrameInterstitialOnProceed(web_contents());
      } else if (ss_ui_manager->IsBlockingProceedableIframeContent(
                     web_contents())) {
        ss_ui_manager->AddBlockedIFramesToTrusted(web_contents());
        TRACE_EVENT0("SmartScreen", "UserClickedProceedInFlyout");

        ss_service_client->LogUserActionTelemetry(
      ...
      if (web_contents()->GetInterstitialPage() != nullptr) {
        ss_service_client
            ->LogUserActionTelemetry(web_contents(), "override", "");
        web_contents()->GetInterstitialPage()->Proceed();
      }
      // Refresh the view to remove the continue to unsafe site button.
      header_->AddSmartScreenButtons(
          false /*showing_interstitial*/,
          false /*is_suspicious*/,
          false /*should_allow_proceed*/);
      break;
...
gfx::Size PageInfoBubbleView::CalculatePreferredSize() const {
  ...
  if (site_settings_view_) {
    ...
    if (tracking_prevention_container_view_) {
      // If bubble is too narrow for tracking prevention text, size to fit
      // width.
      width = std::max(
          width,
          tracking_prevention_container_view_->GetPreferredSize().width());
    }
...
#if defined(OS_WIN)
  // Hide the cookies button in IE mode because it is not currently supported.
  if (web_contents()->IsHostingInternetExplorer()) {
    return;
  }
#endif
  ...
  if (cookie_button_ == nullptr) {
    ...
    info.is_incognito =
        Profile::FromBrowserContext(web_contents()->GetBrowserContext())
            ->IsOffTheRecord();
    const gfx::ImageSkia icon = PageInfoUI::GetPermissionIcon(
        info, GetNativeTheme()->GetSystemColor(
...
views::View* PageInfoBubbleView::GetInitiallyFocusedView() {
...
void PageInfoBubbleView::SetIdentityInfo(const IdentityInfo& identity_info) {
  ...
  if (identity_info.certificate) {
    ...
    const gfx::ImageSkia icon =
        PageInfoUI::GetCertificateIcon(GetNativeTheme()->GetSystemColor(
            ui::NativeTheme::kColorId_EnabledMenuItemForegroundColor));
    ...
    if (certificate_button_) {
      ...
      auto to_delete =
          std::make_unique<PageInfoHoverButton*>(certificate_button_);
    }
  ...
  if (security::features::edge::IsSmartScreenEnabled()) {
    if (identity_info.safe_browsing_status ==
        ...
        identity_info.safe_browsing_status ==
            PageInfo::SAFE_BROWSING_STATUS_UNSAFE_FRAME ||
        identity_info.safe_browsing_status ==
            PageInfo::SAFE_BROWSING_STATUS_SUSPICIOUS) {
      scoped_refptr<smartscreen::SmartScreenUIManager> ss_ui_manager =
          g_browser_process
              ->smartscreen_service_client(true /*get_when_pref_off*/)
              ->ui_manager();
      ...
      header_->AddSmartScreenButtons(
          web_contents()->ShowingInterstitialPage() ||
              ss_ui_manager->IsShowingTopLevelBlockingPage(web_contents()) ||
              ss_ui_manager->IsBlockingProceedableIframeContent(web_contents()),
          identity_info.safe_browsing_status ==
              PageInfo::SAFE_BROWSING_STATUS_SUSPICIOUS,
          ss_ui_manager->GetShouldAllowProceed(web_contents()));

      header_->AddSmartScreenUnsafeUrls(web_contents());
    }
  }
#endif  // BUILDFLAG(ENABLE_SMARTSCREEN)
...
void PageInfoBubbleView::SetPageFeatureInfo(const PageFeatureInfo& info) {
  ...
  icon->SetImage(PageInfoUI::GetVrSettingsIcon(GetNativeTheme()->GetSystemColor(
      ui::NativeTheme::kColorId_EnabledMenuItemForegroundColor)));
  ...
  auto button = std::make_unique<HoverVectorIconButton>(
      this, std::move(icon),
...
void PageInfoBubbleView::SetTrackingPreventionInfo(
    const PageInfoUI::TrackingPreventionInfo& tracking_prevention_info,
    const PageInfoUI::PermissionInfo& permission_info) {
  if (tracking_prevention_view_ == nullptr) {
    tracking_prevention_view_ = new TrackingPreventionView();
    tracking_prevention_view_->AddObserver(this);
    tracking_prevention_container_view_->AddChildView(
        tracking_prevention_view_);
  }

  tracking_prevention_view_->SetTrackingPreventionInfo(
      tracking_prevention_info, permission_info, profile_, web_contents());

  Layout();
  SizeToContents();
}

// TrackingPreventionViewObserver:
void PageInfoBubbleView::OnTrackingPreventionSettingChanged(
    const PageInfoUI::PermissionInfo& permission) {
  OnPermissionChanged(permission);
}

void PageInfoBubbleView::OnTrackingPreventionManagePreferencesClicked() {
  presenter_->OpenTrackingPreventionSettingsView();
  presenter_->RecordPageInfoAction(
      PageInfo::PAGE_INFO_TRACKING_PREVENTION_SETTINGS_OPENED);
}

void PageInfoBubbleView::OnBlockedTrackerItemClicked(
    const std::string& tracker_org_name) {
  if (features::edge::IsLoadStatisticsEnabled() && !tracker_org_name.empty()) {
    presenter_->OpenTrackerDetailsSettingsView(tracker_org_name);
  } else {
    presenter_->OpenBlockedTrackersSettingsView();
  }
  presenter_->RecordPageInfoAction(
      PageInfo::PAGE_INFO_BLOCKED_TRACKERS_SETTINGS_OPENED);
}
...
PageInfoBubbleView::CreateSecurityDescriptionForPasswordReuse() const {
  ...
  auto* service = safe_browsing::EdgePasswordProtectionServiceImpl::
