...
=======
>>>>>>> b48062615859a4c3593b834a8de4f5a99ce2e751
...
UnPackStatus UnPackArchive(const base::FilePath& archive,
  ...
  VLOG(1) << "Opening archive " << PII_STRING(archive.value());
  LzmaUtilImpl lzma_util;
  UnPackStatus status;
  if ((status = lzma_util.OpenArchive(archive)) != UNPACK_NO_ERROR) {
    PLOG(ERROR) << "Unable to open install archive: "
                << PII_STRING(archive.value());
  } else {
    VLOG(1) << "Uncompressing archive to path " <<
            PII_STRING(output_dir.value());
    if ((status = lzma_util.UnPack(output_dir, output_file)) != UNPACK_NO_ERROR)
      PLOG(ERROR) << "Unable to uncompress archive: "
                  << PII_STRING(archive.value());
  }
