...
>>>>>>> 98a553c49b10be1297ad0f89ac110ae29113dfcf
...
#include "chrome/common/extensions/manifest_handlers/edge_sharetarget_handler.h"
#include "chrome/common/win/pwa_identity_proxy_util.h"
#endif
...
base::Value CreateProtocolHandlersForBookmarkApp(
    const std::vector<blink::Manifest::ProtocolHandler>&
        manifest_protocol_handlers) {
  base::Value protocol_handlers(base::Value::Type::LIST);

  for (const auto& entry : manifest_protocol_handlers) {
    base::Value protocol_handler(base::Value::Type::DICTIONARY);

    protocol_handler.SetKey(keys::kProtocolHandlerProtocol,
                            base::Value(entry.protocol));
    protocol_handler.SetStringKey(keys::kProtocolHandlerUrl, entry.url.spec());

    protocol_handlers.Append(std::move(protocol_handler));
  }
...
base::Value CreateShareTargetForBookmarkApp(
    const blink::Manifest::ShareTarget& share_target) {
  // At ShareTarget creation time all appropriate default/fallbacks/validations
  // will have been applied, so they are not needed at this point.
  base::Value share_target_dic(base::Value::Type::DICTIONARY);

  DCHECK(share_target.action.is_valid());
  share_target_dic.SetStringKey(keys::kShareTargetAction,
                                share_target.action.spec());

  if (share_target.enctype ==
      blink::Manifest::ShareTarget::Enctype::kFormUrlEncoded) {
    share_target_dic.SetStringKey(keys::kShareTargetEnctype,
                                  "application/x-www-form-urlencoded");
  } else {
    DCHECK(share_target.enctype ==
  ...
  if (share_target.method == blink::Manifest::ShareTarget::Method::kGet) {
    share_target_dic.SetStringKey(keys::kShareTargetMethod, "GET");
  } else {
    DCHECK(share_target.method == blink::Manifest::ShareTarget::Method::kPost);
    share_target_dic.SetStringKey(keys::kShareTargetMethod, "POST");
  }

  base::Value params_dic(base::Value::Type::DICTIONARY);

  size_t files_count = share_target.params.files.size();
  if (files_count > 0) {
    base::Value files_value(base::Value::Type::LIST);
    for (size_t i = 0; i < files_count; i++) {
      base::Value file_value(base::Value::Type::DICTIONARY);
      size_t accept_count = share_target.params.files[i].accept.size();
      base::Value accept_value(base::Value::Type::LIST);
      for (size_t j = 0; j < accept_count; j++) {
        accept_value.Append(share_target.params.files[i].accept[j]);
      }
      file_value.SetKey(keys::kShareTargetParamsFileAccept,
                        std::move(accept_value));
      file_value.SetStringKey(keys::kShareTargetParamsFileName,
                              share_target.params.files[i].name);
      files_value.Append(std::move(file_value));
    }
    params_dic.SetKey(keys::kShareTargetParamsFiles, std::move(files_value));
  }

  if (!share_target.params.text.is_null())
    params_dic.SetStringKey(
        keys::kShareTargetParamsText,
        base::UTF16ToUTF8(share_target.params.text.string()));

  if (!share_target.params.title.is_null())
    params_dic.SetStringKey(
        keys::kShareTargetParamsTitle,
        base::UTF16ToUTF8(share_target.params.title.string()));

  if (!share_target.params.url.is_null())
    params_dic.SetStringKey(
        keys::kShareTargetParamsUrl,
        base::UTF16ToUTF8(share_target.params.url.string()));

  share_target_dic.SetKey(keys::kShareTargetParams, std::move(params_dic));

  return share_target_dic;
}
...
scoped_refptr<Extension> ConvertWebAppToExtension(
  ...
  if (web_app.display_modifiers.size() > 0) {
    auto display_modifiers = std::make_unique<base::ListValue>();

    for (const auto entry : web_app.display_modifiers) {
      display_modifiers->Append(blink::DisplayModifierToString(entry));
    }

    root->Set(keys::kAppDisplayModifiers, std::move(display_modifiers));
  }
  ...
  if (!web_app.protocol_handlers.empty()) {
    root->SetKey(keys::kProtocolHandlers, CreateProtocolHandlersForBookmarkApp(
...
<<<<<<< HEAD
  int creation_flags = extra_creation_flags | Extension::FROM_BOOKMARK;
#if BUILDFLAG(ENABLE_PWA_WIN10_PACKAGE)
  if (web_app.share_target &&
      base::FeatureList::IsEnabled(features::edge::kIdentityProxyShareTarget)) {
    root->SetKey(keys::kShareTarget,
                 CreateShareTargetForBookmarkApp(web_app.share_target.value()));
  }

  // Sidecar is enabled for PWA only over the Win10 and Win32alacarte
  // is for Vibranium+. Win32alcarte is also enabled for
  // PinSite in addition to PWA.
  bool unsigned_package = false;
  if (identity_proxy_util::IsIdentityProxySupported(&unsigned_package)) {
    bool sidecar_enabled =
        !web_app.scope.is_empty() &&
        base::FeatureList::IsEnabled(features::edge::kEdgePwaWin10Package);

    // WCOS doesn't have alternative solution for enlisting PWA on the Launcher
    // though besides Identity Proxy so we check flag only in the Desktop.
    bool identiy_proxy_enabled =
        (base::FeatureList::IsEnabled(features::edge::kIdentityProxy) ||
         !base::win::IsDesktopSku());  // it is enabled for WCOS all the time.

    if (sidecar_enabled || (unsigned_package && identiy_proxy_enabled))
      creation_flags |= Extension::ALLOW_PWA_WIN10_PACKAGE;
  }
