...
#include "content/browser/dual_engine/dual_engine_find_in_page.h"
#include "content/browser/dual_engine/dual_engine_telemetry_utils.h"
...
>>>>>>> 379274b0f71f52d8a7957f67da00bf0764d6f3cb
...
#if defined(OS_WIN)
#include "content/browser/dual_engine/dual_engine_constants_win.h"
#include "content/browser/dual_engine/internet_explorer_session_host_win.h"
...
#if defined(OS_WIN)
  if (root->favicon_render_frame_host()) {
    root->favicon_render_frame_host()->DeleteRenderFrame(
    FrameDeleteIntention::kInternetExplorerFaviconFrameDiscarded);
    root->favicon_render_frame_host()->SetRenderFrameCreated(false);
  }
#endif
...
#if defined(OS_WIN)
  for (auto& observer : internet_explorer_observers_)
    observer.ResetWebContents();
#endif
...
bool WebContentsImpl::IsVisibleEntryFirstTab() {
  // We may not have a navigation entry yet.
  NavigationEntry* entry = controller_.GetVisibleEntry();
  return entry ? entry->IsFirstTab() : false;
}
...
const GURL& WebContentsImpl::GetOriginalRequestURL() {
  // We may not have a navigation entry yet.
  NavigationEntry* entry = controller_.GetVisibleEntry();
  return entry ? entry->GetOriginalRequestURL() : GURL::EmptyGURL();
}
...
RenderFrameHost* WebContentsImpl::GetFrameByName(std::string name) {
  // TODO: Add functionality to get top-level frames only
  std::vector<content::RenderFrameHost*> frames = GetAllFrames();
...
dlp::Context WebContentsImpl::GetDlpContextFromMainFrame() {
#if defined(OS_WIN)
  // In the case where this is called before Internet Explorer sets its first
  // enterprise id state, we should treat it as though Internet
  // Explorer is not being hosted. This allows the callers to behave
  // consistently until Internet Explorer sets its first state.
  if (IsHostingInternetExplorer() &&
      internet_explorer_enterprise_id_state_ !=
          InternetExplorerEnterpriseIDState::kNone) {
    if (internet_explorer_enterprise_id_state_ ==
        InternetExplorerEnterpriseIDState::kProtected) {
      return dlp::Context(dlp::ProtectionType::kWip,
                          dlp::DataClassification::kProtected,
                          dlp::GetDefaultUniqueId(dlp::ProtectionType::kWip));
    }
    return dlp::ContextFactory::CreatePersonalContext();
  }
  // When WebContents isn't hosting Internet Explorer, we should reset
  // the state so that it won't erroneously use the old state the next time
  // that it hosts Internet Explorer.
  if (internet_explorer_enterprise_id_state_ !=
      InternetExplorerEnterpriseIDState::kNone) {
    internet_explorer_enterprise_id_state_ =
        InternetExplorerEnterpriseIDState::kNone;
  ...
  if (root_node) {
    return root_node->GetDlpContext();
  }
  return GetBrowserContext()->GetDlpContext();
}
...
#if defined(OS_WIN)
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    double zoom_level = HostZoomMap::GetZoomLevel(this);
    int zoom_percent = static_cast<int>(
        blink::PageZoomLevelToZoomFactor(zoom_level) * 100 + 0.5);
    tab_host->SetZoom(zoom_percent);

    // Early return if we're a Dual Engine tab since we don't have a RenderView
    // to sync
    return;
  }
  if (delegate_ &&
      delegate_->ShouldSkipUpdatingZoomOnRenderWidgetHost(this))
    return;
#endif
...
bool WebContentsImpl::ShouldUpdateCursorIfWindowNotFound() {
  if (delegate_) {
    return delegate_->ShouldUpdateCursorIfWindowNotFound(this);
  }
  return false;
}
...
#if defined(OS_WIN)
  // IE mode special case
  if (GetInternetExplorerTabHost()) {
    return true;
  }
...
void WebContentsImpl::SyncRendererPrefs() {
  ...
  #if defined(OS_WIN)
    dual_engine::InternetExplorerTabHost* tab_host =
        GetInternetExplorerTabHost();
    if (tab_host) {
        tab_host->EnableCaretMode(renderer_preferences.caret_browsing_enabled);
    }
  #endif
  SendPageMessage(
...
#if defined(OS_WIN)
  if (IsInternetExplorerIntegrationEnabled()) {
    tc->EnableInternetExplorerIntegration();
  }
#endif
...
// Notify IE when chromium exit from fullscreen mode
#if defined(OS_WIN)
  SetMediaFullscreenModeToInternetExplorer(false);
#endif
...
void WebContentsImpl::ShouldCreateNewWindowAsync(
    RenderFrameHost* opener,
    const GURL& target_url,
    const bool has_user_gesture,
    base::OnceCallback<void(bool)> is_allowed_callback) {
  if (delegate_) {
    delegate_->ShouldCreateNewWebContents(
        this, opener, target_url, has_user_gesture,
        std::move(is_allowed_callback));
  } else {
    std::move(is_allowed_callback).Run(true);
  }
}
...
#if defined(OS_WIN)
  // Always take the IE host pointer here, to make sure it gets cleaned up if
  // we decide not to host it.
  std::unique_ptr<dual_engine::InternetExplorerTabHost> ie_host =
      static_cast<RenderFrameHostImpl*>(opener)->
          TakePendingInternetExplorerHostForNewWindow();
  if (ie_host) {
    DCHECK(IsInternetExplorerIntegrationEnabled() &&
           opener->IsHostingInternetExplorer());
    DCHECK(params.opener_suppressed && !is_guest);
  }
#endif
...
#if defined(OS_WIN)
  if (IsInternetExplorerIntegrationEnabled()) {
    new_contents_impl->EnableInternetExplorerIntegration();
    if (forced_engine_ != dual_engine::ForcedEngine::kNone) {
      new_contents_impl->SetDualEngineForcedEngine(forced_engine_);
    }
  }
#endif
...
#if defined(OS_WIN)
      if (opener->IsHostingInternetExplorer()) {
        initial_rect = GetInitialRectForNewInternetExplorerWindow(params);
      }
#endif
...
#if defined(OS_WIN)
      if (ie_host) {
        DCHECK(delegate_->ShouldResumeRequestsForCreatedWindow());

        // Hand off the IE host
        new_contents_impl->HostInternetExplorerTab(std::move(ie_host));
...
#if defined(OS_WIN)
      load_params->internet_explorer_initiated =
          opener->IsHostingInternetExplorer();
#endif
...
#if defined(OS_WIN)
gfx::Rect WebContentsImpl::GetInitialRectForNewInternetExplorerWindow(
    const mojom::CreateNewWindowParams& params) {
  gfx::Rect initial_rect;
  if (params.features &&
      (params.disposition == WindowOpenDisposition::NEW_POPUP ||
      params.disposition == WindowOpenDisposition::NEW_WINDOW)) {
    initial_rect = GetTopLevelNativeWindow()->GetBoundsInScreen();
...
void WebContentsImpl::HostInternetExplorerTab(
    std::unique_ptr<dual_engine::InternetExplorerTabHost> ie_host) {
  RenderFrameHostImpl* rfh =
      GetRenderManager()->SetSpeculativeRenderFrameHostForInternetExplorer(
          nullptr /* site_instance */);

  auto* observer = rfh->GetInternetExplorerTabHostObserver();
  ie_host->SetDelegate(observer);
  ie_host->AddObserver(observer);

  GetController().SetInternetExplorerPendingEntryIndex(-1);
  // Set the initial Internet Explorer specific state in RenderFrameHostImpl.
  rfh->SetInitialInternetExplorerState(std::move(ie_host));

  rfh->GetInternetExplorerTabHost()->CommitNewWindow();

  GetRenderManager()->CommitRenderFrameHostForInternetExplorer();
  UpdateInternetExplorerHostedWindow();
}
#endif
...
bool WebContentsImpl::ShouldIgnoreUnresponsiveRenderer() {
  ...
  if (delegate_ && delegate_->ShouldAllowUnresponsiveRendererEvent(this))
    return false;
...
void WebContentsImpl::OnEndDragging() {
  if (delegate_)
    delegate_->OnEndDragging();
}
...
#if defined(OS_WIN)
  has_attached_interstitial_ = true;
  for (auto& observer : internet_explorer_observers_) {
    observer.OnPostInterstitialAttach();
  }
#endif
}
...
#if defined(OS_WIN)
  has_attached_interstitial_ = false;
  for (auto& observer : internet_explorer_observers_) {
    observer.OnPostInterstitialDetach();
  }
#endif
}
...
void WebContentsImpl::GetSelectionMarkup(
    WebContents::GetSelectionMarkupCallback callback) {
  RenderFrameHostImpl* focused_frame = GetFocusedFrame();
  if (!focused_frame) {
    std::move(callback).Run(std::string());
  } else {
    focused_frame->GetFrameInputHandler()->GetSelectionMarkup(
        std::move(callback));
  }
}
...
void WebContentsImpl::PasteHTMLFromFormat(const std::string& format) {
  auto* input_handler = GetFocusedFrameInputHandler();
  if (!input_handler)
    return;

  input_handler->PasteHTMLFromFormat(format);
  for (auto& observer : observers_)
    observer.OnPaste();
}
...
void WebContentsImpl::StartAreaSelect(bool screenshot_mode) {
  auto* frame = ::features::IsAreaSelectionForSubScrollersEnabled()
                    ? GetFocusedFrame()
                    : GetMainFrame();
  ...
  if (!input_handler)
    return;
  input_handler->StartAreaSelect(screenshot_mode);
  RecordAction(base::UserMetricsAction(screenshot_mode
                                           ? "StartAreaSelectScreenshot"
                                           : "StartAreaSelectWebObjects"));
}
...
void WebContentsImpl::OnSavePage(
    content::SavePagePreferences* save_page_prefs) {
#if defined(OS_WIN)
  // For now we don't support saving dual engine pages as they wouldn't be
  // re-opened in dual engine mode.
  if (GetInternetExplorerTabHost()) {
    return;
  }
#endif
  ...
  save_package_->GetSaveInfo(save_page_prefs);
}
...
void WebContentsImpl::SetVisibilityAndNotifyObservers(Visibility visibility) {
  ...
  if (root->speculative_frame_host()) {
    dual_engine::InternetExplorerTabHost* tab_host =
        root->speculative_frame_host()->GetInternetExplorerTabHost();
    if (tab_host) {
      tab_host->OnWebContentsVisibilityChanged(visibility_);
    }
  }
#endif
}
...
WebContentsImpl::GetMojoImageDownloader() {
#if defined(OS_WIN)
  if (IsHostingInternetExplorer()) {
    return GetFrameTree()
        ->root()
        ->render_manager()
        ->GetMojoImageDownloaderForInternetExplorer();
  }
#endif
  return GetMainFrame()->GetMojoImageDownloader();
}
...
void WebContentsImpl::Find(int request_id,
  ...
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    dual_engine::mojom::FindOptionsPtr dual_engine_options =
      dual_engine::BlinkFindOptionsToDualEngineFindOptions(std::move(options));
    tab_host->FindOnPage(request_id, base::UTF16ToUTF8(search_text),
                         std::move(dual_engine_options));
    return;
  }
#endif
...
void WebContentsImpl::StopFinding(StopFindAction action) {
  ...
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->StopFindOnPage(
        static_cast<dual_engine::mojom::StopFindAction>(action));
    return;
  }
#endif
...
void WebContentsImpl::DidStartNavigation(NavigationHandle* navigation_handle) {
  ...
  if (navigation_handle->IsInMainFrame()) {
    // Only clear the favicon urls when the main frame is navigating. Navigation
    // events for other frames may come in after the favicon urls have been
    // loaded, in which case we do not want to clear them.
    favicon_urls_.clear();
  }
...
void WebContentsImpl::UpdateFaviconURL(
  ...
  // Like DidUpdateFaviconURL except sent for all frames, not just main frames.
  NotifyFaviconCandidatesChanged(source, candidates);
#endif // BUILDFLAG(ENABLE_SMARTSCREEN)

  if (!source->GetParent()) {
    // Only cache favicons for main frame.
    favicon_urls_ = std::move(candidates);

    for (auto& observer : observers_) {
      // Only call DidUpdateFaviconURL for main frame.
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
void WebContentsImpl::NotifyFaviconCandidatesChanged(
    RenderFrameHost* render_frame_host,
    const std::vector<blink::mojom::FaviconURLPtr>& candidates) {

  for (auto& observer : observers_) {
    observer.FaviconCandidatesChanged(
        render_frame_host->GetFrameTreeNodeId(), candidates);
  }
}
#endif // BUILDFLAG(ENABLE_SMARTSCREEN)
...
#if defined(OS_WIN)
  if (is_main_frame) {
    if (old_host && old_host->IsHostingInternetExplorer()) {
      old_host->GetInternetExplorerTabHost()->OnEngineSwapping();
    }
    if (new_host->IsHostingInternetExplorer()) {
      new_host->GetInternetExplorerTabHost()->OnEngineSwapping();
    }
    if ((old_host && old_host->IsHostingInternetExplorer()) ||
        new_host->IsHostingInternetExplorer()) {
      for (auto& observer : internet_explorer_observers_) {
        observer.OnEngineSwapped();
      }
    }
    if (old_host && old_host->IsHostingInternetExplorer()) {
      old_host->GetInternetExplorerTabHost()->OnDidEngineSwap();
    }
    if (new_host->IsHostingInternetExplorer()) {
      new_host->GetInternetExplorerTabHost()->OnDidEngineSwap();
    }
  }
#endif
}
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
  void WebContentsImpl::NotifySubframeEntryCommitted(int frame_tree_node_id) {
    for (auto& observer : observers_)
      observer.SubframeEntryCommitted(frame_tree_node_id);
  }
#endif
...
void WebContentsImpl::RunJavaScriptDialog(
  ...
  bool should_suppress_after_firing_page_handler =
      delegate_ && delegate_->ShouldSuppressDialogsButFirePageHandlers(this);

  if (dialog_manager_ && !should_suppress_after_firing_page_handler) {
...
void WebContentsImpl::RunBeforeUnloadConfirm(
  ...
  for (auto& observer : observers_)
    observer.BeforeUnloadDialogShown();
}
...
void WebContentsImpl::UpdateTitleForFrame(RenderFrameHost* render_frame_host,
    const base::string16& title) {
  for (auto& observer : observers_)
    observer.FrameTitleUpdated(render_frame_host->GetFrameTreeNodeId(),
                               title);
}
...
#if defined(OS_WIN)
  // IE tabs are not visible and initialized in the same sense as other web
  // contents, and we want them to show up as a hang.
  if (!IsHostingInternetExplorer()) {
#endif
...
void WebContentsImpl::SaveCurrentAndSetNewUserAgentOverride(
    const std::string& override,
    bool override_in_new_tabs) {
  // We shouldn't be ignoring a past state without restoring that state first.
  if (previous_user_agent_override_saved_)
    return;

  previous_user_agent_override_saved_ = true;
  previous_user_agent_override_ = GetUserAgentOverride();
  previous_should_override_user_agent_in_new_tabs_ =
      should_override_user_agent_in_new_tabs_;
  SetUserAgentOverride(blink::UserAgentOverride::UserAgentOnly(override),
                       override_in_new_tabs);
}

void WebContentsImpl::RestorePreviousUserAgentOverride() {
  if (!previous_user_agent_override_saved_)
    return;

  SetUserAgentOverride(previous_user_agent_override_,
                       previous_should_override_user_agent_in_new_tabs_);
  previous_user_agent_override_saved_ = false;
  previous_user_agent_override_.ua_string_override.clear();
}

bool WebContentsImpl::HasSavedUserAgentOverride() const {
  return previous_user_agent_override_saved_;
}

#if defined(OS_WIN)
dual_engine::InternetExplorerTabHost*
WebContentsImpl::GetInternetExplorerTabHost() const {
  RenderFrameHostImpl* rfh = frame_tree_.root()->current_frame_host();
  return rfh->GetInternetExplorerTabHost();
}
#endif  // defined(OS_WIN)
...
bool WebContentsImpl::ShouldShowBlankPageForFailedNavigation() {
  return (delegate_ && delegate_->ShouldShowBlankPageForFailedNavigation(this));
}
...
#if defined(OS_WIN)
void WebContentsImpl::InitializeDualEngineForTesting(
    dual_engine::CreateIEServiceFunction create_function) {
  auto* session_host =
      BrowserContext::GetInternetExplorerSessionHost(GetBrowserContext());
  if (session_host) {
    session_host->InitializeDualEngineForTesting(std::move(create_function));
  }
}

void WebContentsImpl::PopulateInternetExplorerTestInformation(
    dual_engine::InternetExplorerTestInformation& test_information) {
  GetInternetExplorerTabHost()->PopulateTestInformation(test_information);
}

// This method is only called on WebContent instances hosted in a tab
void WebContentsImpl::EnableInternetExplorerIntegration() {
  internet_explorer_integration_enabled_ = true;

  // If the --ie-mode-force command line was specified, force any
  // WebContentsImpls capable of hosting IE to do so. This is intended for
  // testing scenarios such as WebDriver.
  if (dual_engine::IsInternetExplorerModeForceEnabled()) {
    SetDualEngineForcedEngine(dual_engine::ForcedEngine::kInternetExplorer);
  }
}

void WebContentsImpl::SetDualEngineForcedEngine(
    dual_engine::ForcedEngine engine) {
  CHECK(IsInternetExplorerIntegrationEnabled());
  CHECK(forced_engine_ == dual_engine::ForcedEngine::kNone ||
        engine == forced_engine_);

  if (engine != forced_engine_) {
    forced_engine_ = engine;

    base::PostTask(
        FROM_HERE, {BrowserThread::UI},
        base::BindOnce(
            &WebContentsImpl::OnInternetExplorerHostingForced,
            weak_factory_.GetWeakPtr()));
  }
}

bool WebContentsImpl::IsHostingInternetExplorer() {
  return GetMainFrame()->IsHostingInternetExplorer();
}

bool WebContentsImpl::IsInternetExplorerIntegrationEnabled() {
  return internet_explorer_integration_enabled_;
}

dual_engine::ForcedEngine WebContentsImpl::GetDualEngineForcedEngine() {
  return forced_engine_;
}

void WebContentsImpl::OnInternetExplorerHostingForced() {
  if (delegate_) {
    delegate_->OnInternetExplorerHostingForced(this);
  }
}

bool WebContentsImpl::HasAttachedInterstitialPage() {
  return has_attached_interstitial_;
}

bool WebContentsImpl::IsInternetExplorerWindowVisible() {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  return tab_host && tab_host->IsVisible();
}

bool WebContentsImpl::IsInternetExplorerWindowUnderCursor() {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  return tab_host && tab_host->IsInternetExplorerWindowUnderCursor();
}

void WebContentsImpl::ForceCloseInternetExplorer() {
  GetInternetExplorerTabHost()->ForceClose();
}

gfx::NativeView WebContentsImpl::GetInternetExplorerNativeView() {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (!tab_host)
    return nullptr;
  return tab_host->GetNativeView();
}

void WebContentsImpl::UpdateInternetExplorerHostedWindow() {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->UpdateHostedWindow();
  }
}

class DualEngineDeferredWindowHiderImpl
    : public content::DualEngineDeferredWindowHider {
 public:
  DualEngineDeferredWindowHiderImpl(base::WeakPtr<WebContentsImpl> web_contents)
      : web_contents_(web_contents) {}

  void HideWindow() override {
    if (web_contents_) {
      web_contents_->InternetExplorerFinishDeferredWindowHide(false);
  ...
  void CancelHide() override {
    if (web_contents_) {
      web_contents_->InternetExplorerFinishDeferredWindowHide(true);
      web_contents_ = nullptr;
    }
  }

 private:
  base::WeakPtr<WebContentsImpl> web_contents_;
};

void WebContentsImpl::InternetExplorerFinishDeferredWindowHide(bool cancel) {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->FinishDeferredBrowserWindowHide(cancel);
  }
}

std::unique_ptr<content::DualEngineDeferredWindowHider>
WebContentsImpl::CreateInternetExplorerDeferredWindowHider() {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (!tab_host) {
    return nullptr;
  }
  tab_host->DeferHideWindow();
  return std::make_unique<content::DualEngineDeferredWindowHiderImpl>(
      weak_factory_.GetWeakPtr());
}

// Switch to Internet Explorer engine can be initiated in two cases:
// 1. Due to navigation to a new url: recover_existing_region will be false.
// 2. Due to a history navigation: recover_existing_region will be true. We need
// to get the entry_id of the entry being navigated to and the GUID of the
// region in which the entry is present. Send this information to IE so that it
// can recover a previous tab.
void WebContentsImpl::InitiateSwitchToInternetExplorer(
    SiteInstance* site_instance,
    const GURL& initial_nav_url,
    int pending_nav_entry_id,
    bool recover_existing_region) {
  RenderFrameHostImpl* rfh =
      GetRenderManager()->SetSpeculativeRenderFrameHostForInternetExplorer(
          site_instance);

  dual_engine::RenderFrameHostInternetExplorerTabHostObserver* observer =
      rfh->GetInternetExplorerTabHostObserver();

  int pending_entry_index =
      GetController().GetEntryIndexWithUniqueID(pending_nav_entry_id);

  // Find the count of entries before and after the pending IE region. This
  // has to be sent to IE during initialization to make history navigations
  // from IE to Anaheim work.
  uint32_t entry_count_before_region = 0;
  uint32_t entry_count_after_region = 0;
  GetController().FindEntryCountBeforeAndAfterRegion(pending_entry_index,
                                                     &entry_count_before_region,
                                                     &entry_count_after_region);
  dual_engine::mojom::DualEngineNavEntryCountInfoPtr nav_entry_count_info =
      dual_engine::mojom::DualEngineNavEntryCountInfo::New(
          entry_count_before_region, entry_count_after_region);

  dual_engine::mojom::RecoveryInfoPtr recovery_info = nullptr;
  if (recover_existing_region) {
    // We are recovering an existing region. Get the GUID of the region and the
    // entry_id of the entry to navigate to.
    //
    // If we are unable to get the GUID for the region, treat this like a new
    // navigation and send the recovery_info as null. This will at least load
    // the requested url in IE.
    // This can happen in recovery scenarios where we were unable to recover the
    // GUID for this region.
    NavigationEntryImpl* pending_entry =
        GetController().GetEntryAtIndex(pending_entry_index);
    if (pending_entry) {
      std::string region_guid = GetController().GetInternetExplorerRegionGUID(
          pending_entry->GetInternetExplorerRegionId());
      if (!region_guid.empty()) {
        recovery_info = dual_engine::mojom::RecoveryInfo::New(
            region_guid, pending_entry->GetInternetExplorerEntryId());
      }
    }
  }

  std::unique_ptr<dual_engine::InternetExplorerTabHost> tab_host =
      StartInternetExplorerEngine(
          observer, initial_nav_url, std::move(recovery_info),
          std::move(nav_entry_count_info), pending_nav_entry_id);

  // Set the index of the entry we are navigating to. If we are navigating to a
  // a new IE region, this will be -1.
  GetController().SetInternetExplorerPendingEntryIndex(pending_entry_index);
  tab_host->EnableCaretMode(renderer_preferences_.caret_browsing_enabled);
  // Set the initial Internet Explorer specific state in RenderFrameHostImpl.
  rfh->SetInitialInternetExplorerState(std::move(tab_host));
}

void WebContentsImpl::AddInternetExplorerObserver(
    content::WebContentsDualEngineBrowserObserver* observer) {
  internet_explorer_observers_.AddObserver(observer);
}

void WebContentsImpl::RemoveInternetExplorerObserver(
    content::WebContentsDualEngineBrowserObserver* observer) {
  internet_explorer_observers_.RemoveObserver(observer);
}

void WebContentsImpl::CommitInternetExplorerBrowser(
    RenderFrameHostImpl* rfh,
    dual_engine::mojom::DualEngineNavigationStatePtr nav_state,
    dual_engine::mojom::DualEngineSSLInfoPtr ssl_info,
    int pending_nav_entry_id) {
  // Commit the speculative RenderFrameHost:
  if (GetRenderManager()->speculative_frame_host() == rfh) {
    GetRenderManager()->CommitRenderFrameHostForInternetExplorer();
  }

  SetIsCrashed(base::TERMINATION_STATUS_STILL_RUNNING, 0);
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerBrowserCommitted();
  }

  if (showing_context_menu_) {
    // Don't show Edge context menu over IE tab content.
    render_view_host_delegate_view_->HideContextMenu();
  }

  DCHECK(rfh->IsCurrent());
  InternetExplorerDidNavigate(rfh, std::move(nav_state), std::move(ssl_info),
                              pending_nav_entry_id);
}

void WebContentsImpl::OnInternetExplorerCriticalError(
    dual_engine::InternetExplorerCriticalError error) {
  if (base::FeatureList::IsEnabled(features::edge::kNeedIEFallbackForIEMode) &&
      error != dual_engine::InternetExplorerCriticalError::
                   INTERNET_EXPLORER_EXE_IS_MISSING_MESSAGE) {
    // As a fallback, we will launch IE standalone.
    NavigationRequest* navigation_request =
        frame_tree_.root()->navigation_request();
    if (navigation_request && navigation_request->GetURL().is_valid()) {
      LaunchInInternetExplorer(navigation_request->GetURL());
    }
  }

  CleanupInternetExplorerFailure();
  if (delegate_) {
    delegate_->OnInternetExplorerCriticalError(this, error);
  }
}

void WebContentsImpl::OnInternetExplorerConnectionLost() {
  ResetLoadProgressState();
  GetMainFrame()->OnInternetExplorerLoadingActivityChanged(false);
  SetIsCrashed(base::TERMINATION_STATUS_PROCESS_CRASHED, 0);

  CleanupInternetExplorerFailure();
  for (auto& observer : internet_explorer_observers_)
    observer.InternetExplorerConnectionLost(GetCrashedStatus());
}

void WebContentsImpl::CleanupInternetExplorerFailure() {
  // Clear pending async switch state
...
void WebContentsImpl::OnInternetExplorerGotFocus() {
  NotifyWebContentsFocused(nullptr);
}

void WebContentsImpl::OnInternetExplorerLostFocus() {
  NotifyWebContentsLostFocus(nullptr);
}

void WebContentsImpl::FocusInternetExplorerWindowThroughTabTraversal(
    bool reverse) {
  GetInternetExplorerTabHost()->FocusThroughTabTraversal(reverse);
}

void WebContentsImpl::ContinueDualEngineNavigationInEdge(const GURL& url) {
  if (!url.is_valid()) {
    LOG(WARNING) << __func__ << " Invalid URL: " << url.possibly_invalid_spec();
    dual_engine::RecordContinueDualEngineNavigationInEdgeResult(
        dual_engine::ContinueDualEngineNavigationInEdgeResult::
            kRejectInvalidURL);
  } else if (!dual_engine::IsAllowedCrossEngineURLScheme(url)) {
    LOG(WARNING) << __func__ << " Unexpected scheme: " << url;
    dual_engine::RecordContinueDualEngineNavigationInEdgeResult(
        dual_engine::ContinueDualEngineNavigationInEdgeResult::
            kRejectDisallowedScheme);
  } else if (GetDualEngineForcedEngine() ==
             dual_engine::ForcedEngine::kInternetExplorer) {
    LOG(WARNING) << __func__ << " Unexpected in forced mode: " << url;
    dual_engine::RecordContinueDualEngineNavigationInEdgeResult(
        dual_engine::ContinueDualEngineNavigationInEdgeResult::
            kRejectForcedMode);
  } else {
    // Accept the delegated navigation. DualEngineNavigationThrottle might still
    // reject it based on policy settings or EMIE list lookup.
    dual_engine::RecordContinueDualEngineNavigationInEdgeResult(
        dual_engine::ContinueDualEngineNavigationInEdgeResult::kAccept);
    content::NavigationController::LoadURLParams params(url);
    params.is_renderer_initiated = true;
    params.internet_explorer_initiated = true;
    params.initiator_origin = url::Origin();
    GetController().LoadURLWithParams(params);

    ReceivingNavigationRequestFromInternetExplorer(url, false);
  }
}

void WebContentsImpl::ReceivingNavigationRequestFromInternetExplorer(
    const GURL& url,
    bool new_window) {
  for (auto& observer : internet_explorer_observers_) {
    observer.ReceivingNavigationRequestFromInternetExplorer(url, new_window);
  }
}

void WebContentsImpl::InternetExplorerDidNavigate(
    RenderFrameHostImpl* rfh,
    dual_engine::mojom::DualEngineNavigationStatePtr nav_state,
    dual_engine::mojom::DualEngineSSLInfoPtr ssl_info,
    int pending_nav_entry_id) {
  uint32_t nav_flags = nav_state->nav_flags;
  std::unique_ptr<NavigationRequest> navigation_request =
      GetMainFrame()->TakeNavigationRequestForIE();
  GetController().InternetExplorerDidNavigate(
      rfh, std::move(nav_state), std::move(ssl_info), navigation_request.get());

  if (navigation_request) {
    navigation_request->LogIENavigationTimeTelemetry();
    navigation_request->ResetForInternetExplorerNavigation(
        net::ERR_ABORTED, pending_nav_entry_id);
  ...
  for (auto& observer : internet_explorer_observers_) {
    observer.DidStartInternetExplorerNavigation(
        nav_flags & static_cast<uint32_t>(
                        dual_engine::mojom::NavigationFlags::kMainFrame),
        nav_flags & static_cast<uint32_t>(
                        dual_engine::mojom::NavigationFlags::kSameDocument),
        nav_flags & static_cast<uint32_t>(
                        dual_engine::mojom::NavigationFlags::kErrorPage));
    observer.DidFinishInternetExplorerNavigation(
        nav_flags & static_cast<uint32_t>(
                        dual_engine::mojom::NavigationFlags::kMainFrame),
        nav_flags & static_cast<uint32_t>(
                        dual_engine::mojom::NavigationFlags::kSameDocument),
        nav_flags & static_cast<uint32_t>(
                        dual_engine::mojom::NavigationFlags::kErrorPage));
    if (entry) {
      observer.InternetExplorerCommittedUrl(entry->GetURL());
    }
  }
}

void WebContentsImpl::NavigateWithInternetExplorerEngine(
    NavigationHandle* navigation_handle,
    ExitRegionCallback callback) {
  NavigationRequest* request = NavigationRequest::From(navigation_handle);

  NavigationEntryImpl* pending_entry = nullptr;
  int pending_nav_entry_id = request->nav_entry_id();
  // A synchronous, renderer-initiated navigation will not have a
  // NavigationEntry associated with it, and pending_nav_entry_id will return 0.
  if (pending_nav_entry_id) {
    pending_entry = GetController().GetEntryWithUniqueID(pending_nav_entry_id);
  }

  if (!pending_entry) {
    // If there is no entry corresponding to this navigation in
    // NavigatonControllerImpl's entry list, this is a new navigation.
    if (IsHostingInternetExplorer()) {
      // We are already hosting an IE tab. Ask it to navigate.
      GetMainFrame()->NavigateToNewPageWithInternetExplorerEngine(
          request->GetURL(), pending_nav_entry_id);
    } else {
      // We are not hosting an IE tab. Start a switch to IE.
      InitiateSwitchToInternetExplorer(nullptr /* site_instance */,
                                       request->GetURL(), pending_nav_entry_id,
                                       false /* recover_existing_region */);
    }
  } else {
    // If there is an entry corresponding to this navigation in
    // NavigatonControllerImpl's entry list, we should check the type of
    // navigation.
    dual_engine::InternetExplorerNavigationType navigation_type =
        GetController().ClassifyInternetExplorerNavigation(pending_entry);

    switch (navigation_type) {
      case dual_engine::IE_NAVIGATION_TYPE_RELOAD:
        if (IsHostingInternetExplorer()) {
          // Send reload to internet explorer engine.
          GetMainFrame()->ReloadInInternetExplorerEngine(
              pending_nav_entry_id, navigation_handle->GetReloadType());
        } else {
          // We are not hosting an IE tab. This can happen if we were hosting an
          // IE tab but then IE crashed. Initiate a switch to IE and ask it to
          // navigate to the pending entry's entry_id.
          InitiateSwitchToInternetExplorer(
              request->dest_site_instance(), request->GetURL(),
              pending_nav_entry_id, true /* recover_existing_region */);
        }
        break;
      case dual_engine::IE_NAVIGATION_TYPE_RESTORE:
        // We are restoring an entry. Initiate a switch to IE and ask it to
        // navigate to the pending entry's entry_id.
        InitiateSwitchToInternetExplorer(
            nullptr /* site_instance */, request->GetURL(),
            pending_nav_entry_id, true /* recover_existing_region */);
        break;
      case dual_engine::IE_NAVIGATION_TYPE_HISTORY_CURRENT_REGION:
        // This is a history navigation in the current region.
        if (IsHostingInternetExplorer()) {
          // We are currently hosting an IE tab. Ask it to navigate to the
          // target entry_id.
          GetMainFrame()->GoToEntryInInternetExplorerEngine(
              pending_entry->GetInternetExplorerEntryId(),
              pending_nav_entry_id);
        } else {
          // We are not hosting an IE tab. This can happen if we were hosting an
          // IE tab but then IE crashed. Initiate a switch to IE and ask it to
        ...
        break;
      case dual_engine::IE_NAVIGATION_TYPE_HISTORY_DIFFERENT_REGION:
        // This is a history navigation in a different region.
        if (IsHostingInternetExplorer()) {
          // We are currently hosting an IE tab. Ask it to prepare to exit and
          // pass the callback provided by the throttle. When it is ready to
          // the throttle will call us back to resume the switch.
          GetMainFrame()->PrepareInternetExplorerToExitRegion(
              pending_nav_entry_id, std::move(callback));
        } else {
          // We are not hosting an IE tab. Initiate a switch to IE and ask it to
          // navigate to the pending entry's entry_id.
          InitiateSwitchToInternetExplorer(
              request->dest_site_instance(), request->GetURL(),
              pending_nav_entry_id, true /* recover_existing_region */);
        ...
        break;
      case dual_engine::IE_NAVIGATION_TYPE_HISTORY_EDGE_ENTRY:
        // This is a history navigation to an entry that was previously an Edge
        // entry.
        if (IsHostingInternetExplorer()) {
          // We are currently hosting an IE tab. Ask it to prepare to exit and
          // pass the callback provided by the throttle. When it is ready to
          ...
          // We are not hosting an IE tab. Initiate a switch to IE and ask it to
          // navigate to entry's url. We should replace the Edge entry with the
          // new IE entry and truncate all entries to it's right.
          InitiateSwitchToInternetExplorer(
              nullptr /* site_instance */, request->GetURL(),
              pending_nav_entry_id, false /* recover_existing_region */);
        }
        break;
      default:
        break;
  ...
  for (auto& observer : internet_explorer_observers_) {
    observer.SendingNavigationToInternetExplorer(request->GetURL());
  }
}

void WebContentsImpl::LaunchInInternetExplorer(GURL url) {
  base::FilePath ie_path;
  base::PathService::Get(base::DIR_PROGRAM_FILES, &ie_path);
  ie_path =
      ie_path.AppendASCII("Internet Explorer").AppendASCII("iexplore.exe");

  base::CommandLine command_line(ie_path);
  command_line.AppendArg(url.spec());
  if (GetBrowserContext()->IsOffTheRecord()) {
    command_line.AppendSwitch("-private");
  }
...
void WebContentsImpl::ResumeSwitchToInternetExplorerEngine(
    NavigationHandle* navigation_handle) {
  // The current Internet Explorer tab is now ready to exit.
  NavigationRequest* request = NavigationRequest::From(navigation_handle);

  int pending_nav_entry_id = request->nav_entry_id();
  NavigationEntryImpl* pending_entry =
      GetController().GetEntryWithUniqueID(pending_nav_entry_id);

  if (pending_entry) {
    bool recover_existing_region = false;
    SiteInstance* site_instance = nullptr;
    // There are two cases for which we can be called:
    // 1. When we are switching from the current Internet Explorer region to
    //    another existing Internet Explorer region. In this case the pending
    //    entry is an Internet Explorer entry.
    // 2. When we are switching from the current Internet Explorer region to a
    //    new region (dual_engine::IE_NAVIGATION_TYPE_HISTORY_EDGE_ENTRY case).
    //    In this case the pending entry is not an Internet Explorer entry.
    if (pending_entry->is_internet_explorer_entry()) {
      // We are recovering an existing region.
      recover_existing_region = true;
      // Use the site instance of the Internet Explorer region we are going to.
      site_instance = request->dest_site_instance();
    }
    InitiateSwitchToInternetExplorer(site_instance, request->GetURL(),
                                     pending_nav_entry_id,
                                     recover_existing_region);
  }
}

void WebContentsImpl::SyncDualEngineSwitchingData() {
  BrowserContext* browser_context = GetBrowserContext();
  auto* session_host =
      BrowserContext::GetInternetExplorerSessionHost(browser_context);
  if (session_host) {
    session_host->LazyParseEmieSitelist(
        content::GetContentClient()->browser()->GetCachedEmieXmlPath(
            browser_context));
  }
}

void WebContentsImpl::SetMediaFullscreenModeToInternetExplorer(
    bool is_fullscreen) {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->SetMediaFullscreen(is_fullscreen);
  }
}

void WebContentsImpl::SetBrowserFullscreenModeToInternetExplorer(
    bool is_fullscreen) {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->SetBrowserFullscreen(is_fullscreen);
  }
}

std::unique_ptr<dual_engine::InternetExplorerTabHost>
WebContentsImpl::StartInternetExplorerEngine(
    dual_engine::RenderFrameHostInternetExplorerTabHostObserver* observer,
    const GURL& initial_nav_url,
    dual_engine::mojom::RecoveryInfoPtr recovery_info,
    dual_engine::mojom::DualEngineNavEntryCountInfoPtr nav_entry_count_info,
    int pending_nav_entry_id) {
  std::unique_ptr<dual_engine::NavigationStartTimeState> nav_start_time_state =
      CreateNavigationStartTimeStateForInitialNavigation(
          BrowserContext::HasInternetExplorerSession(GetBrowserContext()),
          !recovery_info.is_null());
  auto* session_host =
      BrowserContext::GetInternetExplorerSessionHost(GetBrowserContext());
  if (session_host) {
    bool is_popup = delegate_->GetDisplayMode(this) ==
        blink::mojom::DisplayMode::kStandalone;

    gfx::AcceleratedWidget host_window = GetTopLevelNativeWindow()
                                             ->GetRootWindow()
                                             ->GetHost()
                                             ->GetAcceleratedWidget();

    std::unique_ptr<dual_engine::InternetExplorerTabHost> tab_host =
        session_host->CreateInternetExplorerTab(
            observer, initial_nav_url,
            forced_engine_ == dual_engine::ForcedEngine::kInternetExplorer,
            is_popup, host_window, std::move(recovery_info),
            std::move(nav_entry_count_info), std::move(nav_start_time_state),
            pending_nav_entry_id);

    // Listen to the tab host for IE mode changes.
    tab_host->AddObserver(observer);
    tab_host->OnWebContentsVisibilityChanged(visibility_);
...
std::unique_ptr<dual_engine::NavigationStartTimeState>
WebContentsImpl::CreateNavigationStartTimeStateForInitialNavigation(
    bool has_session, bool has_recovery_data) {
  dual_engine::InternetExplorerNavigationTypeFlag nav_type_flag =
      dual_engine::InternetExplorerNavigationTypeFlag::kNone;
  if (has_recovery_data) {
    if (has_session)
      nav_type_flag =
          dual_engine::InternetExplorerNavigationTypeFlag
              ::kInitialNavForRecoveryHasSession;
    else
      nav_type_flag =
          dual_engine::InternetExplorerNavigationTypeFlag
              ::kInitialNavForRecoveryNoSession;
  } else {
    if (has_session)
      nav_type_flag =
          dual_engine::InternetExplorerNavigationTypeFlag
              ::kInitialNavHasSession;
    else
      nav_type_flag =
          dual_engine::InternetExplorerNavigationTypeFlag
              ::kInitialNavNoSession;
  }
  std::unique_ptr<dual_engine::NavigationStartTimeState> nav_start_time_state =
      std::make_unique<dual_engine::NavigationStartTimeState>(
          base::TimeTicks::Now(), nav_type_flag);
  return nav_start_time_state;
}

void WebContentsImpl::SimulateRawKeyEventToInternetExplorer(
    const NativeWebKeyboardEvent& event) {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->SimulateKeyboardEventFromWebContents(event);
  }
}

void WebContentsImpl::SimulateMouseEventToInternetExplorer(
    const blink::WebMouseEvent& event) {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->SimulateMouseEventFromWebContents(event);
  }
}

void WebContentsImpl::InternetExplorerNavigateWithoutEntry(
    const GURL& url) const {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->Navigate(url, 0 /* pending_nav_entry_id */,
                       false /* skip_on_before_unload */);
  }
}

bool WebContentsImpl::InternetExplorerGoBack() const {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->ExecNavigationCommand(
        dual_engine::mojom::NavigationCommand::kGoBack,
...
bool WebContentsImpl::InternetExplorerGoForward() const {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->ExecNavigationCommand(
        dual_engine::mojom::NavigationCommand::kGoForward,
        0 /* pending_nav_entry_id */);
    return true;
  }
  return false;
...
void WebContentsImpl::InternetExplorerNotifyNavigationEntriesDeleted(
    bool should_delete_entries,
    uint32_t entry_count_before_region,
    uint32_t entry_count_after_region) const {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->NotifyNavigationEntriesDeleted(
        should_delete_entries,
        dual_engine::mojom::DualEngineNavEntryCountInfo::New(
            entry_count_before_region, entry_count_after_region));
  }
}

void WebContentsImpl::InternetExplorerCloneActiveRegionRecoveryData(
    std::string src_guid,
    std::string dest_guid) {
  auto* session_host =
      BrowserContext::GetInternetExplorerSessionHost(GetBrowserContext());
  if (session_host) {
    dual_engine::InternetExplorerTabHost* tab_host =
        GetInternetExplorerTabHost();
    // Get the browser window hwnd of the current Internet Explorer tab. The
    // Internet Explorer session will copy the tab recovery data of the tab
    // associated with this hwnd to a new file with GUID dest_guid.
    if (tab_host) {
      session_host->CloneActiveTabRecoveryData(tab_host->GetBrowserWindow(),
                                               src_guid, dest_guid);
    }
  }
}

void WebContentsImpl::InternetExplorerCloneInactiveRegionRecoveryData(
    std::string src_guid,
    std::string dest_guid) {
  auto* session_host =
      BrowserContext::GetInternetExplorerSessionHost(GetBrowserContext());
  if (session_host) {
    session_host->CloneInactiveTabRecoveryData(src_guid, dest_guid);
  }
}

void WebContentsImpl::InternetExplorerDeleteActiveRegionRecoveryData(
    std::string tab_guid) {
  // Ask the Internet Explorer host to delete the recovery data.
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->DeleteActiveRegionRecoveryData(tab_guid);
  }
}

void WebContentsImpl::InternetExplorerDeleteInactiveRegionRecoveryData(
    std::string tab_guid) {
  // We might not be hosting an Internet Explorer tab currently. So, we need to
  // ask the Internet Explorer session to delete the recovery data.
  auto* session_host =
      BrowserContext::GetInternetExplorerSessionHost(GetBrowserContext());
  if (session_host) {
    session_host->DeleteInactiveRegionRecoveryData(tab_guid);
  }
}

void WebContentsImpl::NotifyInternetExplorerRegionUpdated(
    const int region_id,
    const std::string& guid) {
  // Post a task to call the observers. This has to be done asynchronously
  // because of the case when we clone a tab. This method is called for each
  // region that is cloned to notify the observers. However, the observers have
  // not initialized by this time. That will happen in the same call stack, so
  // it is safe to call them once the stack unwinds.
  base::PostTask(
      FROM_HERE, {BrowserThread::UI},
      base::BindOnce(&WebContentsImpl::OnNotifyInternetExplorerRegionUpdated,
                     weak_factory_.GetWeakPtr(), region_id, guid));
}

void WebContentsImpl::OnNotifyInternetExplorerRegionUpdated(
    const int region_id,
    const std::string& guid) {
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerRegionUpdated(region_id, guid);
  }
}

void WebContentsImpl::NotifyInternetExplorerRegionDeleted(const int region_id) {
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerRegionDeleted(region_id);
  }
}

void WebContentsImpl::NotifyInternetExplorerEntryCommitted(const int index) {
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerEntryCommitted(index);
  }
}

void WebContentsImpl::NotifyInternetExplorerEntryInserted(const int index) {
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerEntryInserted(index);
  }
}

void WebContentsImpl::NotifyInternetExplorerEntryChanged(const int index) {
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerEntryChanged(index);
  }
}

void WebContentsImpl::NotifyInternetExplorerEntryDeleted(const int index) {
  for (auto& observer : internet_explorer_observers_) {
    observer.InternetExplorerEntryDeleted(index);
  }
}

void WebContentsImpl::OnInternetExplorerUnhandledKeyboardAccelerator(
    const ui::KeyEvent& key_event) {
  if (GetDelegate()) {
    content::NativeWebKeyboardEvent web_platform_event(key_event);
    GetDelegate()->HandleKeyboardEvent(this, web_platform_event);
  }
}

void WebContentsImpl::OnInternetExplorerRequestHostFocus(
    bool reverse,
    /* opt */ const ui::KeyEvent* key_event) {
  if (key_event && GetInternetExplorerNativeView()) {
    // If an accelerator (ex. F6) is consumed as part of this message, give the
    // accelerator handling a chance to move focus.
    aura::client::FocusClient* client =
        aura::client::GetFocusClient(GetInternetExplorerNativeView());
    if (client) {
      aura::Window* focusedWindow = client->GetFocusedWindow();
      OnInternetExplorerUnhandledKeyboardAccelerator(*key_event);
      // If the accelerator handling moved focus, we're done.
      if (focusedWindow != client->GetFocusedWindow()) {
        return;
      }
    }
  }
  // Fallback to the tab cycling behavior.
  GetDelegateView()->TakeFocus(reverse);
}

void WebContentsImpl::OnInternetExplorerTitleUpdate(
    uint32_t current_entry_id,
    const base::string16& title) {
  NavigationEntryImpl* current_entry = GetController().GetLastCommittedEntry();
  if (current_entry) {
    if (current_entry_id != ULONG_MAX) {
      // Update the title only if the last committed entry matches the current
      // entry in IE for which this update was sent.
      if (current_entry->is_internet_explorer_entry() &&
          current_entry->GetInternetExplorerEntryId() == current_entry_id) {
        UpdateTitleForEntry(current_entry, title);
      }
    } else {
      // This case is to support the non-development version of the API which
      // calls here with the value for current_entry_id as ULONG_MAX.
      UpdateTitleForEntry(current_entry, title);
    }
  }
}

void WebContentsImpl::DualEngineNavigateToOffsetInEdge(int offset) {
  GetController().DualEngineNavigateToOffsetInEdge(offset);
}

bool WebContentsImpl::InternetExplorerAllowWindowManipulation() {
  if (GetDelegate()) {
    return GetDelegate()->ShouldInternetExplorerAllowWindowManipulation();
  }
  return false;
}

void WebContentsImpl::OnInternetExplorerRequestHostBrowserFullscreen() {
  delegate_->OnInternetExplorerRequestHostBrowserFullscreen(this);
}

void WebContentsImpl::UpdateDualEngineSecureLockIcon(
    dual_engine::mojom::DualEngineSSLInfoPtr ssl_info) {
  GetController().UpdateDualEngineSecureLockIcon(std::move(ssl_info));
}

BrowserContext* WebContentsImpl::InternetExplorerGetBrowserContext() {
  return GetBrowserContext();
}

void WebContentsImpl::OnInternetExplorerAddFavorite(const base::string16& title,
                                                    const GURL& url) {
  delegate_->OnInternetExplorerAddFavorite(title, url, this);
}

void WebContentsImpl::OnInternetExplorerCreateUrlTip(const GURL& url) {
  delegate_->OnInternetExplorerCreateUrlTip(url, this);
}

void WebContentsImpl::OnInternetExplorerHideUrlTip() {
  delegate_->OnInternetExplorerHideUrlTip(this);
}

void WebContentsImpl::OnInternetExplorerAttentionStateChanged(
    bool needs_attention) {
  delegate_->OnInternetExplorerAttentionStateChanged(this, needs_attention);
}

bool WebContentsImpl::OnInternerExplorerPrint() {
  dual_engine::InternetExplorerTabHost* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    tab_host->Print();
    return true;
...
void WebContentsImpl::InternetExplorerDocumentComplete(
    RenderFrameHostImpl* rfh,
    uint32_t current_entry_id,
    std::vector<dual_engine::mojom::DualEngineVisibleListEntryPtr>
        visible_list_entries) {
  GetController().InternetExplorerDocumentComplete(
      rfh, current_entry_id, std::move(visible_list_entries));

  for (auto& observer : internet_explorer_observers_) {
    observer.DocumentOnLoadCompletedInInternetExplorerMainFrame();
  }
}

void WebContentsImpl::OnInternetExplorerFaviconUpdate(
    dual_engine::mojom::DualEngineFaviconInfoPtr favicon_info) {
  // Discard the update if current_enrty_id or current_page_url doesn't
  // match with the last committed entry. Otherwise update the favicon by
  // sending the candidates to UpdateFaviconURL
  content::NavigationEntryImpl* entry = GetController().GetLastCommittedEntry();
  if (entry && entry->is_internet_explorer_entry() &&
      entry->GetInternetExplorerEntryId() == favicon_info->current_entry_id &&
      entry->GetURL() == favicon_info->current_page_url) {
    std::vector<blink::mojom::FaviconURLPtr> faviconCandidates;
    for (size_t i = 0; i < favicon_info->favicon_urls.size(); i++) {
      faviconCandidates.push_back(blink::mojom::FaviconURL::New(
          favicon_info->favicon_urls[i],
          blink::mojom::FaviconIconType::kFavicon, std::vector<gfx::Size>()));
    }
    UpdateFaviconURL(GetMainFrame(), std::move(faviconCandidates));
  }
}

void WebContentsImpl::OnInternetExplorerPopupBlocked(const GURL& contextUrl,
                                                     const GURL& popupUrl) {
  delegate_->OnInternetExplorerPopupBlocked(this, contextUrl, popupUrl);
}

void WebContentsImpl::OnInternetExplorerGeolocationFeatureInUse(
    const GURL& context_url,
    dual_engine::SitePermissionFeatureMode feature_mode) {
  delegate_->OnInternetExplorerGeolocationFeatureInUse(this, context_url,
                                                       feature_mode);
}

void WebContentsImpl::OnInternetExplorerZoomChanged(uint32_t zoom_percentage) {
  NavigationEntry* entry = GetController().GetLastCommittedEntry();
  if (!entry)
    return;

  double old_zoom_level = HostZoomMap::GetZoomLevel(this);
  double new_zoom_level =
      blink::PageZoomFactorToZoomLevel(zoom_percentage / 100.0);

  // IE reports actual zoom instead of logical zoom, so zoom changed events are
  // fired even if the logical zoom hasn't change (i.e. zoom is 100% but the
  // scale factor changed to 1.5 for high dpi so actual zoom is 150%).
  // Filter out no-ops so that we don't show zoom ui when nothing has actually
  // changed.
  if (!blink::PageZoomValuesEqual(old_zoom_level, new_zoom_level)) {
    GURL url = HostZoomMap::GetURLFromEntry(entry);

    HostZoomMap::GetForWebContents(this)->OnInternetExplorerZoomChanged(
        net::GetHostOrSpecFromURL(url), new_zoom_level);
  }
}

void WebContentsImpl::FindOnPageResultCoalescer(int correlation) {
  if (has_pending_find_on_page_result_ &&
      correlation == pending_find_correlation_) {
    has_pending_find_on_page_result_ = false;
    gfx::Rect dummy_rect;
    NotifyFindReply(pending_find_on_page_result_.request_id,
                    pending_find_on_page_result_.match_count,
                    dummy_rect, pending_find_on_page_result_.active_match,
                    true /* final_update */);
  }
}

// IE sends multiple find results for a single keypress. IE does not send the
// |final_update| flag, so the last result is determined using a delayed task.
// When |final_update| is true, an attached screen reader will read the result,
// so it needs to be set appropriately to avoid unnecessary announcements.
void WebContentsImpl::OnInternetExplorerFindOnPageResult(int request_id,
                                                         int match_count,
                                                         int active_match) {
  if (has_pending_find_on_page_result_) {
    has_pending_find_on_page_result_ = false;
    ...
    NotifyFindReply(pending_find_on_page_result_.request_id,
                    pending_find_on_page_result_.match_count,
                    dummy_rect,
                    pending_find_on_page_result_.active_match,
                    false /* final_update */);
  }

  has_pending_find_on_page_result_ = true;
  pending_find_correlation_++;
  pending_find_on_page_result_.request_id = request_id;
  pending_find_on_page_result_.match_count = match_count;
  pending_find_on_page_result_.active_match = active_match;

  base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&WebContentsImpl::FindOnPageResultCoalescer,
          weak_factory_.GetWeakPtr(), pending_find_correlation_),
      base::TimeDelta::FromMilliseconds(500));
}

void WebContentsImpl::InternetExplorerAllButCurrentEntryDeleted(
    std::vector<dual_engine::mojom::DualEngineVisibleListEntryPtr>
        visible_list_entries) {
  GetController().InternetExplorerAllButCurrentEntryDeleted(
      std::move(visible_list_entries));
}

void WebContentsImpl::OnInternetExplorerInputAttempted() {
  for (auto& observer : internet_explorer_observers_) {
    observer.OnInternetExplorerInputAttempted();
  }
}

void WebContentsImpl::OnInternetExplorerMixedContentBlocked() {
  delegate_->OnInternetExplorerMixedContentBlocked(this);
}

void WebContentsImpl::OnInternetExplorerDocumentModeSet(
    uint32_t document_mode) {
  DCHECK_GT(document_mode, 0u);

  delegate_->OnInternetExplorerDocumentModeSet(this, document_mode);
}

void WebContentsImpl::OnTabCrashed(bool is_unrecoverable) {
  if (is_unrecoverable) {
    // Repeated failure to create a tab is a critical error. We can't continue
    // hosting IE in this condition.
    auto* session_host =
        BrowserContext::GetInternetExplorerSessionHost(GetBrowserContext());
    if (session_host) {
      session_host->Shutdown();
    }
  }
}

void WebContentsImpl::RequestInternetExplorerContentsActivation(
    bool needs_activation) {
  if (delegate_) {
    dialog_manager_ = delegate_->GetJavaScriptDialogManager(this);
  }
  if (dialog_manager_) {
    dialog_manager_->InternetExplorerIsShowingDialog(this, needs_activation);
  }
}

void WebContentsImpl::OnInternetExplorerVisibilityChanged(
    content::Visibility visibility) {
  for (auto& observer : internet_explorer_observers_) {
    observer.OnInternetExplorerVisibilityChanged(visibility);
  }
}

void WebContentsImpl::OnInternetExplorerHwndVisibilityChanged(bool visible) {
  for (auto& observer : internet_explorer_observers_) {
    observer.OnInternetExplorerHwndVisibilityChanged(visible);
  }
}

gfx::Size WebContentsImpl::GetContentViewSize() {
  return view_->GetViewBounds().size();
}

void WebContentsImpl::OnInternetExplorerIndicatorItemsSet(
    dual_engine::InternetExplorerProtectedMode protected_mode,
    dual_engine::InternetExplorerUrlZone url_zone,
    bool enterprise_mode) {
  delegate_->OnInternetExplorerIndicatorItemsSet(this, protected_mode, url_zone,
                                                 enterprise_mode);
}

void WebContentsImpl::OnInternetExplorerEnterpriseIDSet(bool is_enterprise_id) {
  if (dlp::IsAddressBarIndicatorEnabled()) {
    InternetExplorerEnterpriseIDState new_state =
        is_enterprise_id ? InternetExplorerEnterpriseIDState::kProtected
                         : InternetExplorerEnterpriseIDState::kPersonal;

    // We only need to do work when the state changes.
    if (internet_explorer_enterprise_id_state_ != new_state) {
      internet_explorer_enterprise_id_state_ = new_state;
      delegate_->OnInternetExplorerEnterpriseIDStateChanged(this);
    }
  }
}

bool WebContentsImpl::IsInternetExplorerModalDialog() {
  auto* tab_host = GetInternetExplorerTabHost();
  if (tab_host) {
    return tab_host->IsIEHostWindowDisabled();
  }
  return false;
}
...
void WebContentsImpl::ShowDlpDialog(bool show_block_dialog,
                                    DlpDialogCallback callback) {
  DCHECK(delegate_);
  if (delegate_) {
    if (dlp::DlpEnabled()) {
      delegate_->ShowDlpDialog(this, show_block_dialog, std::move(callback));
    }
  }
}
