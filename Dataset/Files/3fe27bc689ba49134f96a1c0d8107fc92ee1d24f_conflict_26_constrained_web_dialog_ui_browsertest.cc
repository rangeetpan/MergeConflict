...
// Also checking the variable for windows animation
...
IN_PROC_BROWSER_TEST_F(ConstrainedWebDialogBrowserTest, MAYBE_BasicTest) {
  ...
  gfx::NativeWindow window_ = dialog_delegate->GetNativeDialog();
  EXPECT_TRUE(window_);
#if defined(USE_AURA)
  EXPECT_FALSE(window_->GetProperty(aura::client::kAnimationsDisabledKey));
#endif
...
IN_PROC_BROWSER_TEST_F(ConstrainedWebDialogBrowserTest,
  ...
  EXPECT_TRUE(window_);
#if defined(USE_AURA)
  EXPECT_TRUE(window_->GetProperty(aura::client::kAnimationsDisabledKey));
#endif
  EXPECT_TRUE(IsShowingWebContentsModalDialog(web_contents));
}
