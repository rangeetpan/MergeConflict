...
<<<<<<< HEAD
#include "chrome/browser/ui/search/edge_ntp_user_data_logger.h"
=======
...
#else
  if (IsCacheableNTP(contents)) {
    UMA_HISTOGRAM_TIMES("Microsoft.Tab.NewTabOnload.Web", duration);
  } else {
    UMA_HISTOGRAM_TIMES("Microsoft.Tab.NewTabOnload.Local", duration);
  }
#endif
...
void SearchTabHelper::DidFinishNavigation(
  ...
  if (IsCacheableNTP(web_contents_)) {
    NTPUserDataLogger::GetOrCreateFromWebContents(web_contents())
        ->LogCacheableNTPLoad(search::CACHEABLE_NTP_LOAD_SUCCEEDED);
  }
}
...
void SearchTabHelper::FocusOmnibox(bool focus) {
  if (focus) {
    // Notify accessibility users of the focus redirect.
    edge::a11y::RaiseFakeboxRedirectEvent(web_contents());
  }
...
void SearchTabHelper::QueryAutocomplete(const base::string16& input,
  ...
  autocomplete_input.set_from_focus(input.empty()
                                        ? AutocompleteInputFocusType::kOmnibox
                                        : AutocompleteInputFocusType::kNone);
