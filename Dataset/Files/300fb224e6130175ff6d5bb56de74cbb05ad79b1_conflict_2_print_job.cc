...
#include "chrome/browser/printing/d2d_print_invoker.h"
...
<<<<<<< HEAD
#include "printing/printer_device_info.h"
=======
>>>>>>> d7b608079247e5114ab56b18804341bfe0db802d
...
void PrintJob::StartConversionToNativeFormat(
  ...
  if (settings.printer_is_textonly()) {
    ...
    if (printing::features::ShouldPrintUsingD2D())
      StartD2DPrinting(print_data, content_area);
    else
...
class PrintJob::D2DPrintingState {
 public:
  D2DPrintingState() {}

  void Start(scoped_refptr<base::RefCountedMemory> data,
             const PdfRenderSettings& conversion_settings,
             const PrinterDeviceInfo& device_info,
             D2DPrintInvoker::CompletedCallback completed_callback) {
    d2d_invoker_ = D2DPrintInvoker::StartPdfD2DPrinting(
        data, conversion_settings, device_info, std::move(completed_callback));
  }

 private:
  std::unique_ptr<D2DPrintInvoker> d2d_invoker_;
};

void PrintJob::StartD2DPrinting(scoped_refptr<base::RefCountedMemory> bytes,
                                const gfx::Rect& content_area) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  DCHECK(!d2d_printing_state_);
  d2d_printing_state_ = std::make_unique<D2DPrintingState>();
  ...
  PrinterDeviceInfo device_info = {settings.device_name(),
                                   worker_->GetPrintTicketStream(),
                                   document()->name()};

  d2d_printing_state_->Start(
      bytes, render_settings, device_info,
      base::BindOnce(&PrintJob::OnD2DPrintingCompleted, this));
}

void PrintJob::OnD2DPrintingCompleted(bool is_success) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  // Todo- Add telemetry-
  // https://microsoft.visualstudio.com/Edge/_workitems/edit/25971863
  OnDocumentDone();
}
