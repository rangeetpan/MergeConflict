...
<<<<<<< HEAD
#include "components/sync/base/edge_sync_features.h"
...
class TwoClientPreferencesSyncTest : public SyncTest {
 ...
 private:
  void EnableFeatureFlags();
  DISALLOW_COPY_AND_ASSIGN(TwoClientPreferencesSyncTest);
};

void TwoClientPreferencesSyncTest::SetUp() {
  EnableFeatureFlags();

  base::CommandLine::ForCurrentProcess()->AppendSwitch(
      switches::kSyncAllowAllSyncablePrefs);

  // Pass control back to SyncTest
  SyncTest::SetUp();
}

void TwoClientPreferencesSyncTest::EnableFeatureFlags() {
  feature_list_.InitAndEnableFeature(features::edge::kEdgeSyncPreferences);
}
