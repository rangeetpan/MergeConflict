...
class EdgeMIPCertificateTransparencySSLUITestBase
    : public CertVerifierBrowserTest {
 public:
  EdgeMIPCertificateTransparencySSLUITestBase()
      : CertVerifierBrowserTest(),
        https_server_(net::EmbeddedTestServer::TYPE_HTTPS) {}
  ~EdgeMIPCertificateTransparencySSLUITestBase() override {}
  ...
  void SetUp() override { CertVerifierBrowserTest::SetUp(); }

  void TestMIPDomainCTVerification(net::EmbeddedTestServer* https_server,
    ...
    // The domains are added explicitly via AddMIPDomainsForCTExclusion()
    // in chrome/browser/net/profile_network_context_service.cc
    static const base::StringPiece kMIPDomains[] = {
        "hostedrms.com", "aadrm.com", "aadrm.de", "aadrm.us", "aadrm.cn"};
    constexpr uint32_t kMIPDomainCount =
        sizeof(kMIPDomains) / sizeof(kMIPDomains[0]);
    for (uint32_t i = 0; i < kMIPDomainCount; i++) {
      ui_test_utils::NavigateToURL(
          browser(), https_server->GetURL(std::string(kMIPDomains[i]), "/"));
      ...
      ASSERT_TRUE(entry);
      uint32_t error = entry->GetSSL().cert_status;
...
class EdgeMIPCertificateTransparencySSLUITest
    : public EdgeMIPCertificateTransparencySSLUITestBase {
 public:
  EdgeMIPCertificateTransparencySSLUITest()
      : EdgeMIPCertificateTransparencySSLUITestBase() {}
  ~EdgeMIPCertificateTransparencySSLUITest() override {}

  void SetUp() override {
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndDisableFeature(chrome_pdf::features::kEnableMIPForPDF);
    EdgeMIPCertificateTransparencySSLUITestBase::SetUp();
  }
};

// Visit MIP domains which are excluded from CT verification.
// Simulate CT verification failure by using a certificate issued after
// the Certificate Transparency requirement date of April 2018,
// and verify that the CT check fails for the particular domains.
// Disabled by Pump Deputy: bug 23422629
IN_PROC_BROWSER_TEST_F(EdgeMIPCertificateTransparencySSLUITest,
                       DISABLED_EdgeMIPDomainCTVerification) {
  TestMIPDomainCTVerification(
      https_server(), net::CERT_STATUS_CERTIFICATE_TRANSPARENCY_REQUIRED);
}

class EdgeOverrideMIPCertificateTransparencySSLUITest
    : public EdgeMIPCertificateTransparencySSLUITestBase {
 public:
  EdgeOverrideMIPCertificateTransparencySSLUITest()
      : EdgeMIPCertificateTransparencySSLUITestBase() {}
  ~EdgeOverrideMIPCertificateTransparencySSLUITest() override {}

  void SetUp() override {
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(chrome_pdf::features::kEnableMIPForPDF);
    EdgeMIPCertificateTransparencySSLUITestBase::SetUp();
  }
};
...
// Simulate CT verification failure by using a certificate issued after
// the Certificate Transparency requirement date of April 2018,
// and verify that the CT check passes for the particular domains.
IN_PROC_BROWSER_TEST_F(EdgeOverrideMIPCertificateTransparencySSLUITest,
                       EdgeOverrideMIPDomainCTVerification) {
  TestMIPDomainCTVerification(https_server(), 0);
}
...
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSReportingCloseTab, 20307595)) {
  ASSERT_TRUE(https_server_expired_.Start());
...
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSProceedReporting, 20307595)) {
  certificate_reporting_test_utils::ExpectReport expect_report =
...
// is sent or not sent depending on the Finch config.
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSGoBackReporting, 20307595)) {
  certificate_reporting_test_utils::ExpectReport expect_report =
      certificate_reporting_test_utils::GetReportExpectedFromFinch();
  TestBrokenHTTPSReporting(
      certificate_reporting_test_utils::EXTENDED_REPORTING_OPT_IN,
...
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSProceedReportingWithNoOptIn, 20307595)) {
  TestBrokenHTTPSReporting(
...
// be sent, regardless of Finch config.
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSGoBackShowYesCheckNoParamYesReportNo, 20307595)) {
  TestBrokenHTTPSReporting(
      certificate_reporting_test_utils::EXTENDED_REPORTING_DO_NOT_OPT_IN,
      SSL_INTERSTITIAL_DO_NOT_PROCEED,
      certificate_reporting_test_utils::CERT_REPORT_NOT_EXPECTED, browser());
...
// report.
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSProceedShowNoCheckYesReportNo, 20307595)) {
...
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSInIncognitoReportNo, 20307595)) {
  TestBrokenHTTPSReporting(
      certificate_reporting_test_utils::EXTENDED_REPORTING_OPT_IN,
      SSL_INTERSTITIAL_PROCEED,
...
// disabled by policy.
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBrokenHTTPSNoReportingWhenDisallowed, 20307595)) {
...
// Finch config.
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBadClockReportingWithNoOptIn, 20307595)) {
...
// is sent or not sent depending on the Finch config.
IN_PROC_BROWSER_TEST_F(SSLUITestWithExtendedReporting,
                       EDGE_DISABLED_PERMANENT(TestBadClockReportingWithOptIn, 20307595)) {
  certificate_reporting_test_utils::ExpectReport expect_report =
      certificate_reporting_test_utils::GetReportExpectedFromFinch();
  TestBadClockReporting(
      certificate_reporting_test_utils::EXTENDED_REPORTING_OPT_IN,
...
IN_PROC_BROWSER_TEST_F(LegacyTLSInterstitialTest, ControlSiteNoInterstitial) {
  ...
  histograms.ExpectTotalCount(SSLErrorHandler::GetHistogramNameForTesting(), 0);
#else
  // Edge is hardcoded to not have exceptions.
  EXPECT_TRUE(
      chrome_browser_interstitials::IsShowingLegacyTLSInterstitial(tab));

  // Interstitial metrics should have been recorded from this navigation.
  histograms.ExpectTotalCount(SSLErrorHandler::GetHistogramNameForTesting(), 2);
...
// over legacy TLS.
// Edge disabled: we don't support the TLS config component and are hardcoded to not have exceptions.
IN_PROC_BROWSER_TEST_F(LegacyTLSInterstitialTest, EDGE_DISABLED_PERMANENT(ControlSitePagesCached, 24814525)) {
  InitializeLegacyTLSConfigWithControl();
  base::RunLoop run_loop;
  InitializeLegacyTLSConfigWithControlNetworkService(&run_loop);
