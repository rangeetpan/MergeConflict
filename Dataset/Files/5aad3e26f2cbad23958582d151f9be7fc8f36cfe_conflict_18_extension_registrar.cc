...
#include "chrome/browser/extensions/edge_user_settings_protector.h"
...
void ExtensionRegistrar::EnableExtension(const ExtensionId& extension_id) {
  ...
  if (extension &&
      extension_prefs_->HasDisableReason(
          extension_id, disable_reason::MS_DISABLE_DEFAULT_SETTING_TAKEOVER)) {
    EdgeUserSettingsProtector::Get(browser_context_)
        ->LogTelemetryOnExtensionEnabled(extension);
  }

  if (extension &&
      extension_prefs_->HasDisableReason(
          extension_id, disable_reason::MS_DISABLE_DUE_TO_IMPORT)) {
    base::RecordAction(base::UserMetricsAction(
        "Microsoft.Extensions.ImportedExtension_Enabled"));
  }
