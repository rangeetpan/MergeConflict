...
bool OmniboxViewViews::GetIconForCommandId(int command_id,
                                           gfx::Image* icon) const {
  if (command_id == IDC_PASTE_AND_GO) {
    *icon = gfx::Image(gfx::CreateVectorIcon(
        vector_icons::kPasteSearchIcon, views::MenuConfig::instance().icon_size,
...
<<<<<<< HEAD
  ...
  if (event.IsShiftDown() && HasSecondaryButtonInSelectedLine()) {
...
bool OmniboxViewViews::HasSecondaryButtonInSelectedLine() const {
  if (qbox::features::IsEdgeQBoxEnabled()) {
    OmniboxPopupModel* popup_model = model()->popup_model();
    size_t selected_line = popup_model->selected_line();
    if (model()->result().match_at(selected_line).ShouldShowTabMatchButton()) {
      return true;
    }
    return false;
  }

  return GetSecondaryButtonForSelectedLine() != nullptr;
}
...
bool OmniboxViewViews::MaybeFocusSecondaryButton() {
  if (HasSecondaryButtonInSelectedLine() &&
...
bool OmniboxViewViews::IsCommandIdEnabled(int command_id) const {
  ...
  if (command_id == IDC_CONTENT_CONTEXT_COPYASLINKPREVIEW &&
      features::IsAdvancedClipBoardEnabled()) {
...
void OmniboxViewViews::OnAfterCutOrCopy(ui::ClipboardBuffer clipboard_buffer) {
  ...
  bool should_write_advanced_html =
      write_url && features::IsAdvancedClipBoardEnabled();
  base::string16 anchor_text;
