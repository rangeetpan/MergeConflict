...
=======
#include <memory>
>>>>>>> 0c246d7ccd0aa1e5c2e5dabafb4f4a599ae4d54f
#include <utility>
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, TextfieldInnerRectBounds) {
  ...
  bounds = textfield_accessibility()->GetInnerTextRangeBoundsRect(
      0, 5, ui::AXCoordinateSystem::kScreenDIPs,
      ui::AXClippingBehavior::kClipped, &offscreen_result);
  EXPECT_EQ(textfield_->View::GetContentsBoundsInScreen(), bounds);

  // Ltr textfield.
  textfield_->SetText(base::ASCIIToUTF16("text"));
  bounds = textfield_accessibility()->GetInnerTextRangeBoundsRect(
      0, 4, ui::AXCoordinateSystem::kScreenDIPs,
      ui::AXClippingBehavior::kClipped, &offscreen_result);
  EXPECT_EQ(gfx::Rect(64, 74, 20, 32), bounds);
  ...
  bounds = textfield_accessibility()->GetInnerTextRangeBoundsRect(
      0, 4, ui::AXCoordinateSystem::kScreenDIPs,
      ui::AXClippingBehavior::kClipped, &offscreen_result);
  EXPECT_EQ(gfx::Rect(216, 74, 20, 32), bounds);
}
