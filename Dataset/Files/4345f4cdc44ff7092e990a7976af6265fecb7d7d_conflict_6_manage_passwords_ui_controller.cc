...
ManagePasswordsUIController::ManagePasswordsUIController(
  ...
  SetShouldShowEdgeSavePasswordAnimation(false);
...
void ManagePasswordsUIController::OnAutomaticPasswordSave(
  ...
  if (base::FeatureList::IsEnabled(
          autofill::features::kAutofillEdgePasswordConsentUX)) {
    ClearPopUpFlagForBubble();
...
void ManagePasswordsUIController::NeverSavePassword() {
  ...
  if (base::FeatureList::IsEnabled(
          autofill::features::kAutofillEdgePasswordConsentUX) ||
      passwords_data_.GetCurrentForms().empty())
    passwords_data_.OnInactive();
  else
    passwords_data_.TransitionToState(password_manager::ui::MANAGE_STATE);
  UpdateBubbleAndIconVisibility();
...
// BEGIN_EDGE_AUTOFILL_CODE
void ManagePasswordsUIController::OnAnimationEnded() {
  passwords_data_.OnInactive();
  SetShouldShowEdgeSavePasswordAnimation(false);
...
void ManagePasswordsUIController::SavePassword(const base::string16& username,
  ...
  if (base::FeatureList::IsEnabled(
          autofill::features::kAutofillEdgePasswordConsentUX)) {
    // Can not change state to INACTIVE state while animating password key
    if (!ShouldShowEdgeSavePasswordAnimation()) {
      // Remove the bubble after the user takes an action.
      passwords_data_.OnInactive();
    }
  } else {
    passwords_data_.TransitionToState(password_manager::ui::MANAGE_STATE);
