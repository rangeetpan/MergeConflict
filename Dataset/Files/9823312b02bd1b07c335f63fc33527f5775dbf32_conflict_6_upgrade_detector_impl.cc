...
#include "chrome/browser/ui/webui/help/version_updater_modern.h"
...
constexpr bool ShouldDetectOutdatedBuilds() {
  ...
  return true;
#else   // BUILDFLAG(MICROSOFT_EDGE_BRANDING)
  return false;
...
UpgradeDetectorImpl::UpgradeDetectorImpl(const base::Clock* clock,
    ...
    : UpgradeDetector(clock, tick_clock),
      single_thread_task_runner_(base::CreateSingleThreadTaskRunner(
...
void UpgradeDetectorImpl::DetectOutdatedInstall() {
  ...
  if (network_time.is_null() || build_date_.is_null() ||
    ...
    // Edge: commented out the debug assertion below since our CI machines hit
    // it when running unit tests. This could be hit if the network time had a
    // big uncertainty or the machine's current time is old. Asserting or
    // returning early here does not currently affect the correctness of any
    // unit test.
    // NOTREACHED();
    return;
  }
...
void UpgradeDetectorImpl::NotifyOnUpgradeWithTimePassed(
  ...
  if (new_stage != UPGRADE_ANNOYANCE_NONE ||
    ...
    NotifyUpgrade();
    // Record the time passed between upgrade detection and notification.
    // Minimum elapsed time is set to be small in case we experiment with
    // showing the notification very soon after detection.
    constexpr base::TimeDelta kMin = base::TimeDelta::FromMinutes(1);
    // Maximum elapsed time is set to several days more than
    // kDefaultHighThreshold, when the notification makes the icon red.
    constexpr base::TimeDelta kMax = base::TimeDelta::FromDays(14);
    constexpr int kBuckets = 50;
    base::UmaHistogramCustomTimes("Microsoft.Setup.Update.NotifyUpgradeTime",
                                  time_passed, kMin, kMax, kBuckets);
  }
}
...
void UpgradeDetectorImpl::Init() {
  ...
  if (simulating_outdated_) {
    ...
    // available on non MICROSOFT_EDGE_BRANDING.
  ...
  // On Windows, only enable upgrade notifications for Microsoft Edge builds.
  // Chromium does not use an auto-updater.
#if !defined(OS_WIN) || BUILDFLAG(MICROSOFT_EDGE_BRANDING)

  // On macOS, only enable upgrade notifications if the updater (Keystone) is
  // present.
#if defined(OS_MACOSX)
  if (!keystone_glue::KeystoneEnabled() && !mau_glue::MAUExists())
    return;
#endif
