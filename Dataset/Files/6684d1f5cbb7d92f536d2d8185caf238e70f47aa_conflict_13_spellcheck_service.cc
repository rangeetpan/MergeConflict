...
std::string SpellcheckService::GetTagToPassToWindowsSpellchecker(
    ...
    const std::string& supported_language_full_tag) {
  // BCP47 language tag to be passed to the Windows spellchecker API.
  std::string tag_to_pass;
  ...
  if (base::EqualsCaseInsensitiveASCII(supported_language_full_tag,
    ...
    tag_to_pass = supported_language_full_tag;
  } else {
    ...
    // "sr" which is ambiguous as Serbian can use Cyrllic or Latin script.
    // There is one unfortunate special case (so far the only one known). The
    // accept language "sr" is supported, and if you use it as a display
    ...
    if (base::EqualsCaseInsensitiveASCII("sr", accept_language)) {
      ...
      tag_to_pass = SpellcheckService::GetLanguageAndScriptTag(
          supported_language_full_tag,
          /* include_script_tag */ true);
    }
  }

  DVLOG(2) << "SpellcheckService::GetTagToPassToWindowsSpellchecker: "
              "returning "
           << tag_to_pass << " for input accept language " << accept_language
           << " and full tag " << supported_language_full_tag;
...
void SpellcheckService::InitWindowsDictionaryLanguages(
  ...
  for (const auto& windows_spellcheck_language : windows_spellcheck_languages) {
    ...
    if (base::EqualsCaseInsensitiveASCII(
            "sr-Cyrl", SpellcheckService::GetLanguageAndScriptTag(
                           windows_spellcheck_language,
                           /* include_script_tag */ true))) {
      AddWindowsSpellcheckDictionary("sr",
                                     windows_spellcheck_language);
    }
  }
  ...
  if (first_run::IsChromeFirstRun()) {
    ...
    // Some first run scenarios will add an accept language to preferences that
    // is not found in the hard-coded list in kAcceptLanguageList. That is,
    // despite its name kAcceptLanguageList does not represent all possible
    // accept language tags for a network request, but instead at least for
    // spellchecking provides the key for mapping user preferred languages to
    // the BCP47 tag passed to spellcheck API requests. Thus only languages in
    // kAcceptLanguageList can be spellchecked. An example is an installation on
    // a device where Finnnish is the Windows display language--the initial
    // accept language preferences are observed to be "fi-FI,fi,en-US,en". Only
    // "fi" is contained in kAcceptLanguageList.
    std::string first_user_language;
    std::vector<std::string> accept_languages;
    l10n_util::GetAcceptLanguages(&accept_languages);
    for (const auto& user_language : user_languages) {
