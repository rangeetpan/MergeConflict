...
#include "chrome/common/microsoft_url_loader_throttle.h"
#include "chrome/common/pepper_permission_util.h"
#include "chrome/common/pref_names.h"
<<<<<<< HEAD
#include "chrome/common/pref_names_util.h"
#include "chrome/common/prefer_anonymous_loader_throttle.h"
=======
>>>>>>> c8c8610bcb12575ab9a1968b45c325655cba6565
...
#include "components/microsoft/core/common/url_constants.h"
...
#include "microsoft_apis/microsoft_api_key_wrapper.h"
...
// Edge includes
#include "chrome/browser/edge_dlp/dlp_profile_navigation_throttle.h"
#include "chrome/browser/edge_family_safety/family_safety_navigation_throttle.h"
#include "chrome/browser/edge_tenant/tenant_restrictions_loader_throttle.h"
#include "chrome/browser/edge_trust/trust_manager/manager.h"
#include "chrome/browser/edge_trust/trust_utils.h"
#include "chrome/browser/edge_webshot/chrome_webshot_client.h"
#include "chrome/common/edge_header_features.h"
#include "components/edge_entity_extraction/content/browser/entity_extraction_driver.h"
#include "components/edge_entity_extraction/core/common/edge_entity_extraction_features.h"
#include "components/edge_smartscreen/buildflags/buildflags.h"
#include "components/edge_trust/tracking_protection/options.h"
#include "components/edge_trust/trust_throttle/trust_navigation_throttle.h"
#include "components/password_manager/core/common/edge_password_manager_features.h"
...
#include "chrome/browser/dual_engine/dual_engine_navigation_delay_throttle_win.h"
#include "chrome/browser/dual_engine/dual_engine_navigation_throttle_win.h"
...
#include "chrome/browser/dual_engine/dual_engine_site_permission_service_factory_win.h"
#include "chrome/browser/dual_engine/dual_engine_site_permission_service_win.h"
#include "chrome/browser/dual_engine/dual_engine_sitelist_win.h"
...
#include "chrome/browser/edge_embedded_browser/chrome_content_browser_client_embedded_browser_part.h"
#include "chrome/browser/edge_embedded_browser/embedded_browser_navigation_throttle.h"
#include "edge_embedded_browser/public/mojom/embedded_browser_factory.mojom.h"
#endif
...
#include "chrome/browser/edge_edu_policy/bing_ads_suppression_features.h"
#include "chrome/browser/edge_edu_policy/bing_ads_suppression_navigation_throttle.h"
#endif

#if defined(MICROSOFT_EDGE_BUILD) && defined(OS_MACOSX)
#include "components/edge_mac_touch_bar/content/browser/mac_touch_bar_tab_helper.h"
#endif
...
#include "chrome/browser/edge_smartscreen/smartscreen_iframe_navigation_throttle.h"
#include "chrome/browser/edge_smartscreen/smartscreen_service_client.h"
#include "chrome/browser/edge_smartscreen/smartscreen_ui_manager.h"
#include "chrome/browser/edge_smartscreen/url_checker_delegate_impl.h"
#include "components/edge_smartscreen/browser/browser_url_loader_throttle.h"
#include "components/edge_smartscreen/browser/url_checker_delegate.h"
#include "components/edge_smartscreen/common/smartscreen_prefs.h"
#endif
...
#include "chrome/browser/media/ms_playready_hardware_caps_win.h"
#endif

#if BUILDFLAG(ENABLE_WDAG)
#include "chrome/browser/win/wdag/nav_throttle.h"
#endif

#if BUILDFLAG(USE_REACT_NATIVE_SANDBOX)
#include "services/rn_jse/public/mojom/constants.mojom.h"
#endif

#if defined(EDGE_HISTORY_JOURNAL_TELEMETRY)
#include "chrome/browser/metrics/chrome_metrics_service_accessor.h"
#include "chrome/browser/profiles/edge_profile_utils.h"
#include "components/edge_hj/browser/anchor_click_metrics.h"
#include "components/edge_hj/browser/authentication_service_factory.h"
#include "components/edge_hj/browser/telemetry_client.h"
#include "components/edge_hj/public/features.h"
#endif  // defined(EDGE_HISTORY_JOURNAL_TELEMETRY)

#if BUILDFLAG(ENABLE_MEDIA_INTERNALS_SERVICES)
#include "components/metrics/edge_metrics_features.h"
...
#include "chrome/browser/ssl/edge_ssl_client_certificate_selector.h"
#include "chrome/browser/ui/tabs/edge_browser_main_extra_parts_tabs.h"
...
#if defined(EDGE_HISTORY_JOURNAL_TELEMETRY)
bool IsMSAAccountAuthenticated(content::BrowserContext* browser_context) {
  ...
  return IsUsingMSAccount(profile);
}
std::string GetMsaAccountId(content::BrowserContext* browser_context) {
  if (browser_context == nullptr) {
    return std::string();
  }
  ...
  return GetEdgeMsaAccountId(profile);
}

bool IsPersonalizationReportingEnabled(
    content::BrowserContext* browser_context) {
  Profile* profile = Profile::FromBrowserContext(browser_context);
  if (!profile)
    return false;
  ...
  CHECK(pref_service);
  return ChromeMetricsServiceAccessor::IsPersonalizationReportingEnabled(
      pref_service);
}
...
#if !defined(OS_ANDROID) && EXCLUDED_FROM_EDGE
...
#endif  // !defined(OS_ANDROID) && EXCLUDED_FROM_EDGE
...
std::string GetProduct() {
  return version_info::GetMicrosoftEdgeProductNameAndVersionForUserAgent();
}
...
ChromeContentBrowserClient::ChromeContentBrowserClient(
    ...
    : testing_mode_enabled(false), startup_data_(startup_data) {
#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
  base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
  if (command_line->HasSwitch(switches::kEmbeddedBrowserWebview)) {
    extra_parts_.push_back(
        new embedded_browser::ChromeContentBrowserClientEmbeddedBrowserPart);
  }
#endif
...
#if !defined(OS_ANDROID)
  main_parts->AddParts(new EdgeBrowserMainExtraPartsTabs);
#endif

#if defined(EDGE_HISTORY_JOURNAL_TELEMETRY)
  if (edge_hj::features::IsEdgeHJTelemetryEnabled()) {
    if (edge_hj::features::IsEdgeHJSanEnabled()) {
      edge_hj::AuthenticationServiceFactory::Init(
          GetIdentityManager, IsMSAAccountAuthenticated, GetMsaAccountId);
      edge_hj::TelemetryClient::InitializeTelemetryClient(
          ChromeMetricsServiceAccessor::IsUrlReportingEnabled,
          IsPersonalizationReportingEnabled, IsMSAAccountAuthenticated,
          GetMsaAccountId);
    } else {
      edge_hj::TelemetryClient::InitializeTelemetryClient(
          ChromeMetricsServiceAccessor::IsUrlReportingEnabled, nullptr,
          IsMSAAccountAuthenticated, GetMsaAccountId);
    }
  }
#endif  // defined(EDGE_HISTORY_JOURNAL_TELEMETRY)
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
void ChromeContentBrowserClient::CacheSmartScreenServiceClient() {
  if (!smartscreen_service_client_) {
    smartscreen_service_client_ = g_browser_process
        ->smartscreen_service_client(false /*get_when_pref_off*/);
  }
}
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
  CacheSmartScreenServiceClient();
#endif // BUILDFLAG(ENABLE_SMARTSCREEN)

  // Get a pointer to our TrustManager singleton, if it's available. This will
  // be used by trust features to access state and shared policy checks.
  trust_manager_ = g_browser_process->trust_manager();

  password_protection_client_ = g_browser_process->password_protection_client();
}
...
// https://crbug.com/739418.
  ...
  // https://permanently-removed.invalid is a standard tombstone for Google
  // sites skipped for zero-exhaust, not a genuine URL. This should always come
  // last in this function.
  isolated_origin_list.erase(
      std::remove(
          isolated_origin_list.begin(), isolated_origin_list.end(),
          url::Origin::Create(GURL("https://permanently-removed.invalid"))),
      std::end(isolated_origin_list));

  isolated_origin_list.push_back(url::Origin::Create(GURL(edge::kBingOrigin)));
...
#if defined(MICROSOFT_EDGE_BUILD)
  static const char* const kEdgeSurfGameSwitches[] = {
    error_page::switches::kDisableSurfGame,
  };
  command_line->CopySwitchesFrom(browser_command_line,
                                 kEdgeSurfGameSwitches,
                                 base::size(kEdgeSurfGameSwitches));
#endif // defined(MICROSOFT_EDGE_BUILD)

#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
  static const char* const kEmbeddedBrowserWebViewSwitches[] = {
      switches::kEmbeddedBrowserWebview,
      switches::kEmbeddedBrowserWebviewDpiAwareness,
  };
  command_line->CopySwitchesFrom(browser_command_line,
                                 kEmbeddedBrowserWebViewSwitches,
                                 base::size(kEmbeddedBrowserWebViewSwitches));
#endif
...
#if defined(MICROSOFT_EDGE_BUILD)
      if (prefs->HasPrefPath(prefs::kAllowSurfGame) &&
          !prefs->GetBoolean(prefs::kAllowSurfGame)) {
            command_line->AppendSwitch(
              error_page::switches::kDisableSurfGame);
          }
#endif // defined(MICROSOFT_EDGE_BUILD)
...
#if defined(OS_WIN)
const gfx::ImageSkia*
ChromeContentBrowserClient::GetDefaultInternetExplorerFavicon() {
  ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
  return rb.GetNativeImageNamed(IDR_DEFAULT_FAVICON_IE).ToImageSkia();
}
#endif
...
ChromeContentBrowserClient::AllowServiceWorkerOnIO(
  ...
  bool allow_cookies = io_data->GetCookieSettings()->IsCookieAccessAllowed(
      ...
      content_settings::kInvalidProcessID, content_settings::kInvalidFrameID);
...
ChromeContentBrowserClient::AllowServiceWorkerOnUI(
  ...
  bool allow_cookies =
      CookieSettingsFactory::GetForProfile(profile)->IsCookieAccessAllowed(
          scope, site_for_cookies, top_frame_origin,
          content_settings::kInvalidProcessID,
          content_settings::kInvalidFrameID);
...
void ChromeContentBrowserClient::OnTrackerAllowed(
    int process_id,
    int routing_id,
    const GURL& url,
    const GURL& first_party_url,
    const tracking_prevention::Mitigation mitigation_applied) {
  ...
  base::PostTask(
      ...
      base::BindOnce(
          &content_settings::TabSpecificContentSettings::TrackerAllowed,
          wc_getter, url, first_party_url, mitigation_applied));
}

void ChromeContentBrowserClient::OnTrackerBlocked(
    int process_id,
    int routing_id,
    const GURL& url,
    const GURL& first_party_url,
    const tracking_prevention::TrackerType tracker_type) {
  base::RepeatingCallback<content::WebContents*(void)> wc_getter =
      base::BindRepeating(&GetWebContentsFromProcessAndFrameId, process_id,
                          routing_id);
  base::PostTask(
      FROM_HERE, {BrowserThread::UI},
      base::BindOnce(
          &content_settings::TabSpecificContentSettings::TrackerBlocked,
          wc_getter, url, first_party_url, tracker_type));
}
...
std::string ChromeContentBrowserClient::GetGeolocationApiKey() {
  std::string api_key;
  api_key = MicrosoftApiKeyWrapper::GetInstance()->GetEdgeServicesApiKey();
  return api_key;
}
...
bool CertPatternMatch(
    const std::pair<certificate_matching::CertificatePrincipalPattern,
                    certificate_matching::CertificatePrincipalPattern>& pattern,
    const std::unique_ptr<net::ClientCertIdentity>& cert) {
  return (pattern.first.Matches(cert->certificate()->issuer())) &&
         (pattern.second.Matches(cert->certificate()->subject()));
}
...
std::unique_ptr<net::ClientCertIdentity> AutoSelectCertificate(
  ...
  return nullptr;
#else
  std::vector<std::pair<certificate_matching::CertificatePrincipalPattern,
                        certificate_matching::CertificatePrincipalPattern>>
      pattern_list;
  for (const base::Value& filter : filters->GetList()) {
    DCHECK(filter.is_dict());
    ...
    pattern_list.push_back(
        std::make_pair(std::move(issuer_pattern), std::move(subject_pattern)));
  }

  // Use the first certificate that is matched by the filter unless
  // ForceCertificatePromptsOnMultipleMatches is enabled. In that case, if
  // there are multiple certificates that match the filters, the cert picker
  // will appear and it will contain the certs that are matched by the auto
  // selection filter.
  const PrefService* service = g_browser_process->local_state();
  const bool force_cert_picker_on_multiple_matches =
      service->GetBoolean(prefs::kForceCertificatePromptsOnMultipleMatches);
  net::ClientCertIdentityList filtered_client_certs;

  for (auto& current_client_cert : client_certs) {
    for (const auto& current_pattern : pattern_list) {
      if (CertPatternMatch(current_pattern, current_client_cert)) {
        if (force_cert_picker_on_multiple_matches) {
          filtered_client_certs.push_back(std::move(current_client_cert));
          break;
        } else {
          return std::move(current_client_cert);
        }
      }
    }
  }

  if (force_cert_picker_on_multiple_matches && !filtered_client_certs.empty()) {
    client_certs = std::move(filtered_client_certs);
    if (client_certs.size() == 1) {
      return std::move(client_certs.front());
    }
  }
...
base::OnceClosure ChromeContentBrowserClient::SelectClientCertificate(
  ...
  if (testing_mode_enabled) {
    ...
    test_filtered_certs_ = std::move(client_certs);
    return base::OnceClosure();
# if !defined(OS_MACOSX) && !defined(OS_ANDROID)
  } else if (base::FeatureList::IsEnabled(
                 features::edge::kIdentityNewCertPicker)) {
    return ShowEdgeSSLClientCertificateSelector(web_contents, cert_request_info,
...
// - http://crbug.com/308033, http://crbug.com/696364.
  ...
  web_prefs->autoplay_policy =
      EdgeAutoplayConfig::GetAutoplayPolicy(profile, contents);
#else
...
void ChromeContentBrowserClient::BrowserURLHandlerCreated(
  ...
  handler->AddNewTabHandlerPair(&search::HandleNewTabURLRewrite,
                                &search::HandleNewTabURLReverseRewrite,
                                false);
  handler->AddNewTabHandlerPair(&search::HandleFirstTabWithNewTabURLRewrite,
                                &search::HandleNewTabURLReverseRewrite,
                                true);
#endif  // defined(OS_ANDROID)
  ...
  switch (sandbox_type) {
    case service_manager::SandboxType::kRenderer:
      return base::string16(install_static::GetSandboxSidPrefix()) +
             L"129201922";
#if BUILDFLAG(USE_REACT_NATIVE_SANDBOX)
    case service_manager::SandboxType::kReactNativeJs:
      return base::string16();
#endif
#if defined(MICROSOFT_EDGE_BUILD) && !BUILDFLAG(ENABLE_NACL)
    case service_manager::SandboxType::kCollections:
      return base::string16();
    case service_manager::SandboxType::kEntityExtraction:
      return base::string16();
#else  // MICROSOFT_EDGE_BUILD
    case service_manager::SandboxType::kCollections:
    case service_manager::SandboxType::kEntityExtraction:
      CHECK(0);
      return base::string16();
    ...
    case service_manager::SandboxType::kUtilityPrint:
      return base::string16();
...
#if BUILDFLAG(ENABLE_MEDIA_FOUNDATION_CDM)
    case service_manager::SandboxType::kMediaFoundationCdm:
#endif
#if BUILDFLAG(ENABLE_WDAG)
    case service_manager::SandboxType::kWdagComms:
#endif
...
// intercepts run.
  ...
  if (chrome::kBrowserProcessExecutableName != exe_path.BaseName().value() &&
      chrome::kWebViewProcessExecutableName != exe_path.BaseName().value())
    return true;
...
#if BUILDFLAG(ENABLE_WDAG)
  // wdag::NavThrottle can redirect navigations between the host and container
  // (vm) so it needs to run before any throttles that may change the navigation
  // in any way.
  std::unique_ptr<content::NavigationThrottle> wdag_throttle =
      wdag::NavThrottle::MaybeCreateThrottleFor(handle);
  if (wdag_throttle)
    throttles.push_back(std::move(wdag_throttle));
#endif

  std::unique_ptr<content::NavigationThrottle> dlp_throttle =
      dlp::DlpProfileNavigationThrottle::MaybeCreateThrottleFor(handle);
  if (dlp_throttle)
    throttles.push_back(std::move(dlp_throttle));
...
// ADO Bug# 23935406
#elif defined(OS_MACOSX) && defined(EDGE_MACOSX_UNIVERSAL_LINKS)
...
#if defined(MICROSOFT_EDGE_BUILD) && !defined(OS_ANDROID)
  if (features::edge::IsBingAdsSuppressionEnabled()) {
    MaybeAddThrottle(
        &throttles,
        BingAdsSuppressionNavigationThrottle::MaybeCreateThrottleFor(handle));
  }
#endif
  ...
  throttles.push_back(
      std::make_unique<
          smartscreen::SmartScreenIFrameNavigationThrottle>(handle));
#endif // #if BUILDFLAG(ENABLE_SMARTSCREEN)
...
#if defined(OS_WIN)
  // Note: DualEngineNavigationDelayThrottle is required to be registered
  // before DualEngineNavigationThrottle. The former has a WillStartRequest()
  // implementation that is used to ensure a precondition is met before
  // the latter's WillStartRequest() is run.
  std::unique_ptr<content::NavigationThrottle> dual_engine_delay_throttle =
      dual_engine::DualEngineNavigationDelayThrottle::MaybeCreateThrottleFor(
          handle);
  if (dual_engine_delay_throttle)
    throttles.push_back(std::move(dual_engine_delay_throttle));

  std::unique_ptr<content::NavigationThrottle> dual_engine_throttle =
      dual_engine::DualEngineNavigationThrottle::MaybeCreateThrottleFor(handle);
  if (dual_engine_throttle)
    throttles.push_back(std::move(dual_engine_throttle));
#endif

#if BUILDFLAG(ENABLE_EMBEDDED_BROWSER_WEBVIEW)
  std::unique_ptr<content::NavigationThrottle> embedded_browser_throttle =
      embedded_browser::EmbeddedBrowserNavigationThrottle::
          MaybeCreateThrottleFor(handle);
  ...
  std::unique_ptr<edge::family::FamilySafetyNavigationThrottle>
      family_safety_throttle =
          edge::family::FamilySafetyNavigationThrottle::MaybeCreateThrottle(
              handle);
  if (family_safety_throttle) {
    throttles.push_back(std::move(family_safety_throttle));
  }
...
void ChromeContentBrowserClient::GetHardwareSecureDecryptionCaps(
    ...
    bool* skip_get_mf_hw_secure_decryption_caps) {
...
#endif
#if BUILDFLAG(ENABLE_MEDIA_FOUNDATION_CDM)
  media::GetPlayReadyHardwareCaps(key_system, video_codecs, encryption_schemes,
                                  skip_get_mf_hw_secure_decryption_caps);
#endif
}
...
ChromeContentBrowserClient::CreateURLLoaderThrottles(
  ...
  // We cannot create a trust throttle without a TrustManager or io_data. If
  // either of these are missing we'll fail to create the throttle and no-op
  // here.
  std::unique_ptr<edge_trust::NavigationThrottle> trust_throttle =
      tracking_prevention::MaybeCreateNavigationThrottle(
          request, trust_manager_, profile, wc_getter);
  if (trust_throttle) {
    result.push_back(std::move(trust_throttle));
  }
  ...
  if (smartscreen_service_client_) {
    if (g_browser_process->local_state()->GetBoolean(
            prefs::kSmartScreenEnabled)) {
      auto url_checker_delegate =
          GetSmartScreenUrlCheckerDelegate(profile->GetResourceContext());
      if (url_checker_delegate) {
        const bool disable_smartscreen_continue_anyway =
            profile->GetPrefs()->GetBoolean(
                prefs::kDisableSmartScreenContinueAnyway);
        smartscreen_service_client_->StoreDisableContinueAnywayPolicyValue(
            disable_smartscreen_continue_anyway);
        auto smartscreen_throttle =
            smartscreen::BrowserURLLoaderThrottle::MaybeCreate(
                smartscreen_service_client_->smartscreen_client(),
                url_checker_delegate, wc_getter, request, GetUserAgent(),
                disable_smartscreen_continue_anyway, profile->GetPrefs(),
                smartscreen_service_client_->GetTestSettings());
        if (smartscreen_throttle) {
          result.push_back(std::move(smartscreen_throttle));
        }
      }
    } else {
      TRACE_EVENT0("SmartScreen", "SmartScreenDisabledBySettings");
    }
  }
#endif
  ...
  if (profile->IsIncognitoProfile()) {
    // Update default search provider if it exists. Otherwise fall back to Bing
    // origin.
    const TemplateURL* dsp = TemplateURLServiceFactory::GetForProfile(profile)
                                 ->GetDefaultSearchProvider();
    profile->GetPrefs()->SetString(prefs::kSearchUrl,
                                   dsp ? dsp->url() : edge::kBingOrigin);
  }

  chrome::mojom::DynamicParams microsoft_dynamic_params = {
      profile->GetPrefs()->GetBoolean(prefs::kForceGoogleSafeSearch),
      profile->GetPrefs()->GetInteger(prefs::kForceBingSafeSearch),
      profile->GetPrefs()->GetInteger(prefs::kForceYouTubeRestrict),
      profile->GetPrefs()->GetString(prefs::kAllowedDomainsForApps),
      profile->GetPrefs()->GetString(prefs::kSearchUrl)};

  // In the case of navigations, our |top_frame_origin| can come from a given
  // request's network isolation key. We don't need to try to acquire it here.
  result.push_back(std::make_unique<MicrosoftURLLoaderThrottle>(
    std::move(microsoft_dynamic_params)));

  if (base::FeatureList::IsEnabled(features::edge::kForceSendAnonymous) ||
      base::FeatureList::IsEnabled(features::edge::kSendAnonymousEnabled)) {
    result.push_back(std::make_unique<PreferAnonymousLoaderThrottle>(
        profile->IsIncognitoProfile(),
        microsoft_dynamic_params.default_search_url));
  }

  // Not necessarily dynamic, but only instantiate Google URL throttle if
  // safe search or YouTube restrict policies are set.
  if (profile->GetPrefs()->GetBoolean(prefs::kForceGoogleSafeSearch) ||
      profile->GetPrefs()->GetInteger(prefs::kForceYouTubeRestrict)) {
    chrome::mojom::DynamicParams google_dynamic_params = {
        profile->GetPrefs()->GetBoolean(prefs::kForceGoogleSafeSearch),
        false,
        profile->GetPrefs()->GetInteger(prefs::kForceYouTubeRestrict),
        profile->GetPrefs()->GetString(prefs::kAllowedDomainsForApps),
  ...
  result.push_back(std::make_unique<GoogleURLLoaderThrottle>(
      ...
      client_data_header,
#endif
      std::move(google_dynamic_params)));
  }
  ...
  // Tentant Restriction throttle
  std::unique_ptr<edge::TenantRestrictionsLoaderThrottle> tenant_throttle =
      edge::TenantRestrictionsLoaderThrottle::MaybeCreateThrottle();
  if (tenant_throttle) {
    result.push_back(std::move(tenant_throttle));
  }
...
#if BUILDFLAG(ENABLE_SMARTSCREEN)
scoped_refptr<smartscreen::UrlCheckerDelegate>
ChromeContentBrowserClient::GetSmartScreenUrlCheckerDelegate(
    content::ResourceContext* resource_context) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);

  if (!smartscreen_url_checker_delegate_) {
    smartscreen_url_checker_delegate_ =
        base::MakeRefCounted<smartscreen::UrlCheckerDelegateImpl>(
            smartscreen_service_client_->ui_manager());
  }

  return smartscreen_url_checker_delegate_;
}
#endif // #if BUILDFLAG(ENABLE_SMARTSCREEN)
...
#if defined(OS_WIN)
base::FilePath ChromeContentBrowserClient::GetCachedEmieXmlPath(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->GetCachedEmieXmlPath();
}

bool ChromeContentBrowserClient::ShouldSendIntranetToInternetExplorer(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->ShouldSendIntranetToInternetExplorer();
}

std::string
ChromeContentBrowserClient::GetInternetExplorerPopupBlockerPermissionData(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineSitePermissionService* service =
      dual_engine::DualEngineSitePermissionServiceFactory::GetForBrowserContext(
          browser_context);
  return service->GetPopupBlockerSitePermissionsConfigString();
}

std::string
ChromeContentBrowserClient::GetInternetExplorerGeolocationPermissionData(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineSitePermissionService* service =
      dual_engine::DualEngineSitePermissionServiceFactory::GetForBrowserContext(
          browser_context);
  return service->GetGeolocationSitePermissionsConfigString();
}

dual_engine::InternetExplorerStickyMode
ChromeContentBrowserClient::GetInternetExplorerStickyModePolicy(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->GetInternetExplorerStickyModePolicy();
}

bool ChromeContentBrowserClient::InternetExplorerShouldRespectAllowRedirect(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->InternetExplorerShouldRespectAllowRedirect();
}

dual_engine::ConfigurableSitesFlags
ChromeContentBrowserClient::GetInternetExplorerConfigurableSitesFlags(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->GetConfigurableSitesFlags();
}

GURL ChromeContentBrowserClient::GetDualEngineReportingPageUrl(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->GetDualEngineReportingPageUrl();
}

std::vector<dual_engine::CookieAndPolicy>
ChromeContentBrowserClient::GetInternetExplorerCookiesToSync(
    content::BrowserContext* browser_context) {
  dual_engine::DualEngineService* dual_engine_service =
      dual_engine::DualEngineServiceFactory::GetForBrowserContext(
          browser_context);
  return dual_engine_service->GetCookiesToSync(
...
bool ChromeContentBrowserClient::IsPwa(WebContents* web_contents) {
#if BUILDFLAG(ENABLE_EXTENSIONS)
  Browser* browser = chrome::FindBrowserWithWebContents(web_contents);
  if (browser && browser->app_controller() &&
      browser->app_controller()->IsHostedApp()) {
    return true;
  }
#endif
  return false;
...
std::string ChromeContentBrowserClient::GetPwaName(WebContents* web_contents) {
#if BUILDFLAG(ENABLE_EXTENSIONS)
  if (IsPwa(web_contents)) {
    Browser* browser = chrome::FindBrowserWithWebContents(web_contents);
    if (browser) {
...
scoped_refptr<content::edge::DomainActionsList>
ChromeContentBrowserClient::GetDomainActionsList() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  return g_browser_process->GetDomainActionsList();
}
...
// Edge
void ChromeContentBrowserClient::InvokeWebShotPreviewForRegionCapture(
    int render_process_id,
    int render_frame_id) {
  ...
  if (rfh) {
    ...
    ::edge::webshot::ChromeWebShotClient::GetOrCreateForWebContents(
        source_web_contents)
        ->InvokeWebShotPreviewForAreaCapture(render_process_id,
                                             render_frame_id);
    } else {
    // TODO(angu): Provide a visual indication to user that frame got destroyed
    }
}
