...
typedef std::vector<web_app::DisplayModifier> DisplayModifiers;
base::LazyInstance<DisplayModifiers>::Leaky g_empty_display_modifiers =
...
const std::vector<DisplayModifier>& WebApp::display_modifiers() const {
  if (!base::FeatureList::IsEnabled(features::kWebAppWindowControlsOverlay))
    return g_empty_display_modifiers.Get();
  return display_modifiers_;
}
...
void WebApp::SetDisplayModifiers(
    std::vector<DisplayModifier> display_modifiers) {
  display_modifiers_ = std::move(display_modifiers);
}
...
bool operator==(const WebApp& app1, const WebApp& app2) {
  return std::tie(app1.app_id_, app1.sources_, app1.name_, app1.launch_url_,
         ...
         std::tie(app2.app_id_, app2.sources_, app2.name_, app2.launch_url_,
                  ...
                  app2.additional_search_terms_, app1.protocol_handlers_,
                  app2.sync_data_);
}
