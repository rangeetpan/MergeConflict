...
<<<<<<< HEAD
#include "chrome/browser/google/about_page_error_metric_features.h"
#include "chrome/browser/google/configuration_concerns.h"
=======
...
bool HasLastAttemptToRenameFailed() {
  base::win::RegKey key(
      InstallUtil::IsPerUserInstall() ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE,
      install_static::GetClientStateKeyPath().c_str(),
      KEY_QUERY_VALUE | KEY_WOW64_32KEY);
  DWORD last_installer_error = 0;
  DWORD installer_error = 0;
  if (key.ReadValueDW(google_update::kRegLastInstallerErrorField,
                      &last_installer_error) == ERROR_SUCCESS &&
      key.ReadValueDW(installer::kInstallerError,
                            &installer_error) == ERROR_SUCCESS) {
    if (last_installer_error == installer::RENAME_FAILED &&
        installer_error == installer::RENAME_FAILED) {
      return true;
    }
  }
...
<<<<<<< HEAD
  if (hresult == GOOPDATE_E_APP_USING_EXTERNAL_UPDATER ||
      hresult == CO_E_SERVER_EXEC_FAILURE) {
    // These particular transient errors are worth retrying.
=======
...
HRESULT UpdateCheckDriver::BeginUpdateCheckInternal(
  ...
  if (wdag::IsContainerInstance()) {
    *error_code = GOOGLE_UPDATE_DISABLED_BY_APPLICATION_GUARD;
    return E_NOTIMPL;
  }
#endif

#if defined(MICROSOFT_EDGE_BUILD)
  if (HasLastAttemptToRenameFailed()) {
    *error_code = GOOGLE_UPDATE_BLOCKED_FROM_UPDATING;
    return E_FAIL;
  }
#endif
=======
...
void UpdateCheckDriver::PollGoogleUpdate() {
  ...
  if (!GetCurrentState(&state, &state_value, &hresult)) {
    ...
    DCHECK_GE(progress, last_reported_progress_);
...
void UpdateCheckDriver::OnUpgradeError(UpdateCheckResult check_result,
  ...
  int configuration_concerns_flags = CheckForConfigurationConcerns();
  base::UmaHistogramSparse("Microsoft.Setup.Update.ConfigurationConcerns",
                           configuration_concerns_flags);

  if (base::FeatureList::IsEnabled(
          features::edge::kEdgeEnableLoggingAboutPageErrors)) {
    TRACE_EVENT2(
        "diagnostic_event", "AboutPageError", "error_code",
        (base::StringPrintf("%d", static_cast<int>(error_code)).c_str()),
        "hresult",
        (base::StringPrintf("0x%lX", update_state_.hresult).c_str()));
  }
