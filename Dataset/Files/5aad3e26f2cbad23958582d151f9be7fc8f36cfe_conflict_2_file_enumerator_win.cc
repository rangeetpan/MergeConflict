...
bool FileEnumerator::IsPatternMatched(const FilePath& src) const {
  switch (folder_search_policy_) {
    ...
    case FolderSearchPolicy::ALL:
      ...
      // manually. Use downlevel apiset if it exists to avoid loading shlwapi.
      base::ScopedNativeLibrary module(base::FilePath(
          FILE_PATH_LITERAL("api-ms-win-downlevel-shlwapi-l1-1-0.dll")));
      if (module.is_valid()) {
        auto path_match_spec_function =
            reinterpret_cast<decltype(::PathMatchSpecW)*>(
                module.GetFunctionPointer("PathMatchSpecW"));
        if (path_match_spec_function)
          return path_match_spec_function(src.value().c_str(),
                                          pattern_.c_str()) == TRUE;
      }
