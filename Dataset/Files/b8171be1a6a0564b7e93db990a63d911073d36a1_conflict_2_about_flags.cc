...
// Header file to add all edge specific includes, avoiding merge conflicts
#include "chrome/browser/edge_about_flags.h"
...
<<<<<<< HEAD
#include "chrome/browser/dual_engine/dual_engine_features.h"
=======
...
// This map contains a list of cmd line switches
//    a) whose state can be modified from about-flags page
//    b) which are derived from modifying an existing chromium cmd-line switch,
//    or a new switch in Edge
// Keep entries in alphabetical order to reduce merge conflicts
// For more info on what entries should be added here see
// https://aka.ms/edgefx/afwiki
const std::set<std::string>& GetEdgeAboutFlagsSwitches() {
  static const base::NoDestructor<const std::set<std::string>>
      EdgeAboutFlagsSwitches({
          "disable-offline-auto-refresh",
          "disable-offline-auto-refresh-visible-only",
          "enable-offline-auto-refresh",
          "enable-offline-auto-refresh-visible-only",
          "extensions-on-edge-urls",
          "ignore-gpu-denylist",
          "internet-explorer-integration",
      });
  return *EdgeAboutFlagsSwitches;
}

const unsigned kOsAll = kOsMac | kOsWin | kOsLinux | kOsCrOS | kOsAndroid;
const unsigned kOsDesktop = kOsMac | kOsWin | kOsLinux | kOsCrOS;

const char LaunchFlagsAtStartupHistogram[] = "Launch.FlagsAtStartup";
const char MicrosoftLaunchFlagsAtStartupHistogram[] =
    "Microsoft.Launch.FlagsAtStartup";
...
// Edge does not support adaptive activation or crowd deny
const FeatureEntry::FeatureParam
...
//
// EDGE: DO NOT ADD VALUES HERE. See https://aka.ms/edgefx/afwiki for details.
const FeatureEntry kFeatureEntries[] = {
// Include generated flags for flag unexpiry; see //docs/flag_expiry.md and
// //tools/flags/generate_unexpire_flags.py.
#include "chrome/browser/unexpire_flags_gen.inc"
    {"ignore-gpu-denylist", flag_descriptions::kIgnoreGpuBlacklistName,
    ...
    {"extensions-on-edge-urls", flag_descriptions::kExtensionsOnChromeUrlsName,
    ...
    {"use-sync-sandbox", flag_descriptions::kSyncSandboxName,
     ...
     SINGLE_VALUE_TYPE_AND_VALUE(
         ...
         "https://ppe.activity.windows.com/v1/feeds/me/syncEntities/")},
#if !defined(OS_ANDROID)
...
// Helper file to add flags specific to Edge, avoiding merge conflicts
#include "edge_about_flags.cc"
...
#if 0  // TODO(frankli): enable it when ready for upstram.
#if BUILDFLAG(ENABLE_MEDIA_FOUNDATION_CDM)
    {"media-foundation-cdm", flag_descriptions::kMediaFoundationCdmName,
     flag_descriptions::kMediaFoundationCdmDescription, kOsWin,
     FEATURE_VALUE_TYPE(media::kMediaFoundationCdm)},
    ...
    // EDGE: DO NOT ADD VALUES HERE. See https://aka.ms/edgefx/afwiki instead.
};
...
// Returns true if the entry was added by Chromium and was not explicitly
// included in Edge.
bool IsExcludedChromiumEntry(const FeatureEntry& entry) {
  bool isChromiumEntry = ((std::string)entry.internal_name).find("edge-") != 0;
  return isChromiumEntry &&
    flags::GetIncludedFlags().find(entry.internal_name) ==
      flags::GetIncludedFlags().end();
}

bool ShouldSkipNonDeprecatedFeatureEntry(const FeatureEntry& entry) {
  return (~entry.supported_platforms & kDeprecated) ||
         IsExcludedChromiumEntry(entry);
}

bool SkipConditionalFeatureEntry(const FeatureEntry& entry) {
  // Don't expose chromium flag entries unless they were opted in
  if (IsExcludedChromiumEntry(entry))
    return true;
  ...
  // Controls Microsoft internal extension selfhosting.
  if (!strcmp("edge-experiment-extension-install", entry.internal_name) &&
      !base::FeatureList::IsMicrosoftInternalMachine()) {
    return true;
  }
  // Controls the ability to emulate lockdown mode for 10s
  if (!strcmp("edge-emulate-lockdown-mode", entry.internal_name) &&
      !base::FeatureList::IsMicrosoftInternalMachine())
    return true;
  // Controls the ability to emulate Windows.Core skus
  if (!strcmp("edge-emulate-windows-core-sku", entry.internal_name) &&
      !base::FeatureList::IsMicrosoftInternalMachine())
    return true;
  // Controls the ability to emulate Windows.Holographic skus
  if (!strcmp("edge-emulate-windows-holographic-sku", entry.internal_name) &&
      !base::FeatureList::IsMicrosoftInternalMachine())
    return true;
  // Controls the ability to emulate Windows.Xbox skus
  if (!strcmp("edge-emulate-windows-xbox-sku", entry.internal_name) &&
      !base::FeatureList::IsMicrosoftInternalMachine())
    return true;
  // Controls automatic tab freezing (Sleeping Tabs feature)
  if (!strcmp("edge-automatic-tab-freeze", entry.internal_name) &&
      !base::FeatureList::IsMicrosoftInternalUser())
    return true;
#if 0  // TODO(frankli): enable it when ready for upstram.
#if BUILDFLAG(ENABLE_MEDIA_FOUNDATION_CDM)
  if (!strcmp("media-foundation-cdm", entry.internal_name) &&
...
#if BUILDFLAG(ENABLE_WDAG)
  if (!strcmp(entry.internal_name, "edge-wdag-prelaunch") &&
      (wdag::IsContainerInstance() || !wdag::host::IsEnabledAndSupported()))
    return true;
#endif
...
void ReportAboutFlagsHistogramSwitches(const std::string& uma_histogram_name,
  ...
  bool is_launch_flags_at_startup_histogram =
      uma_histogram_name == LaunchFlagsAtStartupHistogram;
  for (const std::string& flag : switches) {
    int uma_id = about_flags::testing::kBadSwitchFormatHistogramId;
    bool should_log_in_microsoft_histogram = false;
    if (base::StartsWith(flag, "--", base::CompareCase::SENSITIVE)) {
      ...
      // recording changes in edge specific switches on about://flags page at
      // startup into edge about flags histogram
      should_log_in_microsoft_histogram =
          is_launch_flags_at_startup_histogram &&
          GetEdgeAboutFlagsSwitches().find(switch_name) !=
              GetEdgeAboutFlagsSwitches().end();
    } else {
    ...
    DVLOG(1) << "ReportAboutFlagsHistogram(): histogram='"
             << (should_log_in_microsoft_histogram
                     ? MicrosoftLaunchFlagsAtStartupHistogram
                     : uma_histogram_name)
             << "' '" << flag << "', uma_id=" << uma_id;
    base::UmaHistogramSparse(should_log_in_microsoft_histogram
                                 ? MicrosoftLaunchFlagsAtStartupHistogram
                                 : uma_histogram_name,
                             uma_id);
  }
...
void ReportAboutFlagsHistogramFeatures(const std::string& uma_histogram_name,
  ...
  for (const std::string& feature : features) {
    ...
    // recording changes in edge specific flags on about://flags page at startup
    // into edge about flags histogram
    bool should_log_in_microsoft_histogram =
        is_launch_flags_at_startup_histogram && feature.find("ms") == 0;
    DVLOG(1) << "ReportAboutFlagsHistogram(): histogram='"
             << (should_log_in_microsoft_histogram
                     ? MicrosoftLaunchFlagsAtStartupHistogram
                     : uma_histogram_name)
...
void RecordUMAStatistics(flags_ui::FlagsStorage* flags_storage) {
  ...
  ReportAboutFlagsHistogram(LaunchFlagsAtStartupHistogram, switches, features);
}
...
const std::set<std::string>& GetEdgeAboutFlagsSwitchesForTesting() {
  return GetEdgeAboutFlagsSwitches();
}
