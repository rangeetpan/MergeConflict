...
void LocalHistoryZeroSuggestProvider::Start(const AutocompleteInput& input,
  ...
  if (input.from_focus() != AutocompleteInputFocusType::kOmnibox ||
  ...
  if (!template_url_service ||
      ...
      template_url_service->GetDefaultSearchProvider()->GetEngineType(
        ...
        #else
          template_url_service->search_terms_data()) != SEARCH_ENGINE_BING) {
        #endif
    return;
  }
...
#else
  GURL bing_base_url(
      template_url_service->search_terms_data().BingBaseUrlValue());
  std::string bing_search_url = bing_base_url.spec();
#endif
...
#else
      base::ASCIIToUTF16(bing_search_url), opts,
#endif
...
void LocalHistoryZeroSuggestProvider::QueryURLDatabase(
  ...
  for (const auto& result : results) {
    ...
    AutocompleteMatch match = BaseSearchProvider::CreateSearchSuggestion(
        ...
        /*is_in_private_mode*/ client_->IsIncognitoMode(),
...
std::string LocalHistoryZeroSuggestProvider::GetOrCreateConversationId() {
  base::TimeTicks current_time(base::TimeTicks::Now());
  // Renew token if it expired.
  if (current_time > conversation_expiration_time_) {
    conversation_id_ = base::GenerateGUID();
    conversation_id_.erase(
        std::remove(conversation_id_.begin(), conversation_id_.end(), '-'),
        conversation_id_.end());
  }

  // Extend expiration time another 60 seconds.
  conversation_expiration_time_ = current_time + base::TimeDelta::FromSeconds(60);
