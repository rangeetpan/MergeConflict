...
struct AvailableLocalesTraits
  ...
  static std::vector<std::string>* New(void* instance) {
    ...
    for (int i = 0; i < num_locales; ++i) {
      ...
      if (base::LowerCaseEqualsASCII(locale_name, "fa-af"))
        locale_name = "prs";
...
// Retrieve the UI display language for the operating system.
std::string GetOSDisplayLanguage() {
  std::string os_language;
#if defined(OS_WIN)
  os_language = GetOSDisplayLanguageWin();
#elif defined(OS_MACOSX)
  // Mac OS makes no distinction between preferred languages and display
  // languages.
  GetPreferredLocaleMac(&os_language);
#endif
  return os_language;
}
...
bool CheckAndResolveLocale(const std::string& locale,
  ...
  // code.  For example, en_US@variant should cause us to try en@variant
  // before en.
  std::string::size_type variant_pos = locale.find('@');
  if (variant_pos != std::string::npos) {
    // Valencian is a special case, where we do care about the variant code.
    if (base::LowerCaseEqualsASCII(locale, "ca-es@valencia")) {
      std::string tmp_locale("ca-ES-valencia");
  ...
  struct {
    ...
    const char* dest;
      ...
      {"en", "en-US"}, {"iw", "he"},  {"mo", "ro"},      {"no", "nb"},
      {"pt", "pt-BR"}, {"tl", "fil"}, {"uz", "uz-Latn"},
  };
...
std::string GetApplicationLocaleInternalNonMac(const std::string& pref_locale) {
  ...
  if (!pref_locale.empty()) {
    // Pseudo-loc languages don't follow the expected ICU structure,
    // so we use it without any preprocessing.
    if (base::StartsWith(pref_locale, "qps-", base::CompareCase::SENSITIVE))
...
std::string GetApplicationLocale(const std::string& pref_locale,
  ...
  if (set_icu_locale && !locale.empty()) {
    if (base::StartsWith(pref_locale, "qps-", base::CompareCase::SENSITIVE)) {
      const std::string qps_locale("en-US");
      base::i18n::SetICUDefaultLocale(qps_locale);
    } else {
...
base::string16 GetDisplayNameForLocale(const std::string& locale,
  ...
  if (locale_code == "zh-CN")
    ...
    locale_code = "ro-MD";
  else if (locale_code == "prs")
    locale_code = "fa-AF";

  // Internally, we use prs instead of fa-AF. Because ICU doesn't support prs at
  // all, we need to map it when it's the display locale as well.
  std::string display_locale_code = display_locale;
  if (display_locale_code == "prs")
    display_locale_code = "fa-AF";
