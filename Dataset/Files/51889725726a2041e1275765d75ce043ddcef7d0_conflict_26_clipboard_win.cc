...
#include "base/bind.h"
<<<<<<< HEAD
#include "base/edge_dlp/dlp_wip_clipboard_win.h"
#include "base/edge_dlp/edge_dlp.h"
=======
#include "base/check_op.h"
>>>>>>> 8bee450f65b89496505e75dd50a2e88b0385aed4
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
void RenderAdvancedClipboardFormat(CLIPFORMAT format) {
  if (features::IsLinkPreviewOmniboxEnabled() ||
      features::IsLinkPreviewInPageEnabled()) {
    ui::Clipboard* clipboard = ui::Clipboard::GetForCurrentThread();
    if (clipboard) {
      HGLOBAL handle = clipboard->GetAdvancedHTMLHandle(format);
      DCHECK(handle);
...
bool ClipboardOwnerWndProc(UINT message,
  ...
  case WM_RENDERFORMAT:
    ...
    if (wparam ==
        ...
        wparam == ui::ClipboardFormatType::GetLinkPreviewType()
                      .ToFormatEtc()
                      .cfFormat)
      RenderAdvancedClipboardFormat(wparam);
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
    break;
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
    // We will not render formats at this time since the clipboard for UI thread
    // is destroyed and creating it again will leave processes hanging around
    // after browser is destroyed.
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
    break;
...
void ClipboardWin::ReadAvailableTypes(
  ...
  // Read Custom HTML formats
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  if ((features::IsLinkPreviewOmniboxEnabled() ||
       features::IsLinkPreviewInPageEnabled()) &&
      ::IsClipboardFormatAvailable(
          ClipboardFormatType::GetLinkPreviewType().ToFormatEtc().cfFormat))
    types->push_back(base::UTF8ToUTF16(kMimeTypeLinkPreview));
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)

  if (edge_share::features::edge::IsEdgeShareEnhancedCopyPasteEnabled() &&
      ::IsClipboardFormatAvailable(
          ClipboardFormatType::GetTitledHyperlinkType().ToFormatEtc().cfFormat))
    types->push_back(base::UTF8ToUTF16(kMimeTypeTitledHyperlink));
...
void ClipboardWin::ReadHTML(ClipboardBuffer buffer,
  ...
  ReadHTMLFromFormat(buffer, markup, src_url, fragment_start, fragment_end,
                     ClipboardFormatType::GetHtmlType());
}

void ClipboardWin::ReadHTMLFromFormat(
    ClipboardBuffer buffer,
    base::string16* markup,
    std::string* src_url,
    uint32_t* fragment_start,
  ...
  *fragment_end = 0;
  ...
  HANDLE data = ::GetClipboardData(format_type.ToFormatEtc().cfFormat);
  if (!data)
    return;
...
void ClipboardWin::ReadEnterpriseId(base::string16* result) const {
  DCHECK(dlp::IsClipboardEnforcementOn());
  if (!result) {
    NOTREACHED();
    return;
  }
  ...
  if (dlp::wip::ShouldCheckClipboardFormat()) {
    HANDLE data = ::GetClipboardData(
        ClipboardFormatType::GetEnterpriseIdType().ToFormatEtc().cfFormat);
    if (data) {
  ...
  result->assign(dlp::wip::GetEnterpriseIdForClipboard());
}

void ClipboardWin::ReadTitledHyperlink(
    ClipboardBuffer buffer,
    base::string16* titled_hyperlink_markup) const {
  DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
  std::string src_url;
  uint32_t start, end;
  ReadHTMLFromFormat(buffer, titled_hyperlink_markup, &src_url, &start, &end,
                     ClipboardFormatType::GetTitledHyperlinkType());
}

void ClipboardWin::GetAccessDeniedAsciiText(ClipboardBuffer buffer,
                                            std::string* result) const {
  DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
  if (!result) {
    NOTREACHED();
  ...
  HANDLE data = dlp::wip::GetClipboardAsciiTextAccessDeniedData();
  if (!data)
    return;
...
void ClipboardWin::GetAccessDeniedText(ClipboardBuffer buffer,
                                       base::string16* result) const {
  DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
  if (!result) {
    NOTREACHED();
  ...
  HANDLE data = dlp::wip::GetClipboardTextAccessDeniedData();
  if (!data)
    return;
...
void ClipboardWin::WriteEnterpriseId(const char* enterprise_id,
                                     size_t text_len) {
  if (dlp::IsClipboardEnforcementOn()) {
    base::string16 text;
    base::UTF8ToUTF16(enterprise_id, text_len, &text);
    HGLOBAL glob = CreateGlobalData(text);

    WriteToClipboard(ClipboardFormatType::GetEnterpriseIdType(), glob);
    dlp::wip::SetEnterpriseIdForClipboard(text);
  }
}
...
void ClipboardWin::WriteHTML(const char* markup_data,
                             ...
                             const char* url_data,
                             size_t url_len) {
  WriteHTMLToFormat(markup_data, markup_len, url_data, url_len,
                    ClipboardFormatType::GetHtmlType());
}

void ClipboardWin::WriteHTMLToFormat(const char* markup_data,
                                     size_t markup_len,
                                     const char* url_data,
                                     size_t url_len,
...
void ClipboardWin::WriteBitmap(const SkBitmap& in_bitmap,
  ...
  if (bits && source_hbitmap) {
    ...
    WriteBitmapFromHandle(source_hbitmap,
                          gfx::Size(bitmap.width(), bitmap.height()), format);
  }
...
void ClipboardWin::WriteBitmap(const SkBitmap& in_bitmap) {
  WriteBitmap(in_bitmap, ClipboardFormatType::GetBitmapType());
}

void ClipboardWin::WriteBitmapCapture(const SkBitmap& in_bitmap) {
  WriteBitmap(in_bitmap, ClipboardFormatType::GetBitmapCaptureType());
}
...
void ClipboardWin::WriteBitmapFromHandle(HBITMAP source_hbitmap,
  ...
  WriteToClipboard(format, hbitmap);
}

void ClipboardWin::WriteTitledHyperlink(const char* markup_data,
                                        size_t markup_len,
                                        const char* url_data,
                                        size_t url_len) {
  WriteHTMLToFormat(markup_data, markup_len, url_data, url_len,
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
HGLOBAL ClipboardWin::GetAdvancedHTMLHandle(CLIPFORMAT format) {
  std::string link_preview_data;
  if (!delayed_callback_.is_null())
    delayed_callback_.Run(format, &link_preview_data);

  if (link_preview_data.empty()) {
    // Entity extraction did not complete. Put the original url in the HTML/JSON
    // format on the clipboard.
    link_preview_data = delayed_url_;
    RecordWebCardExtractionResult(WebCardExtractionResult::kFailed);
  } else {
    // Html markup successfully extracted
    RecordWebCardExtractionResult(WebCardExtractionResult::kSuccess);
  }

  if (format == ui::ClipboardFormatType::GetHtmlType().ToFormatEtc().cfFormat) {
    // Create a CF_HTML to be put on clipboard.
    link_preview_data =
        ClipboardUtil::HtmlToCFHtml(link_preview_data, delayed_url_);
  } else {
    // Create the fallback JSON when no data was extracted.
    if (link_preview_data == delayed_url_)
      link_preview_data =
          base::StrCat({"{\"url\":", "\"", link_preview_data, "\""});
  }

  return CreateGlobalData(link_preview_data);
}

void ClipboardWin::DelayRenderFormat(
    const std::string& url,
    CLIPFORMAT format,
    Clipboard::StartExtractionCallback callback) {
  delayed_url_ = url;
  ...
  // Delay render the format by passing nullptr as handle. We will delay render
  // the Link Preview Format when HTML format is delay rendered.
...
void ClipboardWin::RecordWebCardExtractionResult(WebCardExtractionResult extraction_result) {
  UMA_HISTOGRAM_ENUMERATION("Microsoft.Clipboard.AdvancedClipboard.ExtractionResult",
                            extraction_result);
}

void ClipboardWin::ReadLinkPreviewData(ClipboardBuffer buffer,
                                       std::string* result) const {
  DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);

  ReadData(ClipboardFormatType::GetLinkPreviewType(), result);
  TrimAfterNull(result);
}
