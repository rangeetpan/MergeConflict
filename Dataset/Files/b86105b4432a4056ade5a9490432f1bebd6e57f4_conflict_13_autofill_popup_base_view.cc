...
<<<<<<< HEAD
#include "chrome/browser/ui/views/edge_layout_provider.h"
...
>>>>>>> 8ad47e8d21f6866e4a37f47d83a860d41debf514
...
#if defined(OS_WIN)
  return ui::win::IsAeroShadowEnabled();
#else
  return true;
#endif
}
...
int AutofillPopupBaseView::GetCornerRadius() {
  if (!CustomShadowsSupported())
    return 0;
  return EdgeLayoutProvider::Get()->GetCornerRadiusMetric(
      base::FeatureList::IsEnabled(features::kAutofillEdgeSuggestionUI)
          ? views::EMPHASIS_HIGH
...
void AutofillPopupBaseView::DoShow() {
  ...
  if (initialize_widget) {
    ...
    params.opacity = CustomShadowsSupported()
        ? views::Widget::InitParams::WindowOpacity::kTranslucent
        : views::Widget::InitParams::WindowOpacity::kOpaque;
    params.shadow_type = views::Widget::InitParams::ShadowType::kNone;
    params.always_use_non_client_view = true;
...
void AutofillPopupBaseView::DoUpdateBoundsAndRedrawPopup() {
  ...
  popup_bounds.Inset(-GetWidget()->non_client_view()->frame_view()->
      border()->GetInsets());
...
views::NonClientFrameView* AutofillPopupBaseView::CreateNonClientFrameView(
    views::Widget* widget) {
  views::BubbleFrameView* frame =
      new views::BubbleFrameView(gfx::Insets(), gfx::Insets());
  frame->SetBubbleBorder(base::WrapUnique(
      static_cast<views::BubbleBorder*>(CreateBorder().release())));
  return frame;
}

bool AutofillPopupBaseView::ShouldShowWindowTitle() const {
  return false;
}

bool AutofillPopupBaseView::ShouldShowCloseButton() const {
  return false;
}
...
std::unique_ptr<views::Border> AutofillPopupBaseView::CreateBorder() {
  ...
  if (base::FeatureList::IsEnabled(features::kAutofillEdgeSuggestionUI)) {
    border = std::make_unique<views::BubbleBorder>(
        ...
        CustomShadowsSupported() ? views::BubbleBorder::SMALL_SHADOW
