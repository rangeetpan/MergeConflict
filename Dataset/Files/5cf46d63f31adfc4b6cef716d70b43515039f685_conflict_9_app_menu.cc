...
#if defined(MICROSOFT_EDGE_BUILD)
#include "chrome/browser/ui/views/toolbar/feedback_sub_menu_context_menu.h"
#include "chrome/browser/ui/views/toolbar/toolbar_buttons_sub_menu_context_menu.h"
...
// Edge-specific horizontal padding for the + and - zoom buttons
const int kZoomControlsHorizontalPadding = 26;
...
#else
const int kFullscreenPadding = 28 - kZoomControlsHorizontalPadding;
const int kZoomLabelHorizontalPadding = 4;
#endif
...
class ZoomButton : public ImageButton {
 public:
  ...
  explicit ZoomButton(views::ButtonListener* listener, int index)
      : ImageButton(listener) {
    Init(index);
  }
  ZoomButton(const ZoomButton&) = delete;
  ZoomButton& operator=(const ZoomButton&) = delete;

  void Init(int index) {
    // all buttons on menu should must be a custom button in order for
    // the keyboard navigation to work.
    DCHECK(Button::AsButton(this));

    // Since |fullscreen_button_| will reside in a menu, make it ALWAYS
    // focusable regardless of the platform.
    this->SetFocusBehavior(FocusBehavior::ALWAYS);
    this->set_tag(index);
    this->SetImageHorizontalAlignment(ImageButton::ALIGN_CENTER);
    this->SetImageVerticalAlignment(ImageButton::ALIGN_MIDDLE);
    this->SetBackground(std::make_unique<InMenuButtonBackground>(
  ...
  gfx::Size CalculatePreferredSize() const override {
    ...
    pref.Enlarge(kZoomControlsHorizontalPadding, 0);
    return pref;
  }
  ...
  bool HandleAccessibleAction(const ui::AXActionData& action_data) override {
    if (GetEnabled()) {
      // We should set system focus to the item the screen reader is focusing.
      SetHotTracked(true);
  ...
  void UpdateImageColors() override {
    // If we're on Mac, recolor this to match the rest of the menu.
#if defined(OS_MACOSX)
    ui::NativeTheme* native_theme  = GetNativeTheme();
    if(native_theme->UsesHighContrastColors()) {
      switch (state()) {
        case STATE_NORMAL:
          image()->Recolor(
 ...
 public:
  CutCopyPasteView(AppMenu* menu,
    ...
    InMenuButton* cut = CreateAndConfigureButton(
        IDS_CUT, InMenuButtonBackground::NO_BORDER, cut_index);
    InMenuButton* copy = CreateAndConfigureButton(
        IDS_COPY, InMenuButtonBackground::NO_BORDER, copy_index);
    InMenuButton* paste = CreateAndConfigureButton(
        IDS_PASTE, InMenuButtonBackground::NO_BORDER, paste_index);

    // Set the icons from the model.
    gfx::Image icon;
    if (menu_model->GetIconAt(cut_index, &icon))
      cut->SetImage(views::Button::ButtonState::STATE_NORMAL,
                    *icon.ToImageSkia());
    if (menu_model->GetIconAt(copy_index, &icon))
      copy->SetImage(views::Button::ButtonState::STATE_NORMAL,
                      *icon.ToImageSkia());
    if (menu_model->GetIconAt(paste_index, &icon))
      paste->SetImage(views::Button::ButtonState::STATE_NORMAL,
                      *icon.ToImageSkia());
  }
...
class AppMenu::ZoomView : public AppMenuView {
 public:
  ZoomView(AppMenu* menu,
    ...
    // Decrement Zoom Button
    decrement_button_ = new ZoomButton(this);
    decrement_button_->Init(decrement_index);
    decrement_button_->SetBackground(
      std::make_unique<InMenuButtonBackground>(
        InMenuButtonBackground::LEADING_BORDER));

    const base::string16 decrement_button_tooltip =
      GetAccessibleNameForAppMenuItem(menu_model, decrement_index,
                                      IDS_ACCNAME_ZOOM_MINUS2, false);
    decrement_button_->SetAccessibleName(decrement_button_tooltip);
    decrement_button_->SetTooltipText(decrement_button_tooltip);

    if (menu_model->GetIconAt(decrement_index, &icon))
      decrement_button_->SetImage(
          views::Button::ButtonState::STATE_NORMAL, *icon.ToImageSkia());
    AddChildView(decrement_button_);
    ...
    zoom_label_->SetBackground(std::make_unique<InMenuButtonBackground>(
        InMenuButtonBackground::NO_BORDER));
    // An accessibility role of kAlert will ensure that any updates to the
    // zoom level can be picked up by screen readers.
    zoom_label_->GetViewAccessibility().OverrideRole(ax::mojom::Role::kAlert);
    AddChildView(zoom_label_);
    zoom_label_max_width_valid_ = false;

    // Increment Zoom Button
    increment_button_ = new ZoomButton(this);
    increment_button_->Init(increment_index);

    const base::string16 increment_button_tooltip =
        GetAccessibleNameForAppMenuItem(menu_model, increment_index,
                                        IDS_ACCNAME_ZOOM_PLUS2, false);
    increment_button_->SetAccessibleName(increment_button_tooltip);
    increment_button_->SetTooltipText(increment_button_tooltip);

    if (menu_model->GetIconAt(increment_index, &icon))
      increment_button_->SetImage(
          views::Button::ButtonState::STATE_NORMAL, *icon.ToImageSkia());
    AddChildView(increment_button_);

    // Fullscreen Button
    fullscreen_button_ = new ZoomButton(this);
    fullscreen_button_->Init(fullscreen_index);
    fullscreen_button_->SetBackground(
      std::make_unique<InMenuButtonBackground>(
        InMenuButtonBackground::LEADING_BORDER));

    const base::string16 fullscreen_button_tooltip =
        GetAccessibleNameForAppMenuItem(menu_model, fullscreen_index,
                                        IDS_ACCNAME_FULLSCREEN, false);
    fullscreen_button_->SetAccessibleName(fullscreen_button_tooltip);
    fullscreen_button_->SetTooltipText(fullscreen_button_tooltip);

    if (menu_model->GetIconAt(fullscreen_index, &icon))
      fullscreen_button_->SetImage(views::Button::ButtonState::STATE_NORMAL,
                                    *icon.ToImageSkia());
  ...
  void OnThemeChanged() override {
    ...
    const auto& fullscreen_icon =
                menu()->browser_->exclusive_access_manager()
 ...
 private:
  ...
  ZoomButton* increment_button_;

  // Label showing zoom as a percent.
  Label* zoom_label_;

  // Button for decrementing the zoom.
  ZoomButton* decrement_button_;

  ZoomButton* fullscreen_button_;
...
void AppMenu::Init(ui::MenuModel* model) {
  ...
  // Set the alternative accessible name for the root, so the overall menu will
  // have the app menu name as its accessible name.
  root_->SetAlternativeAccName(
      l10n_util::GetStringUTF16(IDS_ACCNAME_APPMENU_NO_SHORTCUT));
...
bool AppMenu::ShowContextMenu(MenuItemView* source,
  ...
  if (IsBookmarkCommand(command_id))
    return bookmark_menu_delegate_->ShowContextMenu(source, command_id, p,
                                                    source_type);

  if ((base::FeatureList::IsEnabled(features::edge::kToolbarBookmarksButton) &&
       command_id == IDC_BOOKMARKS_MENU) ||
      (base::FeatureList::IsEnabled(
           features::edge::kToolbarMoreMenuButtonsEnabled) &&
       command_id == IDC_SHARE) ||
      (IsCollectionsAndPromotable(command_id))) {
    bookmark_menu_->GetSubmenu()->Close();
    views::Widget* parent = views::Widget::GetWidgetForNativeWindow(
        browser_->window()->GetNativeWindow());
    ToolbarButtonsSubMenuContextMenu* context_menu =
        new ToolbarButtonsSubMenuContextMenu(command_id, parent, browser_);
    context_menu->RunMenuAt(p, source_type);
    return true;
  }

  if (!browser_->profile()->IsGuestSession()) {
    // display show/hide smiley button only when Feedback Policy is
    // not configured
    PrefService* prefs = browser_->profile()->GetPrefs();
    if (!prefs->IsManagedPreference(prefs::kEdgeFeedbackEnabled) &&
      command_id == IDC_EDGE_FEEDBACK) {
      views::Widget* parent = views::Widget::GetWidgetForNativeWindow(
          browser_->window()->GetNativeWindow());
      FeedbackSubMenuContextMenu* context_menu =
          new FeedbackSubMenuContextMenu(parent, browser_);
      context_menu->RunMenuAt(p, source_type);
      return true;
    }
  }
...
bool AppMenu::IsCollectionsAndPromotable(int command_id) {
  return (command_id == IDC_TOGGLE_COLLECTIONS &&
          base::FeatureList::IsEnabled(
              features::edge::kToolbarMoreMenuButtonsEnabled) &&
          features::edge::IsCollectionsFeatureEnabled(
...
bool AppMenu::IsItemUnderManagement(int command_id) const {
  if (command_id == 0)
    return false;  // The root item.

  const Entry& entry = command_id_to_entry_.find(command_id)->second;
  return entry.first->IsItemUnderManagementAt(entry.second);
}

bool AppMenu::IsItemUnderFamilySafetyManagement(int command_id) const {
  if (command_id == 0)
    return false;  // The root item.

  const Entry& entry = command_id_to_entry_.find(command_id)->second;
  return entry.first->IsItemUnderFamilySafetyManagementAt(entry.second);
}
...
void AppMenu::ExecuteCommand(int command_id, int mouse_event_flags) {
  if (IsBookmarkCommand(command_id)) {
    UMA_HISTOGRAM_MEDIUM_TIMES("Microsoft.MoreMenu.TimeToAction.OpenBookmark",
                               menu_opened_timer_.Elapsed());
    UMA_HISTOGRAM_ENUMERATION("Microsoft.MoreMenu.MenuAction",
...
void AppMenu::WillShowMenu(MenuItemView* menu) {
  // Trigger usage of the Pinning Wizard feature when the "More tools" submenu
  // is shown. At this point, users will see the effects of the Pinning Wizard
  // feature being enabled or not (a menu item for the wizard will be shown
  // in the "More tools" menu conditional on the feature being enabled).
  // This can be removed once Pinning Wizard experiments are complete.
#if defined(MICROSOFT_EDGE_BUILD) && defined(OS_WIN)
  if (menu->GetCommand() == IDC_MORE_TOOLS_MENU) {
    base::FeatureList::TriggerUsage(features::edge::kPinningWizard);
  }
#endif
...
void AppMenu::PopulateMenu(MenuItemView* parent, MenuModel* model) {
  for (int i = 0, max = model->GetItemCount(); i < max; ++i) {
    ...
    switch (model->GetCommandIdAt(i)) {
      ...
      case IDC_APPS_MENU:
        DCHECK(!apps_sub_menu_model_delegate_.get());
        apps_sub_menu_model_delegate_.reset(
            new RecentTabsMenuModelDelegate(this, model->GetSubmenuModelAt(i),
                                            item));
        break;
      default:
