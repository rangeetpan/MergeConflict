...
#include "components/edge_share/core/common/edge_share_features.h"
...
#include "components/qbox/public/features.h"
...
>>>>>>> 3da0c646df3c1f822641c34d50501c4c2ee646c3
...
#include "components/edge_share/core/common/edge_enhanced_url_copy_paste_formats.h"
#include "components/edge_share/core/common/edge_share_features.h"
...
#endif
#if BUILDFLAG(ENABLE_DESKTOP_IN_PRODUCT_HELP_EDGE_NPS_PROMPT)
#include "chrome/browser/feature_engagement/feedbackrating/feedback_tracker.h"
#include "chrome/browser/feature_engagement/feedbackrating/feedback_tracker_factory.h"
#endif
...
OmniboxViewViews::OmniboxViewViews(OmniboxEditController* controller,
    ...
    : OmniboxView(controller, std::move(client)),
      ...
      e2e_latency_trace_state_(NOT_ACTIVE),
      friendly_suggestion_text_prefix_length_(0),
      scoped_compositor_observer_(this),
      scoped_template_url_service_observer_(this) {
  SetID(VIEW_ID_OMNIBOX);
  SetFontList(font_list);
  UpdateTextSelectionColors();
  // |location_bar_view_| can be null in unit_tests.
  edge_copy_paste_manager_ =
      std::make_unique<edge_share::EnhancedURLCopyPasteManager>(
          location_bar_view_ ? location_bar_view_->profile() : nullptr);
...
void OmniboxViewViews::Init() {
  SetDoesMeasureReadiness(
      base::FeatureList::IsEnabled(features::edge::kLogTextfieldLag, true));
  set_controller(this);
...
void OmniboxViewViews::OnPaint(gfx::Canvas* canvas) {
  if (!qbox::features::IsEdgeQBoxEnabled()) {
    if (latency_histogram_state_ == CHAR_TYPED) {
      ...
      const base::TimeDelta elapsed_time = now - insert_char_time_;

      TRACE_EVENT_END_WITH_BEGIN_TIMESTAMP1("omnibox",
          "Omnibox.CharTypedToRepaintLatency.ToPaint",insert_char_time_,
          "value", elapsed_time.InMillisecondsF());
    ...
    e2e_latency_trace_state_ = ON_PAINT_CALLED;
  }
...
ui::ImageModel OmniboxViewViews::GetIconForCommandId(int command_id) const {
  if (command_id == IDC_PASTE_AND_GO) {
    return ui::ImageModel::FromVectorIcon(vector_icons::kPasteSearchIcon,
        ui::NativeTheme::GetInstanceForNativeUi()->GetSystemColor(
            ui::NativeTheme::kColorId_EnabledMenuItemForegroundColor),
        views::MenuConfig::instance().icon_size);
  }
...
bool OmniboxViewViews::MaybeTriggerSecondaryButton(const ui::KeyEvent& event) {
  ...
  if (qbox::features::IsEdgeQBoxEnabled()) {
    if (event.key_code() == ui::VKEY_RETURN) {
      popup_view_->OpenMatch(selected_line,
        WindowOpenDisposition::SWITCH_TO_TAB, event.time_stamp());
      return true;
    }
    return false;
  }
...
void OmniboxViewViews::ResizePopup(int height, int width) {
  popup_view_->SetSize(height, width);
}

void OmniboxViewViews::OnPopupRendered(std::string time_cost) {
  popup_view_->OnUXRendered(time_cost);
}
...
base::string16 OmniboxViewViews::GetLabelForCommandId(int command_id) const {
  ...
  constexpr size_t kMaxSelectionTextLength = 35;
...
void OmniboxViewViews::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  ...
  node_data->SetLandmarkType(ax::mojom::LandmarkType::kSearch);
...
// Expose keyboard shortcut where it makes sense.
  ...
  if (model()->popup_model()->IsOpen()) {
    if (qbox::features::IsEdgeQBoxEnabled()) {
      content::WebContents* web_contents = popup_view_->GetQBoxWebContents();
      DCHECK(web_contents);
      ui::AXPlatformNodeDelegate* platform_node_delegate =
          ui::AXPlatformNode::FromNativeViewAccessible(
              web_contents->GetRenderWidgetHostView()
                  ->GetNativeViewAccessible())
              ->GetDelegate();
      DCHECK(platform_node_delegate->IsWebContent());
      // Aria roles in the web content should be set up so that the first child
      // in the accessibility tree is the "List" element.
      gfx::NativeViewAccessible first_child =
          platform_node_delegate->GetFirstChild();
      // The accessibility tree may not be set up yet. This gets called often so
      // we can always try again later.
      if (first_child) {
        node_data->AddIntListAttribute(
            ax::mojom::IntListAttribute::kControlsIds,
            {ui::AXPlatformNode::FromNativeViewAccessible(first_child)
                 ->GetUniqueId()});
      }
    } else {
  ...
  node_data->AddBoolAttribute(ax::mojom::BoolAttribute::kEditableRoot, true);
}
...
#if BUILDFLAG(ENABLE_DESKTOP_IN_PRODUCT_HELP_EDGE_NPS_PROMPT)
  if (location_bar_view_) {
    feature_engagement::FeedbackTrackerFactory::GetInstance()
          ->GetForProfile(location_bar_view_->profile())
          ->OnOmniboxFocused();
  }
#endif
...
void OmniboxViewViews::OnBlur() {
  ...
  if (base::FeatureList::IsEnabled(omnibox::kPreventOmniboxCloseOnBlur))
    popup_closes_on_blur = false;
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  if (command_id == IDC_CONTENT_CONTEXT_COPYASLINKPREVIEW &&
      features::IsLinkPreviewOmniboxEnabled()) {
    base::string16 selected_text = GetSelectedText();
    return model()->IsSelectedTextURL(GetSelectedRange().GetMin(),
                                      &selected_text);
  }
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
...
void OmniboxViewViews::DoInsertChar(base::char16 ch) {
  ...
  // Reset e2e start to the current time since we want to measure from the last
  // keystroke until the render is complete after all providers done
  e2e_latency_trace_state_ = CHAR_TYPED;
  e2e_insert_char_time_ = base::TimeTicks::Now();

  // In unit tests popup_view can be null
  if (popup_view_ != nullptr) {
    popup_view_->NotifyBeforeCharInsert();
  }
      ...
      if (MaybeTriggerSecondaryButton(event)) {
        ...
        if (model()->popup_model()->SelectedLineIsTabSwitchSuggestion()) {
          model()->AcceptInput(WindowOpenDisposition::SWITCH_TO_TAB,
                               event.time_stamp());
            edge::a11y::RaiseTabSwitchConfirmEvent(GetNativeViewAccessible());
        } else {
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
void OmniboxViewViews::WriteAdvancedHTMLToClipboard() {
  UMA_HISTOGRAM_COUNTS_1M("Microsoft.Omnibox.CopyPreviewLink", 1);
  base::string16 selected_text = GetSelectedText();
  GURL url;
  ...
  // Put advanced html on the clipboard
  advanced_clipboard_.reset(new ui::AdvancedClipBoard(url, true));
  base::string16 anchor_text;
  if (model()->client())
    anchor_text = model()->client()->GetTitle();
  advanced_clipboard_->DoAdvancedProcessing(
      base::UTF16ToUTF8(anchor_text),
...
void OmniboxViewViews::OnAfterCutOrCopy(ui::ClipboardBuffer clipboard_buffer) {
  ...
  scoped_clipboard_writer.WriteText(selected_text);
  // Preserved so as to reduce merge conflicts with upstream.
#if !defined(MICROSOFT_EDGE_BUILD)
  if (write_url &&
  ...
  std::string titled_hyperlink =
      edge_copy_paste_manager_.get()->CreateTitledHyperlink(
          &url, model()->client()->GetTitle());

  bool should_write_titled_hyperlink =
      write_url &&
      edge_share::features::edge::IsEdgeShareEnhancedCopyPasteEnabled() &&
      // TODO (dagitau, 24716694) Remove the URL check once EE is hooked up.
      // If these two URLs don't match, then the current tab's title doesn't
      // match the one we'd get if we visited |url|.
      url.spec() == model()->client()->GetURL().spec() &&
      !titled_hyperlink.empty();

  // Write to the "Titled Hyperlink Format" on the clipboard.
  if (should_write_titled_hyperlink) {
    edge_copy_paste_manager_.get()->WriteToClipboardTitledHyperlinkFormat(
        titled_hyperlink, url.spec(), &scoped_clipboard_writer);
  }

  edge_share::SupportedPasteFormats applicable_paste_formats =
      edge_copy_paste_manager_.get()->get_platform_supported_paste_formats();
  applicable_paste_formats.set(edge_share::PasteFormat::kTitledHyperlink,
                               should_write_titled_hyperlink);

#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  bool should_write_advanced_html =
      write_url && features::IsLinkPreviewOmniboxEnabled();
  base::string16 anchor_text;
  if (should_write_advanced_html) {
    advanced_clipboard_.reset(new ui::AdvancedClipBoard(url, true));
    // Determine anchor text to be used later
    if (model()->client()) {
      bool use_tab_title = url.spec() == model()->client()->GetURL().spec();
      anchor_text = use_tab_title ? model()->client()->GetTitle()
                                  : base::UTF8ToUTF16(url.spec());
    }
  }
  applicable_paste_formats.set(edge_share::PasteFormat::kWebCard,
                               should_write_advanced_html);
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)

  // Write to the HTML format on the clipboard.
  edge_share::PasteFormat format_for_html_slot =
      edge_copy_paste_manager_.get()->GetRecommendedFormatForHTMLSlot(
          applicable_paste_formats);
  switch (format_for_html_slot) {
    case edge_share::PasteFormat::kTitledHyperlink:
      if (should_write_titled_hyperlink) {
        edge_copy_paste_manager_.get()->WriteToClipboardHTMLFormat(
            base::UTF8ToUTF16(titled_hyperlink), url.spec(),
            &scoped_clipboard_writer);
      }
      break;

#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
    case edge_share::PasteFormat::kWebCard:
      if (should_write_advanced_html) {
        advanced_clipboard_->DoAdvancedProcessing(
            base::UTF16ToUTF8(anchor_text),
            ui::ClipboardFormatType::GetHtmlType().ToFormatEtc().cfFormat,
    ...
    default:
      if (write_url &&
          base::FeatureList::IsEnabled(kOmniboxCanCopyHyperlinksToClipboard)) {
        // Creates a hyperlink of the form |<a href="url">url</a>|
        scoped_clipboard_writer.WriteHyperlink(selected_text, url.spec());
      }
  }
  ...
  if (should_write_advanced_html) {
    ...
    advanced_clipboard_->DoAdvancedProcessing(
        base::UTF16ToUTF8(anchor_text),
        ui::ClipboardFormatType::GetLinkPreviewType().ToFormatEtc().cfFormat,
        &scoped_clipboard_writer);
  }
#endif  // BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  ...
  if (location_bar_view_ &&
      features::edge::IsCollectionsFeatureEnabled(features::edge::kEdgeCollections) &&
      !location_bar_view_->browser()->profile()->IsOffTheRecord()) {
    location_bar_view_->OnOmniboxUrlCutOrCopied(GetSelectedText(), url);
  }
#endif
}
...
#if BUILDFLAG(ENABLE_ADVANCED_CLIPBOARD)
  if (features::IsLinkPreviewOmniboxEnabled()) {
    int copy_index = menu_contents->GetIndexOfCommandId(IDS_APP_COPY);
    menu_contents->InsertItemWithStringIdAt(
        copy_index + 1, IDC_CONTENT_CONTEXT_COPYASLINKPREVIEW,
...
void OmniboxViewViews::OnCompositingDidCommit(ui::Compositor* compositor) {
  ...
  e2e_latency_trace_state_ = COMPOSITING_COMMIT;
}
...
void OmniboxViewViews::OnCompositingStarted(ui::Compositor* compositor,
  ...
  if (e2e_latency_trace_state_ == COMPOSITING_COMMIT)
    e2e_latency_trace_state_ = COMPOSITING_STARTED;
}
  ...
  if (!qbox::features::IsEdgeQBoxEnabled()) {
    ...
    if (latency_histogram_state_ == COMPOSITING_STARTED) {
      ...
      UMA_HISTOGRAM_TIMES("Omnibox.CharTypedToRepaintLatency",
                          elapsed_time);
      TRACE_EVENT_END_WITH_BEGIN_TIMESTAMP1("omnibox",
          "Omnibox.CharTypedToRepaintLatency", insert_char_time_,
          "value", elapsed_time.InMillisecondsF());
      insert_char_time_ = base::TimeTicks();
      latency_histogram_state_ = NOT_ACTIVE;
    }

    if (e2e_latency_trace_state_ == COMPOSITING_STARTED) {
      if (!e2e_insert_char_time_.is_null() &&
          model()->autocomplete_controller()->done()) {
        const base::TimeDelta elapsed_time = now - e2e_insert_char_time_;
        UMA_HISTOGRAM_TIMES(
            "Microsoft.Omnibox.CharTypedToRepaintLatency.AllProvidersDone",
            elapsed_time);
        TRACE_EVENT_END_WITH_BEGIN_TIMESTAMP1("omnibox",
            "Omnibox.CharTypedToRepaintLatency.AllProvidersDone",
            e2e_insert_char_time_, "value", elapsed_time.InMillisecondsF());
        e2e_insert_char_time_ = base::TimeTicks();
        e2e_latency_trace_state_ = NOT_ACTIVE;
      }
    }
  }
...
void OmniboxViewViews::UpdateTextSelectionColors() {
  if (location_bar_view_) {
    ui::NativeTheme* native_theme = ui::NativeTheme::GetInstanceForNativeUi();
    SetSelectionBackgroundColor(native_theme->GetSystemColor(
        ui::NativeTheme::ColorId::
            kColorId_TextfieldSelectionBackgroundFocused));
    SetSelectionTextColor(native_theme->GetSystemColor(
        ui::NativeTheme::ColorId::kColorId_TextfieldSelectionColor));
  }
}

bool OmniboxViewViews::GetAcceleratorForCommandId(int command_id,
    ui::Accelerator* accelerator) const {
  if (GetWidget()->GetAccelerator(command_id, accelerator)) {
    return true;
  }
  return views::Textfield::GetAcceleratorForCommandId(command_id, accelerator);
}

void OmniboxViewViews::OpenMatch(WindowOpenDisposition disposition,
                                 size_t selected_line,
                                 base::TimeTicks match_selection_timestamp) {
  popup_view_->OpenMatch(selected_line, disposition, match_selection_timestamp);
}
