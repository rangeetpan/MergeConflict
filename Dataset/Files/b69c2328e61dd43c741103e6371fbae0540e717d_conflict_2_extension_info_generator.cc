...
<<<<<<< HEAD
#include "extensions/common/edge_extensions_lab_helpers.h"
=======
...
void ExtensionInfoGenerator::CreateExtensionInfoHelper(
  ...
  switch (extension_prefs_->GetExtensionBlacklistState(extension.id())) {
    case BLACKLISTED_MALWARE:
      ...
      if (extension.from_microsoft_webstore())
        blacklist_text = IDS_EXTENSIONS_BLACKLISTED_MALWARE;
      else
        blacklist_text = IDS_EXTENSIONS_BLACKLISTED_MALWARE_FROM_CHROME_STORE;
#else
      blacklist_text = IDS_EXTENSIONS_BLACKLISTED_MALWARE;
#endif
      break;
...
#if MICROSOFT_EDGE_BUILD
      if (extension.from_microsoft_webstore())
        blacklist_text = IDS_EXTENSIONS_BLACKLISTED_EWS_POLICY_VIOLATION;
      else
  ...
  bool is_recommended_extension = !is_policy_location
      && management_policy->MustRemainInstalled(&extension, nullptr);

  if (is_policy_location || is_recommended_extension) {
    info->controlled_info.reset(new developer::ControlledInfo());

    // Recommended installs should behave like force installs. Except messages
    // should differentiate both.
    if (is_policy_location || is_recommended_extension) {
      info->controlled_info->is_recommended = is_recommended_extension;
      info->controlled_info->type = developer::CONTROLLER_TYPE_POLICY;
      info->controlled_info->text = info->controlled_info->is_recommended
          ? l10n_util::GetStringUTF8(
            IDS_EXTENSIONS_INSTALL_LOCATION_ENTERPRISE_RECOMMENDED)
          : l10n_util::GetStringUTF8(
            IDS_EXTENSIONS_INSTALL_LOCATION_ENTERPRISE);
      info->controlled_info->managed_location =
          ManifestURL::UpdatesFromGallery(&extension)
              ? developer::LOCATION_FROM_STORE
              : ManifestURL::UpdatesFromMicrosoftStoreGallery(&extension)
                    ? developer::LOCATION_FROM_MICROSOFT_STORE
  ...
  info->is_from_microsoft_extensions_lab =
      extensions_lab::IsExtensionsLabEnabled() &&
      extension.from_microsoft_extensions_lab();

  info->is_from_microsoft_store =
      !Manifest::IsUnpackedLocation(extension.location()) &&
      ManifestURL::UpdatesFromMicrosoftStoreGallery(&extension);
  ...
  if (extension.location() == Manifest::INTERNAL &&
    ...
    info->location = developer::LOCATION_THIRD_PARTY;
              ...
              extension.from_microsoft_webstore()) ||
             info->is_from_microsoft_extensions_lab) {
    info->location = developer::LOCATION_FROM_MICROSOFT_STORE;
  } else {
