...
<<<<<<< HEAD
#include "chrome/browser/mac/microsoft_edge/autoupdatehelper/mau_glue.h"
...
std::string UpdateStatusToString(VersionUpdater::Status status) {
  ...
  switch (status) {
    ...
    case VersionUpdater::UPDATING:
      status_str = "updating";
      break;
    case VersionUpdater::UPDATER_RUNNING:
      status_str = "updater_running";
      break;
    ...
    case VersionUpdater::DISABLED_BY_ADMIN:
      ...
      break;
    case VersionUpdater::DISABLED_BY_APPLICATION_GUARD:
      status_str = "disabled_by_application_guard";
      break;
    case VersionUpdater::NEED_PERMISSION_TO_UPDATE:
      status_str = "need_permission_to_update";
      break;
    case VersionUpdater::UPDATES_BLOCKED_BY_EXTERNAL_PROCESS:
      status_str = "updates_blocked_by_external_process";
      break;
  }
...
AboutHandler* AboutHandler::Create(content::WebUIDataSource* html_source,
  ...
  html_source->AddLocalizedString("aboutProductLicenseTemplate", IDS_VERSION_UI_LICENSE_TEMPLATE);
  html_source->AddString("chromiumProjectUrl", base::ASCIIToUTF16(chrome::kChromiumProjectURL));
...
#if BUILDFLAG(GOOGLE_CHROME_BRANDING) || defined(MICROSOFT_EDGE_BUILD)
  base::string16 tos = l10n_util::GetStringFUTF16(
      IDS_VERSION_UI_PRIVACY_AGREEMENTS,
      base::UTF8ToUTF16(chrome::kChromeUITermsURL),
      base::UTF8ToUTF16(chrome::kEdgePrivacyStatementURL),
      base::UTF8ToUTF16(chrome::kEdgeMicrosoftServicesAgreementURL));
  html_source->AddString("aboutProductTos", tos);
  html_source->AddLocalizedString("aboutProductTosTemplate", IDS_VERSION_UI_PRIVACY_AGREEMENTS_TEMPLATE);
  html_source->AddLocalizedString("aboutTermsOfUseLinkText", IDS_VERSION_UI_PRIVACY_TERMS_OF_USE_LINK_TITLE);
  html_source->AddString("aboutTermsOfUseLinkUrl", base::UTF8ToUTF16(chrome::kChromeUITermsURL));
  html_source->AddLocalizedString("aboutPrivacyStatementLinkText", IDS_VERSION_UI_PRIVACY_STATEMENT_LINK_TITLE);
  html_source->AddString("aboutPrivacyStatementLinkUrl", base::UTF8ToUTF16(chrome::kEdgePrivacyStatementURL));
  html_source->AddLocalizedString("aboutServicesAgreementLinkText", IDS_VERSION_UI_SERVICES_AGREEMENT_LINK_TITLE);
  html_source->AddString("aboutServicesAgreementLinkUrl", base::UTF8ToUTF16(chrome::kEdgeMicrosoftServicesAgreementURL));
#endif
...
void AboutHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      "promoteUpdater", base::BindRepeating(&AboutHandler::PromoteUpdater,
                                            base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "openMicrosoftAutoupdate",
      base::BindRepeating(&AboutHandler::OpenMAUGUI, base::Unretained(this)));
#endif
...
void AboutHandler::OpenMAUGUI(const base::ListValue* args) {
  version_updater_->OpenMAUGUI();
}
#endif
