...
>>>>>>> 71f2c4f5c7db50f40f9d9a881cd88ea0128a4b10
...
void PasswordFormMetricsRecorder::RecordFirstWaitForUsernameReason(
  ...
  if (recorded_wait_for_username_reason_)
    return;
  // Set |recorded_first_default_suggestion_shown_| to true which is set when
  // logging related to DEFAULT_SUGGESTION_SHOWN is done. If the renderer has to
  // wait for user to confirm the credentials to be filled, we do not want
  // DEAFULT_SUGGESTION_SHOWN logging to be done and hence mark bool related to
  // logging as true showing that it has already been done.
  if (reason != WaitForUsernameReason::kDontWait) {
    recorded_first_default_suggestion_shown_ = true;
  }
...
void PasswordFormMetricsRecorder::RecordFillEvent(ManagerAutofillEvent event) {
  ...
  if (!recorded_first_default_suggestion_shown_ &&
      event == ManagerAutofillEvent::kManagerFillEventAutofilled) {
