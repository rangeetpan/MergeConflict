...
IN_PROC_BROWSER_TEST_F(RenderWidgetHostViewChildFrameBrowserTest,
                       GetWindowSegmentsFromChildFrame) {
  ...
  FrameTreeNode* root = static_cast<WebContentsImpl*>(shell()->web_contents())
                            ->GetFrameTree()
                            ->root();
  RenderWidgetHostViewBase* rwhv = root->child_at(0)
                                       ->current_frame_host()
                                       ->GetRenderWidgetHost()
                                       ->GetView()
                                       ->GetRootView();

  gfx::Rect root_view_rect = rwhv->GetViewBounds();

  int segment_width =
      ExecuteScriptAndGetValue(root->child_at(0)->current_frame_host(),
                               "window.getWindowSegments()[0].width")
          .GetInt();
  int segment_height =
      ExecuteScriptAndGetValue(root->child_at(0)->current_frame_host(),
                               "window.getWindowSegments()[0].height")
          .GetInt();

  EXPECT_EQ(root_view_rect.width(), segment_width);
  EXPECT_EQ(root_view_rect.height(), segment_height);

  int segment_width_from_root =
      ExecuteScriptAndGetValue(root->current_frame_host(),
                               "window.getWindowSegments()[0].width")
          .GetInt();
  int segment_height_from_root =
      ExecuteScriptAndGetValue(root->current_frame_host(),
                               "window.getWindowSegments()[0].height")
          .GetInt();

  EXPECT_EQ(root_view_rect.width(), segment_width_from_root);
  EXPECT_EQ(root_view_rect.height(), segment_height_from_root);
}
