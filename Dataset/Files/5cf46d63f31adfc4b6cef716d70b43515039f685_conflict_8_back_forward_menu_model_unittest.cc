...
class BackFwdMenuModelTest : public ChromeRenderViewHostTestHarness,
 ...
 public:
  ...
  bool GetAcceleratorForCommandId(int command_id,
    ...
    if (command_id == show_history_accelerator.command_id) {
      *accelerator = ui::Accelerator(show_history_accelerator.keycode,
                                     show_history_accelerator.modifiers);
      return true;
    }
    return false;
  }

  void ValidateAccelerator(ui::Accelerator accelerator,
                           AcceleratorMapping accelerator_mapping) {
    EXPECT_TRUE((accelerator.key_code() == accelerator_mapping.keycode) &&
                (accelerator.modifiers() == accelerator_mapping.modifiers));
  }

  AcceleratorMapping show_history_accelerator = {
...
// Test that "Show Full History" is at the correct index and has the correct
// accelerator (Command+Y on Mac, CTRL+H otherwise).
TEST_F(BackFwdMenuModelTest, ShowFullHistoryTest) {
  std::unique_ptr<BackForwardMenuModel> back_model(new BackForwardMenuModel(
      nullptr, BackForwardMenuModel::ModelType::kBackward));
  back_model->set_test_web_contents(web_contents());
  ...
  // There're two more items here: a separator and a "Show Full History".
  EXPECT_EQ(4, back_model->GetItemCount());
  EXPECT_EQ(0, forward_model->GetItemCount());
  EXPECT_EQ(back_model->GetShowFullHistoryLabel(), back_model->GetLabelAt(3));

  ui::Accelerator back_accelerator;
  back_model->GetAcceleratorAt(3, &back_accelerator);
  ValidateAccelerator(back_accelerator, show_history_accelerator);

  NavigateToOffset(-2);

  EXPECT_EQ(0, back_model->GetItemCount());
  EXPECT_EQ(4, forward_model->GetItemCount());
  EXPECT_EQ(forward_model->GetShowFullHistoryLabel(),
            forward_model->GetLabelAt(3));

  ui::Accelerator forward_accelerator;
  forward_model->GetAcceleratorAt(3, &forward_accelerator);
  ValidateAccelerator(forward_accelerator, show_history_accelerator);
}
