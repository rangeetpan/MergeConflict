...
=======
>>>>>>> ec85d275b68b47a80a329009a35080e345f46176
#include "base/no_destructor.h"
#include "base/notreached.h"
#include "base/strings/string_util.h"
#include "base/strings/utf_string_conversions.h"
#include "base/win/redirected_location_util.h"
...
OSInfo* OSInfo::GetInstanceWithRtlGetVersion() {
  static OSInfo* info = []() {
    using PFN_RTL_GET_VERSION = void(WINAPI*)(OSVERSIONINFOEXW*);
    base::NativeLibrary ntdll_module =
        base::LoadNativeLibrary(base::FilePath(L"ntdll.dll"), nullptr);
    if (ntdll_module != nullptr) {
      auto rtl_get_version_func = reinterpret_cast<PFN_RTL_GET_VERSION>(
          base::GetFunctionPointerFromNativeLibrary(ntdll_module,
                                                    "RtlGetVersion"));
      if (rtl_get_version_func != nullptr) {
        _OSVERSIONINFOEXW version_info = {sizeof(version_info)};
        rtl_get_version_func(&version_info);

        DWORD os_type = 0;
        ::GetProductInfo(version_info.dwMajorVersion,
...
OSInfo::OSInfo(const _OSVERSIONINFOEXW& version_info,
    ...
    : version_(Version::PRE_XP),
      host_architecture_(GetHostArchForProcess(GetCurrentProcess())),
...
std::string OSInfo::insider_ring() {
  static constexpr wchar_t kRegKeyInsiderRingDefaultPath[] =
      L"SOFTWARE\\Microsoft\\WindowsSelfHost\\Applicability";
  static constexpr wchar_t kRegKeyInsiderRingKey[] = L"Applicability";
  static constexpr wchar_t kRegKeyInsiderRingValue[] = L"Ring";
  const size_t kMaxStringLength = 2014;
  wchar_t value[kMaxStringLength];
  LONG result = base::win::GetPersistedRegistryValue(
    kRegKeyInsiderRingKey, kRegKeyInsiderRingDefaultPath, kRegKeyInsiderRingValue,
    RRF_RT_REG_SZ, &value, sizeof(value));
  if (result == ERROR_SUCCESS)
    return UTF16ToUTF8(value);
  return "";
}

bool OSInfo::is_host_arm() {
  switch (host_architecture_) {
    case ARM_ARCHITECTURE:
...
OSInfo::WindowsArchitecture MachineTypeToArchitecture(USHORT machineType) {
  switch (machineType) {
    case IMAGE_FILE_MACHINE_I386:
      return OSInfo::X86_ARCHITECTURE;

    case IMAGE_FILE_MACHINE_AMD64:
      return OSInfo::X64_ARCHITECTURE;

    case IMAGE_FILE_MACHINE_IA64:
      return OSInfo::IA64_ARCHITECTURE;

    case IMAGE_FILE_MACHINE_ARM:
    case IMAGE_FILE_MACHINE_THUMB:
    case IMAGE_FILE_MACHINE_ARMNT:
      return OSInfo::ARM_ARCHITECTURE;

    case IMAGE_FILE_MACHINE_ARM64:
      return OSInfo::ARM64_ARCHITECTURE;

    default:
      return OSInfo::OTHER_ARCHITECTURE;
  }
}

// static
OSInfo::WindowsArchitecture OSInfo::GetHostArchForProcess(
    HANDLE process_handle) {
  OSInfo::WindowsArchitecture host_architecture = OSInfo::OTHER_ARCHITECTURE;

  // 'IsWow64Process2' is only available for Win10+
  HMODULE handle = GetModuleHandleW(L"api-ms-win-core-wow64-l1-1-1.dll");
  if (handle != nullptr) {
    typedef BOOL (WINAPI* IsWow64Process2Func)(HANDLE, USHORT*, USHORT*);
    IsWow64Process2Func is_wow64_process2 =
        reinterpret_cast<IsWow64Process2Func>(
            GetProcAddress(handle, "IsWow64Process2"));

    if (is_wow64_process2 != nullptr) {
      USHORT processMachine;
      USHORT nativeMachine;
      if ((*is_wow64_process2)(process_handle,
          &processMachine, &nativeMachine)) {
        host_architecture = MachineTypeToArchitecture(nativeMachine);
      }
    }
  }
