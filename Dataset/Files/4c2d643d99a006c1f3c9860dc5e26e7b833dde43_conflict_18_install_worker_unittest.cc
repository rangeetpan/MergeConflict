...
class MockWorkItemList : public WorkItemList {
 public:
  ...
  MOCK_METHOD6(AddCopyTreeWorkItem,
...
TEST_F(InstallWorkerTest,
       EDGE_DISABLED_ARM64_PENDING(TestInstallChromeSystem, 23741519)) {
  ...
  EXPECT_CALL(work_item_list, AddCopyTreeWorkItem(_, _, _, _, _, _))
      .Times(AtLeast(1));
...
void TestAddNeedEdgeWorkItemsWorker(base::Version* current_version,
                                    bool system_level,
                                    bool install) {
  NiceMock<MockWorkItemList> work_item_list;

  std::unique_ptr<InstallationState> installation_state(
      BuildChromeInstallationState(system_level, current_version));

  std::unique_ptr<InstallerState> installer_state(BuildChromeInstallerState(
      system_level, *installation_state,
      install ? InstallerState::SINGLE_INSTALL_OR_UPDATE
              : InstallerState::UNINSTALL));

  const HKEY kRegRoot = system_level ? HKEY_LOCAL_MACHINE : HKEY_CURRENT_USER;

  const std::wstring ie_enterprise_mode_key_path =
      LR"(Software\Microsoft\Internet Explorer\Main\EnterpriseMode)";
  const std::wstring ms_edge_path_value_name = L"MSEdgePath";
  const std::wstring target_path = installer_state->target_path().value();

  auto mock_work_item = std::make_unique<StrictMockWorkItem>();

  if (install) {
    EXPECT_CALL(work_item_list,
                AddCreateRegKeyWorkItem(kRegRoot, ie_enterprise_mode_key_path,
                                        KEY_WOW64_64KEY))
        .Times(1)
        .WillOnce(Return(mock_work_item.get()));
    EXPECT_CALL(work_item_list,
                AddSetRegStringValueWorkItem(
                    kRegRoot, ie_enterprise_mode_key_path, KEY_WOW64_64KEY,
                    ms_edge_path_value_name, target_path, true))
        .Times(1)
        .WillOnce(Return(mock_work_item.get()));
  } else {
    EXPECT_CALL(work_item_list, AddDeleteRegValueWorkItem(
                                    kRegRoot, ie_enterprise_mode_key_path,
                                    KEY_WOW64_64KEY, ms_edge_path_value_name))
        .Times(1)
        .WillOnce(Return(mock_work_item.get()));
  }

  AddNeedEdgeWorkItems(*installer_state, &work_item_list);

  ASSERT_TRUE(mock_work_item->best_effort());
}

TEST_F(InstallWorkerTest, TestAddNeedEdgeWorkItemsUserInstall) {
  TestAddNeedEdgeWorkItemsWorker(current_version_.get(), false, true);
}

TEST_F(InstallWorkerTest, TestAddNeedEdgeWorkItemsUserUninstall) {
  TestAddNeedEdgeWorkItemsWorker(current_version_.get(), false, false);
}

TEST_F(InstallWorkerTest, TestAddNeedEdgeWorkItemsSystemInstall) {
  TestAddNeedEdgeWorkItemsWorker(current_version_.get(), true, true);
}

TEST_F(InstallWorkerTest, TestAddNeedEdgeWorkItemsSystemUninstall) {
  TestAddNeedEdgeWorkItemsWorker(current_version_.get(), true, false);
}

void TestAddInternetExplorerEdgeIntegrationWorkItems(
    base::Version* current_version,
    bool install) {
  NiceMock<MockWorkItemList> work_item_list;

  std::unique_ptr<InstallationState> installation_state(
      BuildChromeInstallationState(true, current_version));

  std::unique_ptr<InstallerState> installer_state(BuildChromeInstallerState(
      true, *installation_state,
      install ? InstallerState::SINGLE_INSTALL_OR_UPDATE
              : InstallerState::UNINSTALL));

  const std::wstring ie_edge_integration_path =
    LR"(Software\Microsoft\Internet Explorer\EdgeIntegration\AdapterLocations)";
  const std::wstring target_path = installer_state->target_path().value();

  if (install) {
    EXPECT_CALL(work_item_list, AddCreateRegKeyWorkItem(
                                    HKEY_LOCAL_MACHINE,
                                    ie_edge_integration_path, KEY_WOW64_64KEY))
        .Times(1);
    EXPECT_CALL(work_item_list,
                AddSetRegDwordValueWorkItem(
                    HKEY_LOCAL_MACHINE, ie_edge_integration_path,
                    KEY_WOW64_64KEY, target_path, 1UL, true))
        .Times(1);
#if defined(ARCH_CPU_ARM64)
    EXPECT_CALL(work_item_list, AddCreateRegKeyWorkItem(
                                    HKEY_LOCAL_MACHINE,
                                    ie_edge_integration_path, KEY_WOW64_32KEY))
        .Times(1);
    EXPECT_CALL(work_item_list,
                AddSetRegDwordValueWorkItem(
                    HKEY_LOCAL_MACHINE, ie_edge_integration_path,
                    KEY_WOW64_32KEY, target_path, 1UL, true))
        .Times(1);
#endif
  } else {
    EXPECT_CALL(
        work_item_list,
        AddDeleteRegValueWorkItem(HKEY_LOCAL_MACHINE, ie_edge_integration_path,
                                  KEY_WOW64_64KEY, target_path))
        .Times(1);
#if defined(ARCH_CPU_ARM64)
    EXPECT_CALL(
        work_item_list,
        AddDeleteRegValueWorkItem(HKEY_LOCAL_MACHINE, ie_edge_integration_path,
                                  KEY_WOW64_32KEY, target_path))
        .Times(1);
#endif
  }

  AddInternetExplorerEdgeIntegrationWorkItems(*installer_state,
                                              &work_item_list);
}

TEST_F(InstallWorkerTest,
       TestAddInternetExplorerEdgeIntegrationWorkItemsSystemInstall) {
  TestAddInternetExplorerEdgeIntegrationWorkItems(current_version_.get(), true);
}

TEST_F(InstallWorkerTest,
       TestAddInternetExplorerEdgeIntegrationWorkItemsSystemUninstall) {
  TestAddInternetExplorerEdgeIntegrationWorkItems(current_version_.get(),
                                                  false);
}

class BrowserReplacementWorkItems : public testing::Test {
 protected:
  BrowserReplacementWorkItems()
      : scoped_install_details_(/* system install */ true) {}
 ...
 protected:
  void TestAddBrowserReplacementWorkItems(
      base::Version* current_version,
      bool install,
      int should_edge_uwp_remain_enabled_policy_value,
      int integration_level,
      bool expect_to_write_value,
      bool expect_to_delete_value,
      bool expect_to_write_command,
      bool expect_to_delete_command,
      installer::BrowserReplacementStatusResult expected_histogram_result);
...
void BrowserReplacementWorkItems::TestAddBrowserReplacementWorkItems(
    base::Version* current_version,
    bool install,
    int should_edge_uwp_remain_enabled_policy_value,
    int integration_level,
    ...
    bool expect_to_write_command,
    bool expect_to_delete_command,
    installer::BrowserReplacementStatusResult
        expected_histogram_result_RS2_and_up =
            installer::BrowserReplacementStatusResult::VersionTooLow) {
  NiceMock<MockWorkItemList> work_item_list;
  base::HistogramTester histogram_tester;

  // Create our own enlightenment key.
  base::win::RegKey key;
  ASSERT_EQ(ERROR_SUCCESS,
            key.Create(HKEY_LOCAL_MACHINE, installer::kWindowsMicrosoftEdgeKey,
                       KEY_ALL_ACCESS));
  // Set the OSIntegration_level at the indicated level.
  EXPECT_EQ(ERROR_SUCCESS,
            key.WriteValue(installer::kEdgeEnlightenmentRegKeyValue,
                           integration_level));

  // Create policy key.
  base::win::RegKey policyKey;
  ASSERT_EQ(ERROR_SUCCESS, policyKey.Create(HKEY_LOCAL_MACHINE,
                                            installer::kEdgeUpdatePoliciesKey,
                                            KEY_ALL_ACCESS));
  // Set the Policy to indicate whether Edge UWP should remain (or, by default,
  // be hidden) at the indicated value.
  EXPECT_EQ(ERROR_SUCCESS,
            policyKey.WriteValue(installer::kShouldEdgeUWPRemainEnabledValue,
                                 should_edge_uwp_remain_enabled_policy_value));
  ...
  const std::wstring update_state_key(install_static::GetClientStateKeyPath());
  const std::wstring value(installer::kBrowserReplacement);
  const std::wstring command_path(
      installer::GetCommandKey(installer::kCmdFinishBrowserReplacement));

  std::unique_ptr<CreateRegKeyWorkItem> create_reg_key_work_item(
      WorkItem::CreateCreateRegKeyWorkItem(HKEY_LOCAL_MACHINE, command_path,
                                           KEY_WOW64_32KEY));

  std::unique_ptr<SetRegValueWorkItem> set_reg_value_work_item(
      WorkItem::CreateSetRegValueWorkItem(HKEY_LOCAL_MACHINE, command_path,
                                          KEY_WOW64_32KEY, L"", L"", false));

  std::unique_ptr<DeleteRegKeyWorkItem> delete_reg_key_work_item(
      WorkItem::CreateDeleteRegKeyWorkItem(HKEY_LOCAL_MACHINE, command_path,
                                           KEY_WOW64_32KEY));

  // None of this happens lower than RS2.
  if (base::win::GetVersion() > base::win::Version::WIN10_RS2) {
    if (expect_to_write_value) {
      EXPECT_CALL(work_item_list,
                  AddSetRegDwordValueWorkItem(
                      HKEY_LOCAL_MACHINE, update_state_key, KEY_WOW64_32KEY,
                      value, 1UL, true))
          .Times(1);
    }

    if (expect_to_write_command) {
      EXPECT_CALL(work_item_list,
                  AddCreateRegKeyWorkItem(HKEY_LOCAL_MACHINE, command_path,
                                          KEY_WOW64_32KEY))
          .Times(1)
    ...
    if (expect_to_delete_value) {
      // Expect removal of the "browser replacement" key
      EXPECT_CALL(work_item_list, AddDeleteRegValueWorkItem(
                                      HKEY_LOCAL_MACHINE, update_state_key,
                                      KEY_WOW64_32KEY, value))
          .Times(1);
    }

    if (expect_to_delete_command) {
      EXPECT_CALL(work_item_list,
                  AddDeleteRegKeyWorkItem(HKEY_LOCAL_MACHINE, command_path,
                                          KEY_WOW64_32KEY))
          .WillOnce(Return(delete_reg_key_work_item.get()));
    }
  }

  base::FilePath fake_setup_path = temp_dir_.GetPath().Append(L"setup.exe");
  AddEdgeUWPReplacementWorkItems(*installer_state, fake_setup_path,
                                 *current_version_.get(), &work_item_list);

  // Test histogram outcomes
  constexpr const char kHistogramName[] =
      "Microsoft.Setup.Install.BrowserReplacementStatus";
  if (install) {
    if (should_edge_uwp_remain_enabled_policy_value != 0) {
      // Ensure the policy being set is recorded by histograms
      histogram_tester.ExpectBucketCount(
          kHistogramName,
          installer::BrowserReplacementStatusResult::PreventedByPolicy, 1);
    } else if (base::win::GetVersion() <= base::win::Version::WIN10_RS2) {
      // Make sure RS2 and below fired the "version too low" histogram.
      histogram_tester.ExpectBucketCount(
          kHistogramName,
          installer::BrowserReplacementStatusResult::VersionTooLow, 1);
    } else {
      histogram_tester.ExpectBucketCount(
          kHistogramName,
          expected_histogram_result_RS2_and_up, 1);
    }
  } else {
    // Make sure nothing fires for uninstall.
    histogram_tester.ExpectTotalCount(kHistogramName, 0);
  }
}

TEST_F(BrowserReplacementWorkItems, TestAddBrowserReplacementWorkItemsInstall) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), true /*install*/,
      0 /*should_edge_uwp_remain_enabled_policy_value*/,
      3 /*integration_level*/, true /*expect_to_write_value*/,
      false /*expect_to_delete_value*/, false /*expect_to_write_command */,
      false /*expect_to_delete_command */,
      installer::BrowserReplacementStatusResult::Triggered);
}

TEST_F(BrowserReplacementWorkItems,
       TestAddBrowserReplacementWorkItemsUninstall) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), false /*install*/,
      0 /*should_edge_uwp_remain_enabled_policy_value*/,
      3 /*integration_level*/, false /*expect_to_write_value*/,
      true /*expect_to_delete_value*/, false /*expect_to_write_command */,
      false /*expect_to_delete_command */);
}

TEST_F(BrowserReplacementWorkItems,
       TestAddBrowserReplacementWorkItemsInstallUnenlightened) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), true /*install*/,
      0 /*should_edge_uwp_remain_enabled_policy_value*/,
      2 /*integration_level*/, false /*expect_to_write_value*/,
      false /*expect_to_delete_value*/, true /*expect_to_write_command */,
      false /*expect_to_delete_command */,
      installer::BrowserReplacementStatusResult::ReplacementUnavailable);
}

TEST_F(BrowserReplacementWorkItems,
       TestAddBrowserReplacementWorkItemsUninstallUnenlightened) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), false /*install*/,
      0 /*should_edge_uwp_remain_enabled_policy_value*/,
      2 /*integration_level*/, false /*expect_to_write_value*/,
      true /*expect_to_delete_value*/, false /*expect_to_write_command */,
      true /*expect_to_delete_command */);
}

TEST_F(
    BrowserReplacementWorkItems,
    TestAddBrowserReplacementWorkItemsUninstallUnenlightenedEdgeUWPRemainsByPolicy) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), false /*install*/,
      1 /*should_edge_uwp_remain_enabled_policy_value*/,
      2 /*integration_level*/, false /*expect_to_write_value*/,
      true /*expect_to_delete_value*/, false /*expect_to_write_command */,
      true /*expect_to_delete_command */);
}
...
TEST_F(
    BrowserReplacementWorkItems,
    TestAddBrowserReplacementWorkItemsInstallUnenlightenedEdgeUWPRemainsByPolicy) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), true /*install*/,
      1 /*should_edge_uwp_remain_enabled_policy_value*/,
      2 /*integration_level*/, false /*expect_to_write_value*/,
...
TEST_F(BrowserReplacementWorkItems,
       TestAddBrowserReplacementWorkItemsInstallEdgeUWPRemainsByPolicy) {
  TestAddBrowserReplacementWorkItems(
      current_version_.get(), true /*install*/,
      2 /*should_edge_uwp_remain_enabled_policy_value*/,
      3 /*integration_level*/, false /*expect_to_write_value*/,
      false /*expect_to_delete_value*/, false /*expect_to_write_command */,
      false /*expect_to_delete_command */);
}
