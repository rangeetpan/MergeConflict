...
>>>>>>> 7c6f60b6697c91f4a8d2fa9be30fe68b5005845d
...
bool AutofillProfileComparator::IsEdgeMergeableProfile(
    const AutofillProfile& p1,
    const AutofillProfile& p2) const {
  // Check if email is present in both profiles
  ...
  if (!email_1.empty() && !email_2.empty())
    return true;

  // Check if phone numbers are present in both profiles
  ...
  // Check if names are present in both profiles
  const base::string16 full_name_1 = p1.GetInfo(NAME_FULL, app_locale_);
  const base::string16 full_name_2 = p2.GetInfo(NAME_FULL, app_locale_);
  if (!full_name_1.empty() && !full_name_2.empty())
    return true;

  // Check if addresses are present in both profiles
  const base::string16& address1 =
      p1.GetInfo(ADDRESS_HOME_STREET_ADDRESS, app_locale_);
  const base::string16& address2 =
...
bool AutofillProfileComparator::AreMergeable(const AutofillProfile& p1,
  ...
  if (base::FeatureList::IsEnabled(
          features::kAutofillEdgeReduceProfileSaveRestrictions) &&
      !IsEdgeMergeableProfile(p1, p2))
    return false;
...
std::string AutofillProfileComparator::MergeProfile(
  ...
  size_t merged_profile_index = merged_profiles->size();

  // If we have already saved this address, merge in any missing values.
  // Merging the new profile into the existing ones preserves the validity of
  // credit card's billing address reference.
  ...
  for (auto& existing_profile : existing_profile_copies) {
    if (!matching_profile_found &&
      ...
      merged_profile_index = merged_profiles->size() - 1;
    } else if (base::FeatureList::IsEnabled(
                   features::kAutofillEdgeReduceProfileSaveRestrictions) &&
               matching_profile_found &&
               comparator.AreMergeable(
                   merged_profiles->at(merged_profile_index),
                   existing_profile) &&
               merged_profiles->at(merged_profile_index)
                   .SaveAdditionalInfo(existing_profile, app_locale)) {
      merged_profiles->at(merged_profile_index)
