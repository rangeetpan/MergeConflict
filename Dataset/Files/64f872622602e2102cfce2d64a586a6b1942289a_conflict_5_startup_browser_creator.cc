...
#include <limits>
=======
>>>>>>> 8cb7a34d148b265eead7ee688097b24c6e31ad6a
#include <set>
#include <string>
...
#include "chrome/browser/edge_personalized_data_collection/personalized_data_collection_request_helper.h"
#endif  // MICROSOFT_EDGE_BUILD
#include "chrome/browser/edge_pinning_wizard_features.h"
...
#include "chrome/browser/ui/edge_fre/edge_fre_browser_list_observer.h"
#include "chrome/browser/ui/startup/edgeuwp_migration.h"
#include "chrome/browser/ui/startup/startup_browser_creator_impl.h"
#include "chrome/browser/ui/views/edge_fre/edge_fre_modal_view.h"
...
// Edge
#include "chrome/browser/profiles/edge_profiles_state.h"
#include "chrome/browser/signin/signin_util.h"
#include "chrome/browser/ui/edge_force_sign_in_user_manager.h"
...
#include "chrome/browser/edge_fre/auto_update_consent_helper_win.h"
#include "chrome/browser/extensions/extension_service.h"
#include "chrome/browser/metrics/jumplist_metrics_win.h"
#include "chrome/browser/notifications/win/notification_launch_id.h"
#include "chrome/browser/ui/webui/settings/reset_settings_handler.h"
#include "chrome/credential_provider/common/gcp_strings.h"
#include "components/edge_command_line/edgeuwp_argument_parser_win.h"
...
#include "chrome/browser/edge_embedded_browser/connection.h"
#endif

#if BUILDFLAG(ENABLE_WDAG)
#include "chrome/browser/win/wdag/manager.h"
#include "chrome/browser/win/wdag/utilities.h"
#include "components/telemetry_client/wdag_telemetry_client.h"
#include "components/wdag_win/configuration.h"
#include "components/wdag_win/hvsi_flare.h"
#include "components/wdag_win/switches.h"
#endif
...
bool CanOpenProfileOnStartup(Profile* profile) {
  ...
  if (g_browser_process->profile_manager()
    ...
    NOTREACHED() << "Profile locking is not implemented for "
                    "startup_browser_creator. Code will need to be modified as "
                    "locking causes unknown behavior.";
    return false;
  }
...
#if BUILDFLAG(ENABLE_WDAG)
  if (wdag::IsHostInstance()) {
    if (command_line.HasSwitch(wdag::switches::kWdagContainerNavigation) &&
...
bool StartupBrowserCreator::LaunchBrowser(
...
#if defined(OS_WIN)
  // Launch the FRE window if the user has not consented for auto updates.
  // This is explicit for down level windows(Win 7/8/8.1).
  if (!AutoUpdateConsentHelper::HasUserConsentedForAutoUpdate() &&
      edge::fre::EdgeFREViewModal::IsShowing()) {
    edge::fre::EdgeFREViewModal::GetInstance()->GetWidget()->Activate();
    return true;
  }
#endif

  if (signin_util::IsEdgeForceSigninEnabled() &&
      edge_force_sign_in_user_manager::HandleForceSigninAtStartup(
  ...
  if (base::FeatureList::IsEnabled(features::edge::kPinningWizard)) {
    if (g_browser_process->local_state()->GetBoolean(
            prefs::kPinningWizardAllowed)) {
      // Handle microsoft-edge protocol with &pinningWizard query parameter by
      // launching the pinning wizard.
      const base::CommandLine::StringVector& params = command_line.GetArgs();
      for (size_t i = 0; i < params.size(); ++i) {
        base::FilePath param = base::FilePath(params[i]);
        base::string16 protocol_command = param.AsUTF16Unsafe();
        GURL url = GURL(protocol_command);
        if (url.SchemeIs(EdgeUwpArgumentParser::kEdgeProtocol)) {
          std::size_t found = protocol_command.find(L"&pinningWizard=true");
            ...
            edge_pinning_wizard_telemetry::EdgePinningWizardTelemetry::
                FirePinningWizardLaunchHistogram(
                    edge_pinning_wizard_telemetry::PinningWizardEntryPoint::
                        protocolLaunch);
            chrome::microsoft_edge::ShowPinningWizardForProfile(profile);
            return true;
          }
        }
      }
  ...
  if (!first_run::IsChromeFirstRun() &&
      IsPersonalizedDataCollectionReconsentFeatureEnabled()) {
      PersonalizedDataCollectionRequest::MaybeShowDialogAfterBrowserStart(
          profile);
    }

    BrowserList::AddObserver(edge::fre::FreBrowserListObserver::Get());
    return true;
}
...
std::vector<GURL> StartupBrowserCreator::GetURLsFromCommandLine(
  ...
  for (size_t i = 0; i < params.size(); ++i) {
    ...
    if (url.SchemeIs(EdgeUwpArgumentParser::kEdgeProtocol)) {
      ...
      EdgeUwpArgumentParser::HandleMicrosoftEdgeProtocolCommandLine(
          param.AsUTF16Unsafe(), &result);
      url = GURL(result);
      // Only allow http and https to reduce the attack surface.
      if (!url.is_valid() || !(url.SchemeIs(url::kHttpsScheme) ||
                               url.SchemeIs(url::kHttpScheme))) {
        url = GURL(GetDefaultSearchURLForSearchTerms(
            TemplateURLServiceFactory::GetForProfile(profile), result));
      }
    }
...
bool StartupBrowserCreator::ProcessCmdLineImpl(
...
#if defined(OS_WIN)
  // If the Pinning Wizard launch command line option is present (and the
  // Pinning Wizard is allowed by policy), just launch the Pinning Wizard and
  // don't show a browser window.
  if (base::FeatureList::IsEnabled(features::edge::kPinningWizard)) {
    if (g_browser_process->local_state()->GetBoolean(
            prefs::kPinningWizardAllowed)) {
      if (command_line.HasSwitch(switches::kPinningWizard)) {
        chrome::microsoft_edge::ShowPinningWizardForProfile(last_used_profile);
        silent_launch = true;
      }
    }
  }
...
#if defined(OS_WIN)
  // If --uninstall-app-id is specified, remove the target extension
  if (command_line.HasSwitch(switches::kUninstallAppId)) {

    std::string app_id = command_line.GetSwitchValueASCII(
                                  switches::kUninstallAppId);
    Profile* profile = nullptr;

    if (can_use_last_profile) {
      profile = last_used_profile;
    } else {
      const base::FilePath& user_data_dir =
                  g_browser_process->profile_manager()->user_data_dir();
      profile = GetStartupProfile(user_data_dir, command_line, cur_dir);
      if (!profile)
    ...
    // 'true' return will allow uninstall dialog draw time.
    return true;
  }

  if (!ProcessSearchCommandLineSwitches(command_line))
    return false;

  // If --migrate-edgeuwp is specified, migrate edgeuwp data
  if (command_line.HasSwitch(switches::kMigrateEdgeUwp)) {
    Profile* profile = nullptr;
  ...
  if (command_line.HasSwitch(switches::kEmbeddedBrowserWebview)) {
    embedded_browser::EstablishConnection(command_line);
  ...
  if (wdag::IsContainerInstance()) {
    // This comes in as a separate instance of the browser attaching
    // to this instance. Let's let the host know that we're navigating
    // at its behest.
    DCHECK(command_line.HasSwitch(wdag::switches::kWdagNavigationId));
    g_browser_process->wdag_manager()->OnContainerNavigating(
        command_line.HasSwitch(wdag::switches::kWdagNavigationId) ?
            command_line.GetSwitchValueASCII(
                wdag::switches::kWdagNavigationId) :
            "navigation id missing",
        command_line.HasSwitch(switches::kSilentLaunch));
  }

  if (wdag::IsContainerInstance()) {
    // Let HVSI know that we're running. They key off a "visible" window,
    // and by launching silently, they'll think the launch failed, and then
  ...
  if (wdag::IsHostInstance()) {
    if (command_line.HasSwitch(wdag::switches::kWdagContainerNavigation))
      wdag::host::NavigateInContainer(
          GURL(command_line.GetSwitchValueASCII(
              wdag::switches::kWdagContainerNavigation)),
...
#if defined(OS_WIN)
bool StartupBrowserCreator::ProcessSearchCommandLineSwitches(
      const base::CommandLine& command_line) {

  base::win::ScopedHandle hPipe;
  DWORD dwWritten;
  bool return_value = true;

  if (command_line.HasSwitch(switches::kOutputPipeName)) {
    auto sw_value = command_line.GetSwitchValueASCII(switches::kOutputPipeName);

    if (!sw_value.empty()) {
      for (size_t i = 0; i < sw_value.length(); i++)
        if (!isalnum(sw_value[i]))
          return false;

      auto pipe_name = base::ASCIIToUTF16("\\\\.\\pipe\\" + sw_value);
      hPipe.Set(CreateFile(pipe_name.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
                           NULL, OPEN_EXISTING, 0, NULL));
    }
  }

  if (command_line.HasSwitch(switches::kOnStartup)) {
    auto sw_value = command_line.GetSwitchValueASCII(switches::kOnStartup);
    ProfileManager* profile_manager = g_browser_process->profile_manager();
    auto* storage = &profile_manager->GetProfileAttributesStorage();
    auto all_profiles = storage->GetAllProfilesAttributes();
    bool all_recommended = true;
    for (ProfileAttributesEntry* entry : all_profiles) {
      ...
      int restore = profile->GetPrefs()->GetInteger(prefs::kRestoreOnStartup);

      if (sw_value == "?") {
        if (restore != 1 && restore != 5) {
          all_recommended = false;
          break;
        }
      } else if (sw_value == "reset") {
        profile->GetPrefs()->SetInteger(prefs::kRestoreOnStartup, 5);
      }
    }

    if (sw_value == "?" && hPipe.IsValid()) {
        std::string output = (all_recommended ? "1" : "0");
        WriteFile(hPipe.Get(), output.c_str(), output.size(), &dwWritten, NULL);
    }

    // We don't need to process further just stops the start-up.
    return_value = false;
  }

  if (command_line.HasSwitch(switches::kDefaultSearchProvider)) {
    auto sw_value = command_line.GetSwitchValueASCII(
                                              switches::kDefaultSearchProvider);
    ProfileManager* profile_manager = g_browser_process->profile_manager();
    auto* storage = &profile_manager->GetProfileAttributesStorage();
    auto all_profiles = storage->GetAllProfilesAttributes();
    bool all_recommended = true;
    for (ProfileAttributesEntry* entry : all_profiles) {
      ...
      auto* url_service = TemplateURLServiceFactory::GetForProfile(profile);
      if (sw_value == "reset") {
        if (url_service->is_default_search_managed())
          // If DSP is managed don't change anything!
          continue;
        bool IsBingPrevDSP = search::DefaultSearchProviderIsBing(url_service);
        if (url_service->IsExtensionControlledDefaultSearch()) {
          auto* t_url = url_service->GetTemplateURLForGUID(
                          url_service->GetDefaultSearchProvider()->sync_guid());
          extensions::ExtensionSystem::Get(profile)->
              extension_service()->
              DisableExtension(t_url->GetExtensionId(),
        ...
        // If the DSP is reset to Bing and Bing isn't DSP before, update partner
        // code in registry key if there has a PC setting in command line.
        if (command_line.HasSwitch(switches::kPartnerCode) &&
          url_service->IsBingRecommended() && !IsBingPrevDSP) {
          std::string pc_value = command_line.GetSwitchValueASCII(
                                  switches::kPartnerCode);
          profile->GetPrefs()->SetString(prefs::kPartnerCode, pc_value);
        }
      } else if (sw_value == "?") {
        if (!url_service->IsDefaultRecommended() &&
            !url_service->is_default_search_managed()) {
          all_recommended = false;
          break;
        }
      }
...
bool StartupBrowserCreator::LaunchBrowserForLastProfiles(
  ...
  if (last_opened_profiles.empty() || was_windows_notification_launch) {
    ...
    // Edge has no UserManager so if CanOpenProfileOnStartup(last_used_profile)
    // returns false, we fall through to ProcessLastOpenedProfiles which will
    // call LaunchBrowser for last_used_profile even though it _shouldn't_ be
    // opened because CanOpenProfileOnStartup indicates that the profile is
    // locked. This codepath doesn't fire in Edge because we don't have a
    // profile locking mechanism but if we implement profile locking, this
    // browser launch path will need to be fixed to launch some locked profile
    // screen.
    NOTREACHED() << "Edge is trying to launch a profile that is locked, this "
...
#if !EXCLUDED_FROM_EDGE
  // In Edge we don't have a UserManager so if we get to this point and
  // |is_process_startup| indicates that we haven't launched a browser, just
  // launch a browser with the last used profile.
  if (is_process_startup == chrome::startup::IS_PROCESS_STARTUP) {
    return LaunchBrowser(command_line, last_used_profile, cur_dir,
                         is_process_startup, is_first_run);
  }
...
// all others.
  ...
  if (base::FeatureList::IsEnabled(
          features::edge::kChooseProfileForExternalLinkOpening)) {
    const base::CommandLine::StringVector& params = command_line.GetArgs();
    const bool launch_has_urls =
        std::any_of(params.begin(), params.end(), [&cur_dir](auto param) {
          // This logic is copied from
          // StartupBrowserCreator::GetURLsFromCommandLine and should mirror the
          // functionality, less the parts that rely on a Profile*.
          base::FilePath path = base::FilePath(param);
          GURL url = GURL(path.MaybeAsASCII());

          if (!url.is_valid() || url.SchemeIsFile()) {
            base::ThreadRestrictions::ScopedAllowIO allow_io;
            url = url_formatter::FixupRelativeFile(cur_dir, path);
          }
    ...
    if (launch_has_urls) {
      PrefService* service = g_browser_process->local_state();
      const auto behavior = static_cast<EdgeOpenExternalLinksProfileBehavior>(
          service->GetInteger(prefs::kEdgeOpenExternalLinksProfileBehavior));

      // If not using custom profile choice, we fall through to existing
      // logic of last used profile launch.
      if (behavior ==
          EdgeOpenExternalLinksProfileBehavior::kOpenWithChosenProfile) {
        const auto profile_path =
            service->GetFilePath(prefs::kEdgeOpenExternalLinksProfilePath);
        ...
        if (profile_manager->GetProfileAttributesStorage()
          ...
          // This is a valid profile and the browser can be launched with it.
          return profile_path;
        } else {
          // The pref has a value that isn't valid. Reset the prefs to a good
          // state and fall through to open the last used profile.
          edge::profiles::SetExternalLinkBehaviorChoice(
              EdgeOpenExternalLinksProfileBehavior::kOpenWithLastUsed);
        }
      }
    }
  }
...
Profile* GetStartupProfile(const base::FilePath& user_data_dir,
  ...
  base::FilePath profile_path =
      GetStartupProfilePath(user_data_dir, command_line, cur_dir);
  ...
  // Edge UI does not have user manager, so return null to trigger the fallback
  // logic.
  return nullptr;
...
Profile* GetFallbackStartupProfile() {
  ...
  // Skip for Edge UI since it does not have a user manager.
#if EXCLUDED_FROM_EDGE
  // Couldn't initialize any last opened profiles. Try to show the user
  // manager, which requires successful initialization of the guest and system
  // profiles.
  ...
  // locked, starting with the last active.
  auto all_profiles = storage->GetAllProfilesAttributes();
  std::sort(all_profiles.begin(), all_profiles.end(),
            [](ProfileAttributesEntry* a, ProfileAttributesEntry* b) {
  ...
  // All failed, fall back to the default profile.
  return profile_manager->CreateInitialProfile();
}
