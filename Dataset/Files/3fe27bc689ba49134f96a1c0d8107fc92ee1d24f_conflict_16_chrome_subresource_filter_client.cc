...
#include "components/edge_trust/ad_blocking/ad_blocking_activation_throttle.h"
...
bool ShouldCheckUrl(GURL url) {
  components::msft_util::URLType url_type =
      components::msft_util::ClassifyUrlSpecAsURLType(url.spec());
  return (url.SchemeIsHTTPOrHTTPS() &&
          !(url_type == components::msft_util::URLType::OFFICE_NEW_TAB_URL ||
            url_type == components::msft_util::URLType::NEWS_NEW_TAB_URL ||
            url_type == components::msft_util::URLType::LOCAL_NEW_TAB_URL));
}
...
ChromeSubresourceFilterClient::ChromeSubresourceFilterClient(
  ...
  // This is necessary because ChromeSubresourceFilterClient is instantiated
  // in several tests in unit_tests, which doesn't do a full browser process
  // initialization and therefore doesn't contain a TrustManager.
  if (g_browser_process->trust_manager()) {
    ads_checker_ = g_browser_process->trust_manager()->AdsChecker();
  }
}
...
void ChromeSubresourceFilterClient::MaybeAppendNavigationThrottles(
  ...
  if (navigation_handle->IsInMainFrame() && ads_checker_ != nullptr &&
      ShouldCheckUrl(navigation_handle->GetURL())) {
    throttles->push_back(
        std::make_unique<ad_blocking::AdBlockingActivationThrottle>(
            navigation_handle, this, ads_checker_));
  }
