...
const int kCardFIDbitLimitStart = 12;
const int kTravelFIDbitLimitStart = 24;
const int KProfileFIDRange = 4095;
const int kCardFIDRange = 16773120;
const int kIntIDMapLimit = 2047;
const int kIntIDMapTravelLimit = 127;
...
// Keys corresponding to the lists of forms included in feedback data as JSON.
constexpr char kQueryableKey[] = "queryable_forms";
constexpr char kNonQueryableKey[] = "non_queryable_forms";
...
void SaveFormInfoForFeedback(const FormStructure* form,
                             base::Value& storage) {
  base::Value form_value(base::Value::Type::DICTIONARY);
  form_value.SetKey("name", base::Value(form->form_name()));
  form_value.SetKey("signature", base::Value(form->FormSignatureAsStr()));
  form_value.SetKey("fields", base::Value(base::Value::Type::LIST));

  for (const std::unique_ptr<AutofillField>& field : *form) {
    base::Value field_value(base::Value::Type::DICTIONARY);
    field_value.SetKey("name", base::Value(field->parseable_name()));
    field_value.SetKey("signature", base::Value(field->FieldSignatureAsStr()));
    form_value.FindKey("fields")->Append(std::move(field_value));
  }

  storage.Append(std::move(form_value));
}

void SaveFormsForFeedback(base::Value& feedback_info,
                          const std::vector<FormStructure*>& forms,
                          const std::string key) {
  base::Value* value = feedback_info.FindKey(key);
  DCHECK(value);
  for (const FormStructure* form : forms) {
    SaveFormInfoForFeedback(form, *value);
  }
}
...
// Triggers the microsoft pay feature flag only if the user is logged into
// the browser using an MSA account. The reason for this resriction is that
// MSPay only works for MSA users.
void TriggerMicrosoftPayForMSA(signin::IdentityManager* identity_manager) {
  base::Optional<CoreAccountInfo> account_info = base::nullopt;
  ...
  if (account_info &&
      account_info->edge_signin_info.account_type == EdgeAccountType::kMSA) {
    base::FeatureList::TriggerUsage(features::kMicrosoftPay);
  }
}
...
bool AutofillManager::ShouldShowCardsFromAccountOption(
    ...
    const FormFieldData& field) {
  // BEGIN_EDGE_AUTOFILL_CODE
  if (base::FeatureList::IsEnabled(features::kAutofillDisableGooglePay)) {
    return false;
  }
  // END_EDGE_AUTOFILL_CODE
...
void AutofillManager::OnTextFieldDidChangeImpl(const FormData& form,
  ...
  if (!user_did_type_) {
    ...
    // BEGIN_EDGE_AUTOFILL_CODE
    EdgeAutofillMetrics::LogEdgeUserHappinessMetric(
        EdgeAutofillMetrics::EdgeUserHappinessMetric::USER_DID_TYPE,
  ...
  if (autofill_field->is_autofilled) {
    ...
    // BEGIN_EDGE_AUTOFILL_CODE
    EdgeAutofillMetrics::LogEdgeUserHappinessMetric(
        EdgeAutofillMetrics::EdgeUserHappinessMetric::
...
void AutofillManager::FillOrPreviewCreditCardForm(
  ...
  if (should_fetch_card) {
    ...
    // If the suggested card is untagged, offer the user to tag the card.
    if (base::FeatureList::IsEnabled(features::kAutofillCreditCardTag,
                                     /*trigger_usage_on_check=*/true)) {
      if (!credit_card_.GetTag().length()) {
        // Only offer to tag local untagged cards.
        if (credit_card_.record_type() == CreditCard::RecordType::LOCAL_CARD &&
            !driver()->IsIncognito()) {
          client_->GetFormDataImporter()->TryToTagCreditCard(credit_card_);
        }
        // Log histogram when user selected untagged credit card suggestion
        EdgeAutofillMetrics::LogCreditCardTagSuggestionSelectionMetric(
            EdgeAutofillMetrics::CreditCardTagSuggestionSelectionMetric::
                CREDIT_CARD_SUGGESTION_SELECTED_UNTAGGED);
      } else {
        // Log histogram when user selected tagged credit card suggestion
        EdgeAutofillMetrics::LogCreditCardTagSuggestionSelectionMetric(
            EdgeAutofillMetrics::CreditCardTagSuggestionSelectionMetric::
                CREDIT_CARD_SUGGESTION_SELECTED_TAGGED);
      }
    } else {
      // Log histogram when user selected untagged credit card suggestion
      EdgeAutofillMetrics::LogCreditCardTagSuggestionSelectionMetric(
...
// BEGIN_EDGE_AUTOFILL_CODE
void AutofillManager::FillOrPreviewTravelForm(
    AutofillDriver::RendererFormDataAction action,
    int query_id,
    const FormData& form,
    const FormFieldData& field,
    const AutofillTravel& travel) {
  FormStructure* form_structure = nullptr;
  AutofillField* autofill_field = nullptr;
  if (!GetCachedFormAndField(form, field, &form_structure, &autofill_field))
    return;
  ...
  FillOrPreviewDataModelForm(
      action, query_id, form, field, travel, /*is_credit_card=*/false,
      /*cvc=*/base::string16(), form_structure, autofill_field);
}
// END_EDGE_AUTOFILL_CODE
...
void AutofillManager::FillOrPreviewForm(
  ...
  // BEGIN_EDGE_AUTOFILL_CODE
  const AutofillTravel* travel = nullptr;
  // END_EDGE_AUTOFILL_CODE

  if (base::FeatureList::IsEnabled(features::kAutofillEdgeTransientFormFill)) {
    travel = GetTravel(unique_id);
  }
  ...
  if (credit_card)
    ...
    FillOrPreviewTravelForm(action, query_id, form, field, *travel);
  }

// BEGIN_EDGE_AUTOFILL_CODE
  if (autofill::features::IsMicrosoftPayEnabled(client_->GetPrefs())
      && credit_card) {
    CreditCard::RecordType cardRecordType = credit_card->record_type();
    AutofillMetrics::UpdateCardSelectionHistogram(
      personal_data_->GetServerCreditCardCount() > 0,
      (cardRecordType == CreditCard::RecordType::MASKED_SERVER_CARD ||
      cardRecordType == CreditCard::RecordType::FULL_SERVER_CARD));

    AutofillMetrics::UpdateCardSelectionTypeHistogram(cardRecordType);
  }
// END_EDGE_AUTOFILL_CODE
}
...
void AutofillManager::OnDidFillAutofillFormData(const FormData& form,
  ...
  // BEGIN_EDGE_AUTOFILL_CODE
  EdgeAutofillMetrics::LogEdgeUserHappinessMetric(
      EdgeAutofillMetrics::EdgeUserHappinessMetric::SUGGESTIONS_FILLED,
      form_types);
  // END_EDGE_AUTOFILL_CODE
  ...
  if (!user_did_autofill_) {
    ...
    // BEGIN_EDGE_AUTOFILL_CODE
    EdgeAutofillMetrics::LogEdgeUserHappinessMetric(
        EdgeAutofillMetrics::EdgeUserHappinessMetric::SUGGESTIONS_FILLED_ONCE,
        form_types);
    // END_EDGE_AUTOFILL_CODE
  }

  // BEGIN_EDGE_AUTOFILL_CODE
  client_->RaiseAutofillSuggestionFilledEvent();
  // END_EDGE_AUTOFILL_CODE
...
void AutofillManager::DidShowSuggestions(bool has_autofill_suggestions,
  ...
  // BEGIN_EDGE_AUTOFILL_CODE
  EdgeAutofillMetrics::LogEdgeUserHappinessMetric(
      EdgeAutofillMetrics::EdgeUserHappinessMetric::SUGGESTIONS_SHOWN,
      autofill_field->Type().group());
  // END_EDGE_AUTOFILL_CODE
  ...
  if (!did_show_suggestions_) {
    ...
    // BEGIN_EDGE_AUTOFILL_CODE
    EdgeAutofillMetrics::LogEdgeUserHappinessMetric(
        EdgeAutofillMetrics::EdgeUserHappinessMetric::SUGGESTIONS_SHOWN_ONCE,
        autofill_field->Type().group());
    // END_EDGE_AUTOFILL_CODED
  }

  if (autofill_field->Type().group() == CREDIT_CARD) {
    base::TimeTicks credit_card_suggestion_show_time = base::TimeTicks::Now();
    DCHECK(credit_card_suggestion_show_time >
           credit_card_suggestion_poll_time_);
    base::TimeDelta elapsed =
        credit_card_suggestion_show_time - credit_card_suggestion_poll_time_;
    UMA_HISTOGRAM_TIMES("Microsoft.Autofill.TimeTakenToShowOptions.Card",
                        elapsed);
  } else if (autofill_field->Type().group() != TRAVEL) {
    base::TimeTicks profile_suggestion_show_time = base::TimeTicks::Now();
    DCHECK(profile_suggestion_show_time > profile_suggestion_poll_time_);
    base::TimeDelta elapsed =
        profile_suggestion_show_time - profile_suggestion_poll_time_;
    UMA_HISTOGRAM_TIMES("Microsoft.Autofill.TimeTakenToShowOptions.Profile",
                        elapsed);
  }
...
bool AutofillManager::RemoveAutofillProfileOrCreditCard(int unique_id) {
  ...
  if (base::FeatureList::IsEnabled(features::kAutofillEdgeTransientFormFill)) {
    const AutofillTravel* travel = GetTravel(unique_id);
    if (travel)
      personal_data_->RemoveByGUID(travel->guid());
    return true;
  }
...
void AutofillManager::RemoveAutocompleteEntry(const base::string16& name,
                                              const base::string16& value) {
  // BEGIN_EDGE_AUTOFILL_CODE
  client_->RaiseAutofillSuggestionDeletedEvent();
  // END_EDGE_AUTOFILL_CODE
...
AutofillManager::AutofillManager(
    ...
    : AutofillHandler(driver, client->GetLogManager()),
      ...
      credit_card_form_event_logger_(
          std::make_unique<CreditCardFormEventLogger>(
              ...
              personal_data_,
              client_)),
      feedback_info_(base::Value::Type::DICTIONARY),
  ...
  if (enable_download_manager == ENABLE_AUTOFILL_DOWNLOAD_MANAGER) {
    ...
    download_manager_->SetLocalState(client_->GetLocalState());
#endif
  }
  CountryNames::SetLocaleString(app_locale_);

  if (base::FeatureList::IsEnabled(features::kAutofillSignaturesInFeedback)) {
    feedback_info_.SetKey(kQueryableKey, base::Value(base::Value::Type::LIST));
    feedback_info_.SetKey(kNonQueryableKey,
...
bool AutofillManager::RefreshDataModels() {
  ...
  //  BEGIN_EDGE_AUTOFILL_CODE
  const std::vector<AutofillTravel*>& travels = personal_data_->GetTravels();
  // END _EDGE_AUTOFILL_CODE
...
// BEGIN_EDGE_AUTOFILL_CODE
AutofillTravel* AutofillManager::GetTravel(int unique_id) {
  // Unpack the |unique_id| into component parts.
  std::string travel_id;
  SplitFrontendIDTravel(unique_id, &travel_id);

  if (base::IsValidGUID(travel_id))
    return personal_data_->GetTravelByGUID(travel_id);
  return nullptr;
}
// END_EDGE_AUTOFILL_CODE
...
std::vector<Suggestion> AutofillManager::GetProfileSuggestions(
  ...
  if (!base::FeatureList::IsEnabled(features::kAutofillEdgeSuggestionUI)) {
...
std::vector<Suggestion> AutofillManager::GetCreditCardSuggestions(
  ...
  *should_display_gpay_logo =
  ...
  // BEGIN_EDGE_AUTOFILL_ CODE
  autofill::EdgeAutofillMetrics::LogMSPayCardSuggestionShare(
    personal_data_->GetServerCreditCardCount());
  autofill::EdgeAutofillMetrics::LogLocalCardSuggestionShare(
    personal_data_->GetLocalCreditCardCount());
  // Log the number of tagged vs untagged cards in the suggestions
  EdgeAutofillMetrics::LogCreditCardTagSuggestionCountMetric(
    personal_data_->GetTaggedCreditCardCount(),
    personal_data_->GetUntaggedCreditCardCount());
  // END_EDGE_AUTOFILL_CODE
...
// BEGIN_EDGE_AUTOFILL_CODE
std::vector<Suggestion> AutofillManager::GetTravelSuggestions(
    const FormFieldData& field,
    const AutofillType& type) const {
  std::vector<Suggestion> suggestions =
      personal_data_->GetTravelSuggestions(type, field.value);

  for (size_t i = 0; i < suggestions.size(); i++) {
    suggestions[i].frontend_id =
        MakeFrontendIDTravel(suggestions[i].backend_id);
  }
...
void AutofillManager::OnFormsParsed(
  ...
  for (FormStructure* form_structure : form_structures) {
    ...
    if (card_form) {
      ...
      if (autofill::features::IsMicrosoftPayEnabled(client_->GetPrefs()) &&
          IsAutofillCreditCardEnabled()) {
        DCHECK(microsoft_pay_card_enlist_);
        microsoft_pay_card_enlist_->RefreshLocallySavedServerCards();
      }
    }
    if (address_form) {
  ...
  // Record form and field signatures for feedback.
  if (base::FeatureList::IsEnabled(features::kAutofillSignaturesInFeedback)) {
    SaveFormsForFeedback(feedback_info_, queryable_forms, kQueryableKey);
    SaveFormsForFeedback(feedback_info_, non_queryable_forms, kNonQueryableKey);
  }
}
...
int AutofillManager::MakeFrontendID(
  ...
  // In case of Travel put highest byte of 32-bit integers ie above 24 bits
  // For Credit card next 12 bit and for profile lower 12 bit
  // 0000 0000 0000 0000 0000 0000 0000 0000
  // |_______| |____________| |____________|
  //    |             |             |
  // Travel     Credit Card       Profile
  if (base::FeatureList::IsEnabled(features::kAutofillEdgeTransientFormFill)) {
    DCHECK(cc_int_id <= kIntIDMapLimit);
    DCHECK(profile_int_id <= kIntIDMapLimit);
    return (cc_int_id << kCardFIDbitLimitStart) | profile_int_id;
  }
  // Put CC in the high half of the bits.
    ...
    return (cc_int_id << std::numeric_limits<uint16_t>::digits) |
           profile_int_id;
  }
}

// BEGIN_EDGE_AUTOFILL_CODE
int AutofillManager::MakeFrontendIDTravel(
    const std::string& travel_backend_id) const {
  int travel_int_id = BackendIDToInt(travel_backend_id);

  DCHECK(travel_int_id <= kIntIDMapTravelLimit);

  // For travel frontend id will be upper 24 bit integer.
  return (travel_int_id << kTravelFIDbitLimitStart);
}
// END_EDGE_AUTOFILL_CODE
...
void AutofillManager::SplitFrontendID(int frontend_id,
  ...
  if (base::FeatureList::IsEnabled(features::kAutofillEdgeTransientFormFill)) {
    // it's obvious we have alloted higher bits to travel
    int cc_int_id = (frontend_id & kCardFIDRange) >> kCardFIDbitLimitStart;
    int profile_int_id = frontend_id & KProfileFIDRange;
...
// BEGIN_EDGE_AUTOFILL_CODE
void AutofillManager::SplitFrontendIDTravel(
    int frontend_id,
    std::string* travel_backend_id) const {
  int travel_int_id = (frontend_id >> kTravelFIDbitLimitStart);

  *travel_backend_id = IntToBackendID(travel_int_id);
}
// END_EDGE_AUTOFILL_CODE
...
void AutofillManager::GetAvailableSuggestions(
  ...
  if (got_autofillable_form) {
    if (context->focused_field->Type().group() == CREDIT_CARD) {
      ...
      TriggerMicrosoftPayForMSA(client_->GetIdentityManager());
    }
    if (base::FeatureList::IsEnabled(
            features::kAutofillEdgeTransientFormFill) &&
        context->focused_field->Type().group() == TRAVEL) {
      context->is_filling_travel = true;
    }
  ...
  if (context->is_filling_credit_card) {
    *suggestions =
        GetCreditCardSuggestions(field, context->focused_field->Type(),
                                 &context->should_display_gpay_logo);
    credit_card_suggestion_poll_time_ = base::TimeTicks::Now();
  ...
  } else if (context->is_filling_travel) {
    *suggestions = GetTravelSuggestions(field, context->focused_field->Type());
  } else {
    ...
    *suggestions = GetProfileSuggestions(*context->form_structure, field,
                                         *context->focused_field);
    profile_suggestion_poll_time_ = base::TimeTicks::Now();
  }
...
FormEventLoggerBase* AutofillManager::GetEventFormLogger(
  ...
  switch (field_type_group) {
    ...
    case NO_GROUP:
    case UNFILLABLE:
    case TRAVEL:      //TODO: dechouha- Add new transient event logger
      return nullptr;
  }
  NOTREACHED();
  return nullptr;
...
// BEGIN_EDGE_AUTOFILL_CODE
void AutofillManager::InitMSPayEnlistHandler(){
  // If the feature was enabled and microsoft_pay_card_enlist_ was not set,
  // set it.
  if (autofill::features::IsMicrosoftPayEnabled(client_->GetPrefs())) {
    microsoft_pay_card_enlist_.reset(
      new MicrosoftPayCardEnlist(driver(),
      client_->GetIdentityManager(), personal_data_));
  }
}

void AutofillManager::ResetPayEnlistHandler(){
  // Set to nullptr. This is used during test.
  microsoft_pay_card_enlist_.reset(nullptr);
}

// Refreshing the locally saved server cards
void AutofillManager::RefreshLocallySavedServerCardsWrapper(){
  if (autofill::features::IsMicrosoftPayEnabled(client_->GetPrefs())) {
    DCHECK(microsoft_pay_card_enlist_);
    microsoft_pay_card_enlist_->RefreshLocallySavedServerCards();
  }
